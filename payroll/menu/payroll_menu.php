   ...
   function getMenusObjs(){
      ...
      $seesionVars = $this->getSeesionVars();
		if($_SESSION['pay_category'] == 9)
   	  ...
   	  $arrUserLogInInfo = $this->getUserLogInInfoForMenu($seesionVars['empNo']);
	  $pages = 	$arrUserLogInInfo['pages1'] .$arrUserLogInInfo['pages2'];
     $qry="select * from tblPayrollMenu 
      		WHERE moduleStat = 'A' 
      		AND moduleId IN ($pages) $filter";
...
/*					if($arrUserLogInInfo['userLevel']!=1 && $obj->isAdminPage==1){
   ...
   public function render(){
		...
		$js = json_encode($menus);//convet to JSON
		echo $js;
   }
}
