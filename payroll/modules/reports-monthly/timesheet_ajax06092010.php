...
switch ($inputId) 
{
	case "empSearch":		
		...
		{ 
			...
			if ($numEmp>0) 
			{
				$resTS = $inqTSObj->countRec($compCode,$empNo,'',$payPd,WTAX,$table);
				if (mysql_num_rows($resTS) > 0) 
					echo "location.href = 'tax_list.php?inputId=$optionId&empNo=$empNo&empName=$empName&empDiv=$empDiv&empDept=$empDept&empSect=$empSect&groupType=$groupType&orderBy=$orderBy&catType=$catType&payPd=$payPd&table=$table';";
				else 
