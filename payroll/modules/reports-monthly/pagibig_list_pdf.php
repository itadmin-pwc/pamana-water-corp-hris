...
################### INCLUDE FILE #################
	...
	class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->Cell(70,5,"Report ID: PGIBGR01");
			
			$hTitle = "Pag-Ibig Report for the Period of ".$this->pdHeadTitle;
			...
			$this->Cell(30,6,'PAG-IBIG CONT.',0,'','R');
		...
		function displayContent($resQry,$table,$payPd)
		{
			...
			{
				$rTotal = $this->getTaxes($resQry_val['empNo'],$payPd,PAGIBIG_CONTRIB,$table);
				$rTotalHist = $this->getTaxesHist($resQry_val['empNo'],$payPd,PAGIBIG_CONTRIB,$table);
				$sumperMontPag =$rTotal["totAmt"] + $rTotalHist["totAmt"];
				...
				if($sumperMontPag!=0)
				{
					...
					$this->Cell(30,6,number_format($sumperMontPag,2),0,'','R');
					$this->Ln();
					$grandtot+=$sumperMontPag;
					$ctr_emp++;
				}
			}
			/*Grand Total*/
