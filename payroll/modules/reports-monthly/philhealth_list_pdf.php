...
################### INCLUDE FILE #################
	...
	class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->Cell(70,5,"Report ID: PHIRPT01");
			
			$hTitle = "Philhealth Report for the Period of ".$this->pdHeadTitle;
		...
		function displayContent($resQry,$table,$payPd)
		{
			...
			{
				$rTotal = $this->getTaxes($resQry_val['empNo'],$payPd,PHILHEALTH_CONTRIB,$table);
				$rTotalHist = $this->getTaxesHist($resQry_val['empNo'],$payPd,PHILHEALTH_CONTRIB,$table);
				$sumperMontPhic =$rTotal["totAmt"] + $rTotalHist["totAmt"];
				...
				if($sumperMontPhic!=0)
				{
					...
					$this->Cell(30,6,number_format($sumperMontPhic,2),0,'','R');
					$this->Ln();
					$grandtot+=$sumperMontPhic;
					$ctr_emp++;
				}
			}
			/*Grand Total*/
