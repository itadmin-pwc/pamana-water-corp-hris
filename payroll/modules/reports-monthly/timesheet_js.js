// JavaScript Document
	...
	function valSearchTS_TaxReport(thisValue) {
		var optionId=document.frmTS.hide_option.value;
		...
		new Ajax.Request(
		  'timesheet_ajax.php?inputId=empSearch&optionId='+optionId+'&fileName='+fileName+'&payPd='+payPd+'&empBrnCode='+empBrnch+'&thisValue='+thisValue+'&topType='+topType,
		  {
			 asynchronous : true,     
			 onComplete   : function (req){
				eval(req.responseText);
	...
	function valSearchTS_MonthlyGovt(thisValue) {
		var optionId=document.frmTS.hide_option.value;
		var fileName=document.frmTS.fileName.value;
		var payPd=document.frmTS.payPd.value;
		...
		new Ajax.Request(
		  'timesheet_ajax.php?inputId=empSearch&optionId='+optionId+'&fileName='+fileName+'&payPd='+payPd+'&thisValue='+thisValue+'&type='+type,
		  {
			 asynchronous : true,     
			 onComplete   : function (req){
				eval(req.responseText);
	...
	function valSearchTS(thisValue) {
		...
		var payPd=document.frmTS.payPd.value;
		var ext="";
		if (thisValue == "monthly_ytd_payreg" || thisValue == "resigned_emp" || thisValue == "yearly_ytd" || thisValue == "monthly_ytd"  || thisValue=="yearly_ytd_payreg"  || thisValue=="monthly_je") {
	...
	function paySum() {
		var brnch=document.frmTS.empBrnCode.value;
		var payPdfr=document.frmTS.payPdfr.value;
		var payPdto=document.frmTS.payPdto.value;
		if (brnch==0)  {
			alert("Branch is required.");
			return false;
		} else if (payPdfr==0)  {
			alert("From Payroll Period is required.");
			return false;
		} else if (payPdto==0) {
			alert("To Payroll Period is required.");
			return false;
		}
		
		window.open('paySum_pdf.php?brnch='+brnch+'&payPdfr='+payPdfr+'&payPdto='+payPdto);
		
	}	
	
	function MonthlyLoansType(type,payPd) {
		new Ajax.Request(
		  'timesheet_ajax.php?inputId=MonthlyLoansType&type='+type+'&payPd='+payPd,
		  {
			 asynchronous : true,     
			 onComplete   : function (req){
				$('pdPay').innerHTML=req.responseText;
