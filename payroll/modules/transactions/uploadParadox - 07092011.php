...
function getRemarks($tsDate,$empNo)
{
	global $db;
	$finalRem = "";
	
	$qryGetRem = "SELECT [OT RATE CODE] as otRateCode FROM [t_templ_ho]
				  WHERE [DATE] = #{$tsDate}# AND [EMPLOYEE ID#] = '{$empNo}'";
	$resGetRem = $db->Execute($qryGetRem);
			...
			{ 
				...
				if($resGetParadoxData->recordCount() > 0)
				{
					...
					while(!$resGetParadoxData->EOF)
					{
						...
						$qryToNewTsParadaox.= " INSERT INTO tblTsParadox
												(compCode,
												 empNo,
												 tsDate,
												 hrsAbsent,
												...
												('{$_SESSION['company_code']}',
												 ...
												 '".getRemarks($resGetParadoxData->fields['tsDate'],$resGetParadoxData->fields['empNo'])."-".substr($file,0,strlen($file)-23)."');";
					 ...
					 $ins_Statement = $qryToNewTsParadaox;
				...
				$qryGetParadoxData_NWCutOff = "SELECT [EMPLOYEE ID#] as empNo,[DATE] as tsDate,
								  ...
								  FROM [t_templ_ho]
								  GROUP BY  [EMPLOYEE ID#],[DATE],[INCOME TYPE],[ALLOW TYPE],[ALLOWANCE AMOUNT] ORDER BY [DATE]";
				...
				if($resGetParadoxData_NWCutOff->recordCount() > 0)
				{
					...
					while(!$resGetParadoxData_NWCutOff->EOF)
					{
						...
						if(($tmpAllowAdj!=0) || ($tmpAllowAdj!="") )
						{
							...
							$earnTranDtl_AllowAdj.=  "Insert into tblEarnTranDtl(compCode, refNo, empNo, trnCntrlNo, trnCode, trnAmount,payGrp,payCat,earnStat,trnTaxCd)
											  values('".$_SESSION["company_code"]."','".$earnTran_refNo_AllowAdj."','".$resGetParadoxData_NWCutOff->fields['empNo']."','".$cntrlNo_AllowAdj."','".$arr_AllowType["trnCode"]."','".$tmpAllowAdj  ."','".$_SESSION["pay_group"]."','".$_SESSION["pay_category"]."','A','N');";
							$cntrlNo_AllowAdj++;
						}
					...
					if($earnTranDtl_AllowAdj!="")
						$ernTranHeader_AllowAdj.= "Insert into tblEarnTranHeader(compCode,refNo, trnCode, earnRem,earnStat, pdYear, pdNumber)
										 values('".$_SESSION["company_code"]."','".$earnTran_refNo_AllowAdj."','".ADJ_OT."','Allow-Adj on Hyper TS','A','".$pdYear."','".$pdNumber."');";
