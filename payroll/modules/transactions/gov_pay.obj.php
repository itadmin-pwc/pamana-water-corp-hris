<?
	class govPayObj extends commonObj {
		...
		function addGovPayment($agencyCd,$pdYear,$pdMonth,$bnkName,$orNo,$bnkBrnch,$bnkAdd,$totAmtPaid,$remarks,$paidBy,$datePaid)
		{
			...
			$paiddate=date('Y-m-d H:i:s',strtotime($datePaid));
			$createddate=date('Y-m-d H:i:s',strtotime("today"));
			$qryIns = "Insert into tblGovPayments(compCode,agencyCd,pdYear,
						...
						'".$pdMonth."',".$bnkName.",'".$orNo."',".$bnkBrnch.",".$bnkAdd.",'".sprintf("%01.2f",$totAmtPaid)."','".$createddate."',".$remarks.",
						'".str_replace("'","''",stripslashes($paidBy))."','A','".$paiddate."')";
