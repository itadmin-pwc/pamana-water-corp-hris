// JavaScript Document
	...
	function printTSList() {
		...
		document.frmTS.action = 'timesheet_list_pdf.php?empNo='+empNo+'&empName='+empName+'&empDiv='+empDiv+'&empDept='+empDept+'&empSect='+empSect+'&orderBy='+orderBy+'&payPd='+payPd+'&reportType='+reportType+'&empBrnCode='+empBrnCode+'&locType='+locType;
	...
	function valSearchTS(thisValue) {
		...
		new Ajax.Request(
		  'timesheet_ajax.php?hide_empSect='+hide_empSect+'&hide_empDept='+hide_empDept+'&inputId=empSearch&empNo='+empNo+'&empName='+empName+'&empDiv='+empDiv+'&empDept='+empDept+'&empSect='+empSect+'&optionId='+optionId+'&fileName='+fileName+'&orderBy='+orderBy+'&payPd='+payPd+'&thisValue='+thisValue+'&empBrnCode='+empBrnCode+'&locType='+locType,
		  {
			 asynchronous : true,     
			 onComplete   : function (req){
				eval(req.responseText);
	...
	function viewDetails(option,refNo,URL,ele,offset,isSearch,txtSrch,cmbSrch){
		var nUrl = "";
		if (option=="Earnings") nUrl="earnings_list_details.php?refNo="+refNo; else nUrl="deductions_list_details.php?refNo="+refNo;
		...
		resizable: false, 
		minimizable : false,
		title: option+" Proof List Detail", 
	...
	function processGenerateLoans() {
		...
		if (payPd=="" || payPd<0 || payPd=="0") {
			...
			new Ajax.Request(
			  'timesheet_ajax.php?inputId=generateLoans&payPd='+payPd,
			  {
				 asynchronous : true,     
				 onComplete   : function (req){
					eval(req.responseText);
