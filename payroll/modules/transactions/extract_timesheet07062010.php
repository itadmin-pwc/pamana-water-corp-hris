...
if(isset($_GET['action']) == 'procTS'){
	if($extrctTSObj->checkPeriodTimeSheetTag() > 0){
		echo 1;//"alert('Time Sheet for Desired Payroll Group / Category / Period Already Completed');";//already processed
	}
	else{
	...
	function procTimeSheet(){
		...
		new Ajax.Request('<?=$_SERVER['PHP_SELF']?>'+params,{
			...
			onComplete : function(req){
				...
				if(rsTxt == 1){
					alert('Time Sheet\nfor Desired Payroll Group / Category / Period\nAlready Completed');
				}
				if(rsTxt == 2){
