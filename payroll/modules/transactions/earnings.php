	...
	switch ($_GET['action']){
		...
		case 'getEmpInfo':
				if ($_SESSION['pay_category'] != 9) {	
					$empInfo = $earningsObj->getUserInfo($_SESSION['company_code'],$_GET['empNo'],'and empPayCat='.$_SESSION["pay_category"].' and empPayGrp='.$_SESSION["pay_group"].'');
				...
				if($empInfo == 0){
					...
					$payCat = $earningsObj->getPayCat($_SESSION['company_code'],"AND payCat = '{$_SESSION['pay_category']}'");
