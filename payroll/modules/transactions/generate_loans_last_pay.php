	...
	case "generateLoans":
		...
		if ($slctdPd['pdProcessTag']=="N" || $slctdPd['pdProcessTag']=="" || $slctdPd['pdProcessTag']==" ") {
			if ($slctdPd['pdLoansTag']=="N" || $slctdPd['pdLoansTag']=="" || $slctdPd['pdLoansTag']==" ") {
				$loanprocessresult=$loansObj->processLastPayLoans($compCode,$slctdPd,0);	
				...
				if ($loanprocessresult) {
					echo "alert('Generate Loan/Recurring Deductions for Last Pay Successfully created!')";
				}	
				else {
					echo "alert('Generate Loan/Recurring Deductions for Last Pay Failed!')";				
				}	
			} else {
				$loanprocessresult=$loansObj->processLastPayLoans($compCode,$slctdPd,1);	
				if ($loanprocessresult) {
					echo "alert('Re-process Loan/Recurring Deductions for Last Pay Successfully created!');";
				}	
				else {
					echo "alert('Re-process Loan/Recurring Deductions for Last Pay Failed!');";				
				}				
			}
		} else {
  ...
  <table cellpadding="0" cellspacing="1" class="parentGrid">
    <tr>
      ...
      <td width="365" class="parentGridHdr"> &nbsp;<img src="../../../images/grid.png">&nbsp;Generate 
        Loan/Recurring Deductions for Last Pay</td>
	</tr>
	<tr>
		<td height="180" class="parentGridDtl" >
          ...
          <tr> 
            ...
            <td class="style1">:</td>
            <td width="81%" class="gridDtlVal"> <div class="style5" id="pdPay"> 
...
// JavaScript Document
	function processGenerateLoans() {
		...
		if (payPd=="" || payPd<0 || payPd=="0") {
			...
			if (check==1) {
				new Ajax.Request(
				  'generate_loans_last_pay.php?code=generateLoans&payPd='+payPd,
				  {
					 asynchronous : true,     
