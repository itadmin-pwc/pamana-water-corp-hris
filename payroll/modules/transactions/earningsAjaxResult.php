		...
		<td class="parentGridDtl" valign="top">
			...
			<TABLE width="100%" cellpadding="1" cellspacing="1" border="0" class="hdrTable">
				...
				<tr>
					...
					<td class="gridDtlVal" colspan="4">
						<? 
							$opnPrd = $earningsObj->getPayPeriod_OtherEarn($_SESSION['company_code'],"AND payGrp = '{$_SESSION['pay_group']}' AND payCat = '{$_SESSION['pay_category']}' AND pdStat = 'O'");
							if($cmbPeriod == "-"){
								$cmbPeriod = $opnPrd['pdYear']."-".$opnPrd['pdNumber'];
							}
							$earningsObj->DropDownMenu(
								$earningsObj->makeArrDate(
									$earningsObj->getPayPeriod_OtherEarn($_SESSION['company_code'],"AND payGrp = '{$_SESSION['pay_group']}' AND payCat = '{$_SESSION['pay_category']}' AND pdNumber >= '{$opnPrd['pdNumber']}'"),'pdYear','pdNumber','pdPayable',''
