...
if(isset($_GET['action']) == 'populatPayCat'){

	$arrPayCat = $common->getPayCat($_GET['compCode'],'');
	echo $common->DropDownMenu($common->makeArr($arrPayCat,'payCat','payCatDesc',''),'cmbPayCategory','','class="inputs" style="width:200px;');
	exit();
}
...
if(isset($_GET['btnChnangeComp']) == 'SUBMIT'){
	...
	echo "parent.location.href='http://$_SERVER[HTTP_HOST]/".SYS_NAME."/".SYS_NAME_PAYROLL."';";
	exit();
}
							...
							<tr>
								...
								<td class="gridDtlVal" width="50%">
									<font class="gridDtlLblTxt">
										<?$common->DropDownMenu($common->makeArr($common->getCompany(''),'compCode','compName',''),'cmbCompny',$_SESSION['company_code'],'class="inputs" tabindex="1" onchange="populatePayCat(this.value)"');?>
									</font>
								</td>
							</tr>
							<tr>
								...
								<td class="gridDtlVal" width="50%">
									<font class="gridDtlLblTxt">
										<?$common->DropDownMenu($arrPayGrp,'cmbPayGroup',$_SESSION['pay_group'],'class="inputs"'); ?>
									</font>
								</td>
							</tr>
							<tr>
								...
								<td width="1%" class="gridDtlLbl2" align="center">:</td>
								<td class="gridDtlVal" width="50%">
									<font class="gridDtlLblTxt" id="gridDtlLblTxt">
									<?
										$arrPayCatInt = $common->getPayCat($_SESSION['company_code'],'');
									?>
										<?$common->DropDownMenu($common->makeArr($arrPayCatInt,'payCat','payCatDesc',''),'cmbPayCategory',$_SESSION['pay_category'],'class="inputs" onclick="checkComp()" style="width:200px;"'); ?>
									</font>
								</td>
							</tr>
							<tr>
