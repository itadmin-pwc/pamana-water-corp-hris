<?php
	...
	*/
	...
	class PDF extends FPDF
	{
		...
		function dispBranch()
		{
			$qryBranch = "Select brnCode, brnDesc from tblBranch
							...
							and ytdHist.empNo not in (Select empNo from tblLastPayEmp)
							) 
			...
			foreach($arrdispBranch as $arrdispBranch_val)
			{
				...
				{	
					if($arrEmpList_val["empBrnCode"]==$arrdispBranch_val["brnCode"])
					{
						...
						//$taxableAmt = ($arrEmpList_val["YtdTaxable"]  + $arrPrevEmplr["prevEarnings"] + $arrPrevEmplr["tax13th"]) - ($arrEmpList_val["YtdGovDed"] + $arrPrevEmplr["nonTaxSss"]) - ($arrEmpList_val["teuAmt"]);
						$estEarn = (((($arrEmpList_val["YtdTaxable"]  + $arrPrevEmplr["prevEarnings"]) - $arrEmpList_val["YtdGovDed"]) / 23)*24);
						
						$taxableAmt = $estEarn - $arrEmpList_val["teuAmt"];
						
						$taxDue = $this->getAnnualTax($taxableAmt);
						$taxDue = ($taxDue / 24) * 23;
						
						$empOvrUnder = $taxDue - $arrEmpList_val["YtdTax"] - $arrPrevEmplr["prevTaxes"];
						$aveTaxPay = ($taxDue  + $empOvrUnder) / 23;
						...
						if($arrEmpList_val["empWageTag"]=='N')
						{
							if(round($empOvrUnder,1)>0)
							{
									...
									$this->Cell(26.6,5,number_format($empOvrUnder,2),'1','0','R');
	...
	$qryListEmp = "Select ytdHist.compCode, pdYear, ytdHist.empNo, empLastName, empFirstName, empMidName, empBrnCode, empTin, empTeu, teuAmt, empWageTag, empPayType, dateHired, dateResigned,YtdGross, YtdTaxable, YtdGovDed, YtdTax,Ytd13NBonus,
					...
					and empmast.empTeu = tblTeu.teuCode and ytdHist.empNo not in (Select empNo from tblLastPayEmp)
