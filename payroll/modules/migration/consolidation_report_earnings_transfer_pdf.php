<?php
	...
	*/
	...
	class PDF extends FPDF
	{
		...
		{
			$prevYear = date("Y") - 1;
			$qrygetRcDual = "SELECT    sum(empBasic) as rcdBasic,sum(sprtAllowAdvance) as rcdAdvances
							FROM        tblPayrollSummaryHist
							where empNo='".$empNo."' and pdYear='".$prevYear."'";
		...
		function dispPaySum($empNo)
		{
			...
			$qryEmpAllow = "SELECT     empNo,pdYear, YtdGross, YtdTaxable, YtdGovDed, YtdTax, YtdNonTaxAllow, YtdBasic, sprtAdvance, basicReclass, allowReClass, Ytd13NBonus, sprtAdvance,YTd13NAdvance
			...
			if(count($arrEmpAllow_val)>=1)
			{
				...
				$this->Cell(10,5,'','','','L');
				$this->Cell(40,5,'DEC. - 2009','1','','C');
				$this->Cell(145,5,'JAN. - NOV. 2010','1','0','C');
				...
				$thMonthTot = (((float)$arrEmpAllow_val["YtdBasic"] + (float)$arrEmpRcDual["rcdBasic"] - (float)$arrEmpAllow_val['Ytd13NBonus'] - ((float)$arrEmpAllow_val["basicReclass"]))/12);
				
				if ($thMonthTot>30000) 
				{
					$thMonthTaxTot = $thMonthTot-30000;
					$thMonthTot = 30000;
				}
				...
				//$thMonthTotAdvances = $arrYtdData["YTd13NAdvance"];
					if ((float)$arrEmpAllow_val['sprtAdvance']>0 || (float)$arrEmpRcDual['rcdAdvances']>0) 
				{
					$thMonthTotAdvances = (((float)$arrEmpAllow_val['sprtAdvance'] + (float)$arrEmpRcDual['rcdAdvances'] - (float)$arrEmpAllow_val['YTd13NAdvance'] + (float)$arrEmpAllow_val['allowReClass'])/12);
				}
