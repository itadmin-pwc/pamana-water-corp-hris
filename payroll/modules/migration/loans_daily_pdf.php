...
if (!empty($from) && !empty($to)) {
	$dt = "$from - $to";
	$div .= " and convert(datetime,convert(varchar(12), dateadded, 101)) between '$from' and '$to'";
} else {
	$today = date('m/d/Y');
	$dt = "$today";
	$div .= " and convert(datetime,convert(varchar(12), dateadded, 101)) = '$today'";
}
...
FROM         tblEmpLoans INNER JOIN
					  ...
					  inner join tblARTransData ar on tblEmpLoans.empNo=ar.empNo and tblEmpLoans.lonRefno=ar.refNo and tblEmpLoans.lonTypeCd = ar.transType
where userID='{$_SESSION['user_id']}' and ar.status is not null

ORDER BY empLastName,empfirstname,lonSeries";
...
foreach ($arrLoanList as $val){
	...
	$TotLoanAmt += $val['lonWidInterst'];
	if ($pdf->GetY() > 250) HEADER_FOOTER($pdf, $maintEmpLoanObj, $compCode, $TOTAL_WIDTH, $dt);
	$ctr++;
}
	$pdf->Cell(126,$SPACES,"TOTAL",0,0,'L');
	$pdf->Cell(27,$SPACES,number_format($TotLoanAmt,2),0,1,'R');
#########################################################################
if ($pdf->GetY() > 250) HEADER_FOOTER($pdf, $maintEmpLoanObj, $compCode, $TOTAL_WIDTH, $dt);
$pdf->Ln(5);
...
function HEADER_FOOTER($pdf, $maintEmpLoanObj, $compCode, $TOTAL_WIDTH, $dt) {
	...
	$repTitle 		= "NEW LOANS DAILY REPORT ($dt)";
	$refNo    		= ""; 
