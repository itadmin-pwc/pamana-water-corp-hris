...
$pdf->SPACES	       	= 5;
############################ Q U E R Y ##################################
 $qryLoanList = "SELECT tblARTransData.id,tblLoanType.lonTypeShortDesc as loanType, 
 				tblARTransData.refNo,tblARTransData.amount, tblARTransData.dedAmt, tblARTransData.dedSked, tblARTransData.NoDed, tblARTransData.transDate,custNo,invoiceNo
		FROM tblARTransData INNER JOIN
                      tblLoanType ON tblARTransData.transType = tblLoanType.lonTypeCd 
		WHERE userID='{$_SESSION['user_id']}' and status is not null
ORDER BY refNo";
...
foreach ($arrLoanList as $val){
	...
	$pdf->Cell(40,$SPACES,$val['loanType'],0,0);
	$pdf->Cell(82,$SPACES,$val['refNo'],0,0);
	$pdf->Cell(62,$SPACES,$val['invoiceNo'],0,0);
...
function HEADER_FOOTER($pdf, $maintEmpLoanObj, $compCode, $TOTAL_WIDTH, $dt) {
	...
	$reppages 		= "";
	$repId    		= "Report ID: PROOFLIST";
	$repTitle 		= "PROOF LIST (".date('m/d/Y').")";
	$refNo    		= ""; 
	$dtlLabelDown   = " #     Cust. NO            Loan Type             Ref. No.                       Invoice No                    Amount";
	$dtlLabelDown2   = "";
	#########################################################################
