<?php
	...
	*/
	...
	class PDF extends FPDF
	{
		...
		function dispPaySum($empNo)
		{
			$qryEmpAllow = "SELECT     pdYear, pdNumber,grossEarnings, taxableEarnings, minwage_taxableEarnings, totDeductions, sprtAllow, netSalary, taxWitheld, sprtAllowAdvance, empBasic, empEcola
							FROM         tblPayrollSummaryHist where compCode='".$_SESSION["company_code"]."' and pdYear='2010' and empNo='".$empNo."' order by pdYear, pdNumber";;
			$resEmpAllow = $this->execQry($qryEmpAllow);
			$arrEmpAllow = $this->getArrRes($resEmpAllow);
			if(count($arrEmpAllow)>=1)
			{
				$this->SetFont('Courier','B','10'); 
				$this->Cell(10,5,'','','','L');
				$this->Cell(320,5,'PAYROLL SUMMARY FOR 2010','','1','L');
				$this->Cell(10,5,'','','','L');
		...
		function dispPaySum_2011($empNo)
		{
			$qryEmpAllow = "SELECT     pdYear, pdNumber,grossEarnings, taxableEarnings, minwage_taxableEarnings, totDeductions, sprtAllow, netSalary, taxWitheld, sprtAllowAdvance, empBasic, empEcola
							FROM         tblPayrollSummaryHist where compCode='".$_SESSION["company_code"]."' and pdYear='2011' and empNo='".$empNo."' order by pdYear, pdNumber";;
			$resEmpAllow = $this->execQry($qryEmpAllow);
			$arrEmpAllow = $this->getArrRes($resEmpAllow);
			if(count($arrEmpAllow)>=1)
			{
				$this->SetFont('Courier','B','10'); 
				$this->Cell(10,5,'','','','L');
				$this->Cell(320,5,'PAYROLL SUMMARY FOR 2011','','1','L');
				$this->Cell(10,5,'','','','L');
				$this->Cell(20,5,'Year','1','','L');
				$this->Cell(20,5,'PdNumber','1','','L');
				$this->Cell(25.99,5,'Gross Earn.','1','','R');
			...
			foreach($arrdispBranch as $arrdispBranch_val)
			{
				...
				foreach($arrEmpList as $arrEmpList_val)
				{
					...
					if($arrEmpList_val["empBrnCode"]==$arrdispBranch_val["brnCode"])
					{
						...
						//Payroll Summary 2010
						$this->dispPaySum($arrEmpList_val["empNo"]);
						
						//Payroll Summary 2011
						$this->dispPaySum_2011($arrEmpList_val["empNo"]);
	...
	$qryListEmp = "Select * from tblEmpMast where empPayGrp<>0 and  compCode='".$_SESSION["company_code"]."' ".($_GET["empBrnCode"]==0?"":"and empBrnCode='".$_GET["empBrnCode"]."'")."
