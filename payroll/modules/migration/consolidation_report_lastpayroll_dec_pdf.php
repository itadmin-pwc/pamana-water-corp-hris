<?php
	...
	*/
	...
	class PDF extends FPDF
	{
		...
		function dispBranch()
		{
			$qryBranch = "Select brnCode, brnDesc from tblBranch
							...
							and ytdHist.empNo in (select empNo from tblPayrollSummary)
							) 
		...
		function getYtdData($empNo)
		{
			$qryYtdData = "Select * from tblYtdData where compCode='".$_SESSION["company_code"]."' and empNo='".$empNo."'";
			$resYtdData = $this->execQry($qryYtdData);
			return $this->getSqlAssoc($resYtdData);
			...
			foreach($arrdispBranch as $arrdispBranch_val)
			{
				...
				{	
					if($arrEmpList_val["empBrnCode"]==$arrdispBranch_val["brnCode"])
					{
						...
						$arrYtdData = $this->getYtdData($arrEmpList_val["empNo"]);
						
						$taxableAmt = ($arrEmpList_val["YtdTaxable"]  + $arrPrevEmplr["prevEarnings"] + $arrYtdData["YtdTaxable"]) - ($arrEmpList_val["YtdGovDed"] +  $arrYtdData["YtdGovDed"]);
						...
						$empOvrUnder = $taxDue - $arrEmpList_val["YtdTax"] - $arrYtdData["YtdTax"] - $arrPrevEmplr["prevTaxes"];
						
						/*if($arrEmpList_val["empWageTag"]=='N')
						{
							if(number_format($empOvrUnder,2)!=0)
							{*/
									$this->SetFont('Arial','B','10'); 
									$this->Cell(47,6,$empCount.". ".$arrEmpList_val["empNo"]."=".$arrEmpList_val["empLastName"].", ".$arrEmpList_val["empFirstName"]." ".$arrEmpList_val["empMidName"][0].". : Date Hired : ".date("m/d/Y", strtotime($arrEmpList_val["dateHired"]))." : Date Resigned : ".date("m/d/Y", strtotime($arrEmpList_val["dateResigned"]))." : Min. Wage Tag : ".$arrEmpList_val["empWageTag"]. ": Emp. Pay Type : ".$arrEmpList_val["empPayType"]." : Group : ".$arrEmpList_val["empPayGrp"],'','1','L');
							
							
									$this->SetFont('Arial','B','8'); 
									$this->Cell(10,5,'','','','L');
									$this->Cell(53.2,5,'CURRENT','1','','C');
									$this->Cell(26.6,5,'PREVIOUS','1','1','C');
									
									$this->Cell(10,5,'','','','L');
									$this->Cell(26.6,5,'Earnings','1','','C');
									$this->Cell(26.6,5,'Earnings (Dec)','1','','C');
									$this->Cell(26.6,5,'Earnings','1','','C');
									//$this->Cell(26.6,5,'13TH Month','1','','C');
									$this->Cell(26.6,5,'Mandatory','1','','C');
									$this->Cell(26.6,5,'Mandatory (Dec)','1','','C');
									$this->Cell(26.6,5,'Exemption','1','','C');
									$this->Cell(26.6,5,'Taxable Amount','1','','C');
									$this->Cell(26.6,5,'Tax Due','1','','C');
									$this->Cell(26.6,5,'Tax','1','','C');
									$this->Cell(26.6,5,'Tax (Dec)','1','','C');
									
									$this->Cell(26.6,5,'Over (+) / Under(-)','1','1','C');
									//$this->Cell(26.6,5,'Ave. Tax / Payroll','1','1','C');
									
									$this->Cell(10,5,'','','','L');
									$this->Cell(26.6,5,number_format($arrEmpList_val["YtdTaxable"],2),'1','','R');
									$this->Cell(26.6,5,number_format($arrYtdData["YtdTaxable"],2),'1','','R');
									$this->Cell(26.6,5,number_format(($arrPrevEmplr["prevEarnings"]+$arrPrevEmplr["tax13th"]), 2),'1','','R');
									//$this->Cell(26.6,5,number_format($arrPrevEmplr["tax13th"], 2),'1','','R');
									$this->Cell(26.6,5,number_format($arrEmpList_val["YtdGovDed"],2),'1','','R');
									$this->Cell(26.6,5,number_format($arrYtdData["YtdGovDed"], 2),'1','','R');
									...
									$this->Cell(26.6,5,number_format(($arrEmpList_val["YtdTax"]+$arrPrevEmplr["prevTaxes"]),2),'1','','R');
									$this->Cell(26.6,5,number_format($arrYtdData["YtdTax"],2),'1','','R');
	...
	$qryListEmp = "Select ytdHist.compCode, pdYear, ytdHist.empNo, empLastName, empFirstName, empMidName, empBrnCode, empTin, empTeu, teuAmt, empWageTag, empPayType, dateHired, dateResigned,YtdGross, YtdTaxable, YtdGovDed, YtdTax,Ytd13NBonus,
					...
					and empmast.empTeu = tblTeu.teuCode and ytdHist.empNo in (select empNo from tblPayrollSummary)
