...
if(isset($_POST['btnUpload'])) {
	...
	$curdate=date('mdY');
	if ($error == UPLOAD_ERR_OK && $_FILES["fileUpload"]["type"] == "text/plain") {
		$tmp_name = $_FILES["fileUpload"]["tmp_name"];
		$name = $_FILES["fileUpload"]["name"];
		$size = $_FILES["fileUpload"]["size"];				
		move_uploaded_file($tmp_name, "ls-$curdate.txt");
		if ($cspObj->readLSTxtfile("ls-$curdate.txt")) {
			//$cspObj->AddAudit('LSUploadTag');
			$proc = 1;
		} else {
			$proc = 0;
		}
...
if ($proc == 1) {
?>
<script>alert("Records  successfully migrated!");</script>
<? }
elseif ($proc == 0){?>
