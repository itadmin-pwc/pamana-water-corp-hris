...
$inqTSObj->groupType     = $_GET['groupType'];
$inqTSObj->orderBy       = $_GET['orderBy'];
$inqTSObj->catType       = $_GET['catType'];
$empNo         = $_GET['empNo'];
...
foreach ($arrEmpList as $empListVal){
	$arrTotal = $inqTSObj->getEarningsTotal($sessionVars['compCode'],$empListVal['empNo'],$arrPayPd['pdYear'],$arrPayPd['pdNumber']);
	...
	if ($_GET['orderBy'] < 3) {
		...
		if ($pdf->GetY() > 240) { HEADER_FOOTER($pdf, $inqTSObj, $compCode, $orderBy,$arrPayPd); $TOTAL_PAGE++; }
	} else {
	...
	foreach ($arrTSList as $TSVal){
		$total_ = $total_ + $TSVal['trnAmountE'];
		$pdf->Cell(95,3,"",0,0);
		$pdf->Cell(50,3,$inqTSObj->getTransTypeDescArt($sessionVars['compCode'],$TSVal['trnCode']),0,0);
		$pdf->Cell(35,3,$TSVal['trnAmountE'],0,0,'R');
		 if(trim($TSVal['trnTaxCd']) == 'Y'){
			$trnTaxCd =  "YES";
		 }
		 else if(trim($TSVal['trnTaxCd']) == 'N'){
			$trnTaxCd = "NO";
		 }
		 else{
			$trnTaxCd = "---";
		 }
		$pdf->Cell(17,3,$trnTaxCd,0,1,'C');
	}
	########## TOTAL PER EMPLOYEE
...
function HEADER_FOOTER($pdf, $inqTSObj, $compCode, $orderBy,$arrPayPd) {
	...
	$pdf->reppages 		= "";
	$pdf->repId    		= "Report ID: EARNGS001";
	$pdf->repTitle 		= "(".($_GET['reportType']!="tblEarnings"?"Posted":"Pre Posted").") Current Employee Earnings (P. Payable:".$inqTSObj->valDateArt($arrPayPd['pdPayable']).")";
	$pdf->refNo    		= "";
	$pdf->dtlLabelUp    = "                 ";
	$pdf->dtlLabelDown  = "EMPLOYEE                                     TRANSACTION                         AMT   TAX";	
	$pdf->Header();
	########################### F O O T E R  ################################
