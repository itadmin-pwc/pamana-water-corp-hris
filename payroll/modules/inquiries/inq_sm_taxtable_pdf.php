...
$pdf->SPACES	       	= 5;
############################ Q U E R Y ##################################
$query = 	"SELECT tblSmTax.*,teuDesc from tblSmTax inner join tblTeu on tblSmTax.teuCode=tblTeu.teuCode";
//echo $query;
...
foreach ($row as $sss_list)
{
	...
	$pdf->Cell(55,$SPACES,$sss_list['teuDesc'],0,0,'L');
	$pdf->Cell(53,$SPACES,$sss_list['txUpLimit'],0,0,'R');
	$pdf->Cell(62,$SPACES,$sss_list['txFixdAmt'],0,0,'R');
	$pdf->Cell(62,$SPACES,$sss_list['txAddPcent'],0,1,'R');
...
function HEADER_FOOTER($pdf, $inqTaxObj, $compCode) {
	...
	$pdf->reppages 		= "";
	$pdf->repId    		= "Report ID: SMTAXTBL001";
	$pdf->repTitle 		= "SM Tax Table";
	$pdf->refNo    		= "";
