...
switch ($inputId) {
	case "empSearch":		
		...
		if ($thisValue=="searchTS") { ### TIMESHEET TIMESHEET TIMESHEET TIMESHEET TIMESHEET TIMESHEET TIMESHEET TIMESHEET
			if ($numEmp>0) {
				$arrTS = $inqTSObj->getTimeSheet($compCode,$groupType,$catType,'',$arrPayPd['pdFrmDate'],$arrPayPd['pdToDate']);
				if (count($arrTS) > 0) {
					echo "document.frmTS.action = 'timesheet_list.php?inputId=$optionId&empNo=$empNo&empName=$empName&empDiv=$empDiv&empDept=$empDept&empSect=$empSect&groupType=$groupType&orderBy=$orderBy&catType=$catType&payPd=$payPd';";
				} else {
		...
		if ($thisValue=="searchTS2") { ### EARNINGS EARNINGS EARNINGS EARNINGS EARNINGS EARNINGS EARNINGS EARNINGS EARNINGS
			if ($numEmp>0) {
				$arrEarnings = $inqTSObj->getEarnings($compCode,'',$payPd,$arrPayPd['pdNumber']);
				if (count($arrEarnings) > 0) {
					echo "document.frmTS.action = 'earnings_list.php?inputId=$optionId&empNo=$empNo&empName=$empName&empDiv=$empDiv&empDept=$empDept&empSect=$empSect&orderBy=$orderBy&payPd=$payPd&reportType=$reportType';";
				} else {
					echo "document.getElementById('empNo').disabled=false;";
					echo "document.getElementById('empNo').value='';";
		...
		if ($thisValue=="searchTS3") { ### DEDUCTIONS DEDUCTIONS DEDUCTIONS DEDUCTIONS DEDUCTIONS 
			if ($numEmp>0) {
				$arrDeductions = $inqTSObj->getDuductions($compCode,'',$payPd,$arrPayPd['pdNumber']);
				if (count($arrDeductions) > 0) {
					echo "document.frmTS.action = 'deductions_list.php?inputId=$optionId&empNo=$empNo&empName=$empName&empDiv=$empDiv&empDept=$empDept&empSect=$empSect&orderBy=$orderBy&payPd=$payPd&reportType=$reportType';";
				} else {
					echo "document.getElementById('empNo').disabled=false;";
					echo "document.getElementById('empNo').value='';";
					echo "alert('No Earnings record found...');";
	...
	case "TerminatedLoan";
		...
		if (count($inqTSObj->getTerminatedLoans($div,$_GET['from'],$_GET['to'],$loanTypeFilter)) ==0) {
			echo "alert('No Employee record found...');";
		} else {
			$from =$_GET['from'];
			$to = $_GET['to'];
			echo "location.href='terminated_loans_list.php?empDiv=$empDiv&empDept=$empDept&empSect=$empSect&from=$from&to=$to&loanType={$_GET['loanType']}'";
		}
	break;
