...
foreach ($arrLoanList as $loanListVal){
	...
	if ($_GET['orderBy']==1) {
		if ($tempCode!=$loanListVal['empNo']) {
			...
			$pdf->Cell(30,$SPACES,$div['deptShortDesc'],0,0);
			$pdf->SetFont('Courier', '', '10');
		} else {
		...
		$pdf->Cell(40,$SPACES,$loanListVal['lonRefNo'],0,0);
		$pdf->Cell(30,$SPACES,$loanListVal['lonWidInterst'],0,0,'R');
		$pdf->Cell(30,$SPACES,$loanListVal['lonDedAmt2'],0,0,'R');
		$pdf->Cell(30,$SPACES,$loanListVal['lonPayments'],0,0,'R');
		$pdf->Cell(25,$SPACES,$periodDed."  ",0,0,'R');
		...
		if ($loanTotal['refMax'] > "") { 
			...
			if ($splitDesc[2]==$loanListVal['lonTypeCd'] && $splitDesc[1]==$loanListVal['lonRefNo']) {
				...
				$pdf->Cell(40,$SPACES,"",0,0); /////"Total for this Loan Type: "
				$pdf->Cell(35,$SPACES,"TOTAL: ".$loanTotal['totRec']." record/s",0,0);
				$pdf->Cell(15,$SPACES,"",0,0);
				$pdf->Cell(20,$SPACES,$loanTotal['totAmt'],0,0,'R');
				$pdf->Cell(40,$SPACES,"",0,0,'R');
				$pdf->Cell(20,$SPACES,$loanTotal['totPaymnts'],0,0,'R');
				$pdf->Cell(25,$SPACES,"",0,0,'C');
				$pdf->Cell(25,$SPACES,$loanTotal['totCurbal'],0,1,'R');
				$pdf->SetFont('Courier', '', '10');
				$pdf->Line(11,$pdf->GetY(),$TOTAL_WIDTH+6,$pdf->GetY());  /////(X1,Y1,X2,Y2)			  ####### LINE LINE LINE
				$GTotRec += $loanTotal['totRec'];
				$GTotAmt += $loanTotal['totAmt'];
				$GTotPmt += $loanTotal['totPaymnts'];
				$GTotCurBal += $loanTotal['totCurbal'];
		...
		$pdf->Cell(15,$SPACES,$loanListVal['lonRefNo'],0,0);
		$pdf->Cell(20,$SPACES,$loanListVal['lonWidInterst'],0,0,'R');
		$pdf->Cell(20,$SPACES,$loanListVal['lonDedAmt2'],0,0,'R');
		$pdf->Cell(25,$SPACES,$loanListVal['lonPayments'],0,0,'R');
		$pdf->Cell(15,$SPACES,$periodDed,0,0,'C');
		$pdf->Cell(20,$SPACES,$loanListVal['lonCurbal'],0,0,'R');
		$pdf->Cell(21,$SPACES,$loanListVal['empNo'],0,0);
		$pdf->Cell(40,$SPACES,$loanListVal['empLastName']." ".$nameInit,0,0);
		...
		if ($loanTotal['refMax'] > "") { 
			...
			if ($splitDesc[2]==$loanListVal['lonTypeCd'] && $splitDesc[1]==$loanListVal['lonRefNo']) {
				...
				$pdf->Cell(15,$SPACES,"",0,0);
				$pdf->Cell(20,$SPACES,$loanTotal['totAmt'],0,0,'R');
				$pdf->Cell(20,$SPACES,"",0,0,'R');
				$pdf->Cell(20,$SPACES,$loanTotal['totPaymnts'],0,0,'R');
				$pdf->Cell(15,$SPACES,"",0,0,'C');
				$pdf->Cell(20,$SPACES,$loanTotal['totCurbal'],0,0,'R');
				$pdf->Cell(21,$SPACES,"",0,0);
...
$pdf->Cell(21,$SPACES,"",0,0);
$pdf->Cell(40,$SPACES,"",0,0); /////"Total for this Loan Type: "
$pdf->Cell(55,$SPACES,"  GRAND TOTAL: ".$GTotRec." record/s",0,0);
$pdf->Cell(15,$SPACES,"",0,0);
$pdf->Cell(20,$SPACES,number_format($GTotAmt,2),0,0,'R');
$pdf->Cell(40,$SPACES,"",0,0,'R');
$pdf->Cell(20,$SPACES,number_format($GTotPmt,2),0,0,'R');
$pdf->Cell(25,$SPACES,"",0,0,'C');
$pdf->Cell(25,$SPACES,number_format($GTotCurBal,2),0,1,'R');
$pdf->SetFont('Courier', '', '10');
$pdf->Line(11,$pdf->GetY(),$TOTAL_WIDTH+6,$pdf->GetY());  /////(X1,Y1,X2,Y2)			  ####### LINE LINE LINE
$pdf->Ln(5);
...
function HEADER_FOOTER($pdf, $maintEmpLoanObj, $compCode, $orderBy) {
	...
	$pdf->refNo    		= "";
	if ($_GET['orderBy']==1) {
		$pdf->dtlLabelUp    = " Emp.No.  Emp. Name      Department    RefNo                   Total Amt      Ded Amt         Total      Sked      Cur Bal";
		$pdf->dtlLabelDown  = "                                                                                             Paymnts";
	} else {
		$pdf->dtlLabelUp    = " Loan Type       RefNo    Total      Ded     Total   Sked   Current   Emp.No.   Employee Name      Department                ";
		$pdf->dtlLabelDown  = "                           Amt       Amt    Paymnts  Ded    Balance";
	}
	$pdf->Header();
	########################### F O O T E R  ################################
