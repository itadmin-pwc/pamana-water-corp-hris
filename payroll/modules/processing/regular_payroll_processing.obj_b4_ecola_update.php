...
/*									if ($allowArrVal['empNo'] == '002600001') {
	...
	private function getNewEmployee()
	{
		...
		$resAddNewEmp = $this->execQry($qryAddNewEmp);
		return $this->getArrRes($resAddNewEmp);
	}
	...
	private function getEmployeeCntAbsent($table,$empNo,$dateFrom, $dateTo)
	{
		...
		if($table=="tblTimeSheetHist")
		{
			...
			foreach($arrNewEmp as $arrNewEmp_val)
			{
				...
				{
					if($this->getRecCount($resqryTimeSheet)==0)
						$emp_cntAbsent = 12;
				}
			}
		}
...
$qryUpdateEmpLoans = "UPDATE tblEmpLoansDtl SET dedTag = ''
		...
		if ($_GET["pdNum"] == 24) {
			...
			if($TrnsA){
				$TrnsA = $this->execQry($sqlDeleteTaxForAdj);	
			}			
		}
...
/*			$arrEmpNonTaxAllowTot = $this->getEmpAllowance('tblAllowanceBrkDwn',$empForDedVal['empNo'],"AND (tblPayTransType.trnTaxCd = 'N' or tblPayTransType.trnTaxCd='' or tblPayTransType.trnTaxCd is null) AND (tblAllowanceBrkDwn.sprtPS IS NULL or tblAllowanceBrkDwn.sprtPS ='' or tblAllowanceBrkDwn.sprtPS='N') ");
	...
	function getSSSEmplrAmt($amt) {
		$amt = abs($amt);
		$qrySSSEmplr = "SELECT TOP 1 sssEmployer,EC FROM tblSssPhic WHERE (sssEmployee = '$amt')";
		return $this->getSqlAssoc($this->execQry($qrySSSEmplr));
	}
