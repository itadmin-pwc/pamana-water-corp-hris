...
class closeRegPayroll extends commonObj {
	...
	function getARDeductions() {
		$sqlDeductedAR = "SELECT tblEmpLoans.lonRefNo, REPLACE(CONVERT(VARCHAR(11), GETDATE(), 6), ' ', '-') AS Date, 'PAY' AS PType, tblBranch.brnShortName AS brnCode,  tblCustomerNo.custNo AS mmsNo, tblBranch.brnShortName AS site, 1 AS term, '01' AS b_line, name = CASE tblEmpLoans.LonTypeCd WHEN '39' THEN 'CSI' WHEN '35' THEN 'AR OTHERS' WHEN '303' THEN 'MERALCO' END, REPLACE(CONVERT(VARCHAR(11),  GETDATE(), 6), ' ', '-') AS GLDate, '' AS ref, tblEmpLoans.lonRefNo AS line_desc, 1 AS qty, tblEmpLoansDtlhist.ActualAmt * - 1 AS ActualAmt,'' AS vatCode, 'PHP' AS Currency,InvoiceApp =  CASE tblEmpLoans.lonTypeCd WHEN 310 THEN tblEmpLoans.InVoiceNo ELSE 0 END , filename = CASE tblEmpLoans.compCode WHEN 2 THEN 'PG' + REPLACE(CONVERT(VARCHAR(10), GETDATE(), 1), '/', '') + '_' + REPLACE(CONVERT(VARCHAR(12), GETDATE(), 108), ':', '') + '.J01' WHEN 1 THEN  'PJ' + REPLACE(CONVERT(VARCHAR(10), GETDATE(), 1), '/', '') + '_' + REPLACE(CONVERT(VARCHAR(12), GETDATE(), 108), ':', '') + '.J01' WHEN 4 THEN  'DC' + REPLACE(CONVERT(VARCHAR(10), GETDATE(), 1), '/', '') + '_' + REPLACE(CONVERT(VARCHAR(12), GETDATE(),108), ':', '') + '.J01' WHEN 5 THEN  'DS' + REPLACE(CONVERT(VARCHAR(10), GETDATE(), 1), '/', '') + '_' + REPLACE(CONVERT(VARCHAR(12), GETDATE(),108), ':', '') + '.J01' END FROM tblEmpLoans INNER JOIN tblEmpMast ON tblEmpLoans.compCode = tblEmpMast.compCode AND tblEmpLoans.empNo = tblEmpMast.empNo INNER JOIN tblBranch ON tblEmpMast.empBrnCode = tblBranch.brnCode AND tblEmpMast.compCode = tblBranch.compCode INNER JOIN tblEmpLoansDtlhist ON tblEmpLoans.compCode = tblEmpLoansDtlhist.compCode AND tblEmpLoans.empNo = tblEmpLoansDtlhist.empNo AND tblEmpLoans.lonTypeCd = tblEmpLoansDtlhist.lonTypeCd AND tblEmpLoans.lonRefNo = tblEmpLoansDtlhist.lonRefNo INNER JOIN tblCustomerNo ON tblEmpLoans.empNo = tblCustomerNo.empNo AND tblEmpMast.empNo = tblCustomerNo.empNo AND tblEmpMast.compCode = tblCustomerNo.compCode WHERE (tblEmpLoans.lonTypeCd Like '3%') AND (tblEmpLoansDtlhist.pdNumber = '".$this->get['pdNum']."') AND (tblEmpLoansDtlhist.pdYear = '".$this->get['pdYear']."') AND tblEmpLoansDtlhist.trnGrp='{$_SESSION['pay_group']}' AND tblEmpLoansDtlhist.trnCat='{$_SESSION['pay_category']}'";	
	...
	function CreateTxtFile() {
		...
		switch($_SESSION['company_code']) {
			case 1:
				$filename = "PJ".date('dmy')."_".date('His').".J01";
			break;	
			case 2:
				$filename = "PG".date('dmy')."_".date('His').".J01";
			break;	
			case 4:
				$filename = "DC".date('dmy')."_".date('His').".J01";
			break;	
			case 5:
				$filename = "DS".date('dmy')."_".date('His').".J01";
			break;	
		}
		$fname = $filename;
		$filename = "../../../../Oracle_TextFiles/$filename";
