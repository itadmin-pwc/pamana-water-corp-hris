						...
						<TABLE align="center" cellpadding="0" cellspacing="2" border="0" class="childGrid">
							...
							<tr>
								...
								<td class="gridDtlVal2">
										...
										echo date('Y-m-d',strtotime($arrPayPeriod['pdFrmDate'])). " " .date('Y-m-d',strtotime($arrPayPeriod['pdToDate']));
...
<SCRIPT>
	...
	function procRegPayroll(act){
		...
		var dteCvrd = $('DteCvrd').innerHTML.split(' ');
	...
	params = "?action="+act+"&pdNum="+payPd[0]+"&pdYear="+payPd[1].replace(' ','')+"&dtFrm="+dteCvrd[0]+"&dtTo="+dteCvrd[1]+"&pdMonth="+payPd[0];
