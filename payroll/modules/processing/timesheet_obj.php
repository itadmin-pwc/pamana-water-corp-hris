...
class inqTSObj extends commonObj 
{
	...
	function getAllPeriod($compCode,$groupType,$catType,$modulo) { // $module = 0 = 1st period, $modulo = 1 = 2nd period, $modulo = "" = both
		...
		$qry = "SELECT   concat(date_format(pdPayable,'%M')  , ' ' , date_format(pdPayable,'%Y')) AS perMonth, 
				concat(MIN(pdNumber),'-',MAX(pdNumber),'-',MAX(pdYear),'-',MAX(pdYear),'-',MAX(date_format(pdToDate,'%m')),'-',MAX(date_format(pdToDate,'%m'))) AS pdNumber
				FROM tblPayPeriod
				WHERE compCode = '".$_SESSION["company_code"]."' AND payGrp = '".$_SESSION["pay_group"]."' AND payCat = '".$_SESSION["pay_category"]."' and pdNumber<>24
...
*/				}
	...
	function Loans($compCode, $pdYear, $pdMonth,$lonCode,$location) {
			 ...
			 $sqlHDMFLoan = "SELECT tblEmpMast.empNo,tblEmpMast.empLastName, tblEmpMast.empFirstName, tblEmpMast.empMidName, sum(tblEmpLoansDtlHist.ActualAmt) AS Amount, tblEmpLoans.lonAmt,  tblEmpLoans.lonWidInterst,
                      tblEmpMast.empSssNo, empStat = CASE empStat WHEN 'TR' THEN 'T' ELSE ' ' END,lonStart,lonGranted, empBday, empPagibig, tblEmpLoansDtlHist.lonRefNo,  tblEmpLoans.lonStat,tblEmpLoans.lonTypeCd 
                      ...
                      tblEmpLoansDtlHist.lonTypeCd = tblEmpLoans.lonTypeCd AND tblEmpLoansDtlHist.lonRefNo = tblEmpLoans.lonRefNo
						...
						$filter
						AND pdNumber IN (Select pdNumber from tblPayPeriod where compCode='$compCode' AND Month(pdFrmDate) = '$pdMonth' AND pdYear='$pdYear') AND tblEmpLoans.lonRefNo Not IN (Select lonrefNo from tbllonrefno)
	group by tblEmpMast.empNo,tblEmpMast.empLastName, tblEmpMast.empFirstName, tblEmpMast.empMidName,tblEmpLoans.lonAmt,  tblEmpLoans.lonWidInterst,
                      tblEmpMast.empSssNo, empStat,lonStart,lonGranted, empBday, empPagibig, tblEmpLoansDtlHist.lonRefNo,  tblEmpLoans.lonStat,tblEmpLoans.lonTypeCd 
	...
	function loanAdjustment($pdYear,$pdMonth,$location, $trnCode=NULL)
	{
			...
			$sqlLoanAdj = "SELECT     tblDeductionsHist.empNo, tblEmpMast.empLastName, tblEmpMast.empFirstName, tblEmpMast.empMidName, tblLoanType.lonTypeShortDesc, 
							...
							AND pdNumber IN (Select pdNumber from tblPayPeriod where compCode='".$_SESSION["company_code"]."' AND Month(pdFrmDate) = '$pdMonth' AND pdYear='$pdYear') AND tblEmpLoans.lonRefNo Not IN (
	...
	function LoanQuartPayments($compCode, $pdYear, $pdMonth,$lonCode) {
			 $sqlLoanPymnts = "Select * from tblEmpLoansDtlHist 
							...
							AND dedtag IN ('Y','P')
							$filter
							AND pdNumber IN (Select pdNumber from tblPayPeriod where compCode='$compCode' AND Month(pdToDate) IN  ($pdMonth) AND pdYear='$pdYear') order by pdNumber
							";
