...
class inqTSObj extends commonObj 
{
	...
	function getAllPeriod($compCode,$groupType,$catType,$modulo) { // $module = 0 = 1st period, $modulo = 1 = 2nd period, $modulo = "" = both
		...
		$qry = "SELECT   concat(date_format(pdPayable,'%M')  , ' ' , date_format(pdPayable,'%Y')) AS perMonth, 
				concat(MIN(pdNumber),'-',MAX(pdNumber),'-',MAX(pdYear),'-',MAX(pdYear),'-',MAX(date_format(pdPayable,'%m')),'-',MAX(date_format(pdPayable,'%m'))) AS pdNumber
				FROM tblPayPeriod
				WHERE compCode = '".$_SESSION["company_code"]."' AND payGrp = '".$_SESSION["pay_group"]."' AND payCat = '".$_SESSION["pay_category"]."' 
				GROUP BY  concat(date_format(pdPayable,'%m') , ' ' ,date_format(pdPayable,'%Y'))
	...
	function RemTextfile($compCode, $pdYear, $pdMonth, $location)
	{
		
		if (!in_array($_SESSION['company_code'], array(1,2,7,8,9,10,11,12)))
			$qrymtdGovtHist = "exec sp_RemittanceGovt $pdYear,$pdMonth,$compCode $location \n\n";
		else
			$qrymtdGovtHist = "Select * from payroll_company..view_MTDGovthist where pdYEar=$pdYear and pdMonth=$pdMonth order by empLastName, empFirstName,empMidName";
	...
	function Sss_TxtFile($compCode, $pdYear, $pdMonth,$location)
	{	
		...
		$location  = ($_SESSION['company_code'] == 4) ? ",'$location'":"";
		$arrmtdGovtHist = $this->RemTextfile($compCode, $pdYear, $pdMonth, $location);
		
		$txtOut = "00";
		$compName = substr($arrcompName["compName"], 0, 30);
		$txtOut .= $compName.$this->Space(30-strlen($compName));
		$compSssNo = substr(str_replace("-","",$compSSSNo), 0, 10).$this->Space(69);
...
*/				}
	...
	function SSSLoan_TxtFile($compCode, $pdYear, $pdMonth, $location)
	{	
		$arrcompName = $this->getCompany($compCode);
		if ($_SESSION['company_code'] == 4) {
			if ($location == '0001')
		...
		$location  = ($_SESSION['company_code'] == 4) ? "$location":"";
		$arrSSSLoan = $this->Loans($compCode,$pdYear,$pdMonth,1, $location);
		$arrSSSLoanAdj = $this->loanAdjustment($pdYear,$pdMonth,$location, '5902');
		...
		//	echo $SSSLoan_val["empLastName"]."=".$Amort."\n";
			if ($SSSLoan_val['lonTypeCd']=='12')
				$lType=1;
			else
				$lType="S";
			
			$txtOut .="10".$empSssNo.strtoupper(substr($SSSLoan_val["empLastName"],0,15)).$this->Space(15-strlen(substr($SSSLoan_val["empLastName"],0,15))).strtoupper(substr($SSSLoan_val["empFirstName"],0,15)).$this->Space(15-strlen(substr($SSSLoan_val["empFirstName"],0,15))).strtoupper($SSSLoan_val["empMidName"][0]).$this->Space(1).$lType.$lonStart."0".$LoanAmt.$this->Space(10-strlen($LoanAmt),'0').$this->Space(9-strlen($Amort),'0').$Amort.$this->Space(1).($SSSLoan_val["lonStat"]=='T'?"T":"")."\r\n";
			unset($Amort,$LoanAmt);
		}
	...
	function Loans($compCode, $pdYear, $pdMonth,$lonCode,$location) {
		if (!in_array($_SESSION['company_code'], array(1,2,7,8,9,10,11,12))) {
			if ($pdMonth == 4 && $pdYear = 2013) {
				$filter = " and tblEmpMast.empNo <> '002100035'";	
			}
			 $sqlHDMFLoan = "SELECT tblEmpMast.empNo,tblEmpMast.empLastName, tblEmpMast.empFirstName, tblEmpMast.empMidName, tblEmpLoansDtlHist.ActualAmt AS Amount, tblEmpLoans.lonAmt,  tblEmpLoans.lonWidInterst,
                      tblEmpMast.empSssNo, empStat = CASE empStat WHEN 'TR' THEN 'T' ELSE ' ' END,lonStart,lonGranted,pdNumber, empBday, empPagibig, tblEmpLoansDtlHist.lonRefNo,  tblEmpLoans.lonStat,tblEmpLoans.lonTypeCd
                      ...
                      tblEmpLoansDtlHist.lonTypeCd = tblEmpLoans.lonTypeCd AND tblEmpLoansDtlHist.lonRefNo = tblEmpLoans.lonRefNo
						...
						$filter
						AND pdNumber IN (Select pdNumber from tblPayPeriod where compCode='$compCode' AND Month(pdPayable) = '$pdMonth' AND pdYear='$pdYear') AND tblEmpLoans.lonRefNo Not IN (
	Select lonrefNo from tblLonRefNo)
						order by empLastName, empFirstName, empMidName";
		} else {
			$sqlHDMFLoan = "Select * from payroll_company..view_loanshist where lonTypeCd like '".$lonCode."%' and pdYear='$pdYear' and pdNumber IN (Select pdNumber from pg_payroll..tblPayPeriod where pdYEar='$pdYear' and Month(pdPayable)='$pdMonth')  AND lonRefNo Not IN (
	Select lonrefNo from tblLonRefNo) order by empLastName, empFirstName, empMidName";
		}
		return $this->getArrRes($this->execQry($sqlHDMFLoan));						
	}
	
	function loanAdjustment($pdYear,$pdMonth,$location, $trnCode=NULL)
	{
		if (!in_array($_SESSION['company_code'], array(1,2,7,8,9,10,11,12))) {
			if($trnCode!="")
	...
	Select lonrefNo from tblLonRefNo)
			...
			if($trnCode!="")
				$trnCode_filter = "and (trnCode IN (N'5902')) ";
			else
				$trnCode_filter = "and (trnCode IN (N'5901')) ";
				
			$sqlLoanAdj ="Select * from payroll_company..view_loansAdj where pdYEar='$pdYear' and pdNumber IN (Select pdNumber from pg_payroll..tblPayPeriod where pdYEar='$pdYear' and Month(pdPayable)='$pdMonth') and lonrefNo<>'L66775' $trnCode_filter";
		}
	...
	function LoanQuartPayments($compCode, $pdYear, $pdMonth,$lonCode) {
		if (!in_array($_SESSION['company_code'], array(1,2,7,8,9,10,11,12))) {
			if ($pdMonth == 4 && $pdYear = 2013) {
				$filter = " and tblEmpLoansDtlHist.empNo <> '002100035'";	
			}
			 ...
			 $sqlLoanPymnts = " Select * from payroll_company..view_GovLoanshist where pdYear='$pdYear' and lonTypeCd LIKE '$lonCode%'
							AND dedtag IN ('Y','P') and pdNumber IN (Select pdNumber from pg_payroll..tblPayPeriod where pdYEar='$pdYear' and Month(pdPayable)='$pdMonth')";
		}
