...
class closeRegPayroll extends commonObj {
	...
	function getNewRestDays() {
		...
		$res = $this->getSqlAssoc($this->execQry($qryNewRestDay));		
		...
		if($this->getCutOffPeriod() == 2){
			...
			$qryToMtvGovtHist_superConfi.= "INSERT INTO tblMtdGovtHist
									 ...
									 VALUES('21',
									        ...
									        '100');";
			if($Trns){
				$Trns = $this->execQry($qryToMtvGovtHist_superConfi);
			}
			*/	
...
";	
				return $this->getArrRes($this->execQry($sqlDeductedAR));		
	}
	...
	function getXtraBranches() {
		$arr = array();
		$sqlXtra = "Select brnShortName from pg_payroll..tblBranch where extraTag is not null";
		$arrXtra = $this->getArrRes($this->execQry($sqlXtra));
