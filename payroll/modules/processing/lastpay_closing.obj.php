<?
class closelastPay extends commonObj {
	...
	function closeotherdeduction() {
		 $qryotherdeductions="Insert into tblDedTranDtlHist 
					...
					trnAmount, ActualAmt, payGrp, payCat, processtag, dedStat,remarks)  
	...
	function closeloans($seqNo) {
		...
		$qrycloseloans="Insert into tblEmpLoansDtlHist 
						(compCode, empNo, lonTypeCd, lonRefNo, pdYear, pdNumber, trnCat, trnGrp, trnAmountD ,ActualAmt, dedTag, lonLastPay) 
						Select 
						 compCode, empNo, lonTypeCd, lonRefNo, pdYear, pdNumber, trnCat, trnGrp, trnAmountD, ActualAmt, dedTag, lonLastPay 
	...
	private function getAllowBrkDwm(){
		  $qryGetAllowBrkDwn = "SELECT brkDwn.compCode,brkDwn.empNo,brkDwn.allowCode,brkDwn.allowAmt,brkDwn.allowSked,brkDwn.allowTaxTag,brkDwn.allowPayTag,brkDwn.allowStart,brkDwn.allowEnd,brkDwn.allowStat,brkDwn.allowSeries,brkDwn.pdYear,brkDwn.pdNumber,
							         emp.empPayGrp,emp.empPayCat,brkDwn.sprtPS
	...
	{
		$qryAddTaxable = "
		SELECT     SUM(amountToDed) AS amountToDed, empNo, monthPeriodDate FROM tblGov_Tax_Added WHERE (empNo = '".$empNo."') and addStat='N' GROUP BY empNo, monthPeriodDate";
		$resAddTaxable = $this->execQry($qryAddTaxable);
		return  $this->getArrRes($resAddTaxable);
	}	
	######################################################END OF CLOSING EARNINGS#################################################################	
	public function mainProcCloseLastPay(){
		...
		foreach ((array)$this->getAllowBrkDwm() as $allowVal){
			$qryToAllowBrkDwnHst = "INSERT INTO tblAllowanceBrkDwnHst(compCode,empNo,allowCode,allowAmt,allowSked,allowTaxTag,allowPayTag,allowStart,allowEnd,allowStat,pdYear,pdNumber,sprtPS)
		...
		foreach ((array)$this->getYtdData() as $ytdVal){
			...
			foreach($arrGetAddTaxable as $valAddTaxable) {
				if($valAddTaxable["amountToDed"]!=0)
				{
					...
					$qryUpdateGov_Tax = "Update tblGov_Tax_Added set addStat='Y' where empNo='".$ytdVal['empNo']."'
											and monthPeriodDate='".$valAddTaxable["monthPeriodDate"]."' ";
			...
			if($this->checkYtdData($ytdVal['empNo']) > 0){
				 ...
				 $qryToYtdDataHist = "INSERT tblYtdDataHist(compCode,pdYear,empNo,YtdGross,YtdTaxable,
				                                           ...
				                                           payGrp,pdNumber,sprtAllow, YtdBasic)
		...
		foreach ((array)$this->getEmployeeRestDay() as $GetEmpRD_Val){
			if($GetEmpRD_Val["empRestDay"]!=""){
				...
				$updateTblEmpRDBckUp = "Update tblEmpMast set empRestDay=NULL where empNo='".$GetEmpRD_Val["empNo"]."' and compCode='{$_SESSION['company_code']}'";
				if($Trns){
	...
	function TransferUploadedTS() {
		$sql = "  Insert into payroll_company..t_templ_hist (
					...
					SELECT empNo, [DATE], OT_RATE_CODE, INCOME_TYPE, ALLOW_TYPE, OTHER_LOC, DAYS_ABSENT, ABSENT_AMOUNT, REG_IN, REG_OUT, REG_HRSMIN, TARDY_HRS, 
					  ...
					  ";
		$sql .= "delete from payroll_company..t_templ WHERE (compCode = '{$this->session['company_code']}')
					  AND empNo IN (
							  				SELECT empNo FROM tblLastPayEmp
                            				WHERE compCode = '{$this->session['company_code']}' 
											AND pdYear = '{$this->get['pdYear']}'
