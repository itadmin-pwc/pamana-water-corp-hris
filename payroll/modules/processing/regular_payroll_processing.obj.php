...
class regPayrollProcObj extends commonObj {
	...
	private function summarizeCorrection(){
		...
		$resSummrzeCrrctns = $this->execQryI($qrySummrzeCrrctns);
		return $this->getArrResI($resSummrzeCrrctns);
	}
	
	private function summarizeOtAndNd(){
		
		$qrySummrzeOtAndNd = "SELECT compCode, empNo, dayType, SUM(amtOtLe8) AS sumAmtOtLe8, SUM(amtOtGt8) AS sumAmtOtGt8, SUM(amtNdLe8+ifnull(amtRegNDLe8,0)) AS sumAmtNdLe8, SUM(amtNdGt8) 
		...
		$resSummrzeOtAndNd = $this->execQryI($qrySummrzeOtAndNd);
		return $this->getArrResI($resSummrzeOtAndNd);
	}
	...
	private function postAdjustmentOthers($sign){
		...
		$resPostAdjOthrs = $this->execQryI($qryPostAdjOthrs);
		return $this->getArrResI($resPostAdjOthrs);
	}
	...
	private  function getSumPositiveEarnings($empNo){
    	...
    	$qryGetPosEarn = "SELECT empNo,sum(trnAmountE) as sumPostvEarn FROM tblEarnings
						  ...
						  GROUP BY empNo";
		$resGetPosEarn = $this->execQryI($qryGetPosEarn);
		return $this->getSqlAssocI($resGetPosEarn);
	}
	...
	private function getEmpToProcPayBasic(){
		...
		$resGetEmpToProc = $this->execQryI($qryGetEmpToProc);
		return $this->getArrResI($resGetEmpToProc);
	}
	...
	private function proRateBasic($empNo,$dailyRate,$dateHired){
		...
		$resGetProrateBasic = $this->execQryI($qryGetProrateBasic);
		$cntGetProrateBasic = $this->getRecCountI($resGetProrateBasic);
	...
	private function getDailiesBasic($empNo,$dailyRate){
		...
		$resGetRegDaysToTS = $this->execQryI($qryGetRegDaysToTS);
		$rowGetRegDaysToTS = $this->getArrResI($resGetRegDaysToTS);
		...
		foreach($rowGetRegDaysToTS as $rowGetRegDaysToTS_Val)
		{
			...
			if($rowGetRegDaysToTS_Val["dayType"]=='03'){
				...
				{
					if(($rowGetRegDaysToTS_Val["hrsAbsent"]=="")||($rowGetRegDaysToTS_Val["hrsAbsent"]==0))
					{
						$emp_legDays++;
						$emp_allowDays++;
					}
			...
			if($rowGetRegDaysToTS_Val["dayType"]=='05'){
				...
				{
					if(($rowGetRegDaysToTS_Val["hrsAbsent"]!="8")||($rowGetRegDaysToTS_Val["hrsAbsent"]==0))
					{
						$emp_legHolDays++;
						$emp_legDays++;
						$emp_allowDays++;
...
/*		$qryGetLegHolToTS = "SELECT * FROM tblTimeSheet
		...
		$resGetLegHolToTS = $this->execQryI($qryGetLegHolToTS);
		$rowGetLegHolToTS = $this->getArrResI($resGetLegHolToTS);
...
*/		
		...
		if(($legalPay>0) && ($_SESSION["pay_category"]=='3')){
...
/*									if ($allowArrVal['empNo'] == '000100015') {
										echo "{$allowArrVal['allowAmt']}*$noCompDays\n";
										echo "$totalAllowAmnt = $dai_allowAmnt * (({$arr_empEarnings_curr['sumBasic']}--927.36)/{$arr_empEarnings_denom_curr['sumBasic']})\n";	
									}*/
	...
	private function getLegalOThrs($empNo) {
		$sqlLeaglOThrs = "Select sum(hrsOTLe8) as otHrs from tblTimesheet where empNo='$empNo' and daytype IN (3,5,7) and hrsOTLe8>0";	
		$res = $this->getSqlAssocI($this->execQryI($sqlLeaglOThrs));		
...
//alejo code for additional cola in legal and legal+special holiday
		$qryGetAllwTs1 = "SELECT tk.tsDate,tk.hrsAbsent,tk.hrsTardy,tk.hrsUt,tk.dayType,tkhist.otTag
FROM
tbltimesheet tk
INNER JOIN tbltk_timesheethist tkhist ON tkhist.tsDate = tk.tsDate
WHERE tk.compCode = '".$_SESSION["company_code"]."'
								AND tk.empNo = '".$empNo."'
								AND tk.empPayGrp =  '".$_SESSION["pay_group"]."'
								AND tk.empPayCat =  '".$_SESSION["pay_category"]."' and tk.daytype IN ('03','07') and tkhist.otTag ='Y'";

		$resGetAllwTs1 = $this->execQryI($qryGetAllwTs1);	
		foreach ((array)$this->getArrResI($resGetAllwTs1) as $tsResVal1){
			if($tsResVal1['hrsAbsent']>0 && in_array($trnCode, array('8106'))){
			$tranAmount+=10;
		}
			}
//end of alejo code
	...
	private function getEmpForDeduction(){
		...
		$resGetEmpForDeduct = $this->execQryI($qryGetEmpForDeduct);
		return $this->getArrResI($resGetEmpForDeduct);		
	}
	...
	private function computeEmpGrossEarnings($empNo,$and,$minWage){
		...
		$resGetEmpGrossEarn = $this->execQryI($qryGetEmpGrossEarn);
		$rowGetEmpGrossEarn = $this->getSqlAssocI($resGetEmpGrossEarn);
	...
	private function separatePaySlipAllow($empNo,$and,$minWage){
		...
		$resGetSprtPS = $this->execQryI($qryGetSprtPS);
		return $this->getArrResI($resGetSprtPS);
	}	
	
	private function getPrevGrossEarn($empNo){
		
		$getPdPartner = array_search($this->get['pdNum'],$this->getArrPdNumPartner());
		//edited by alejo 2-5-2021
		$qryGetPrevGrossEarn = "SELECT  case when empMinWageTag ='Y' then minwage_taxableEarnings ELSE taxableEarnings   END as grossEarning  FROM tblPayrollSummaryHist WHERE compCode = '{$this->session['company_code']}'
								AND pdYear = '{$this->get['pdYear']}'
		...
		// 						AND empNo = '{$empNo}'
		// 						";
		$resGetPrevGrossEarn = $this->execQryI($qryGetPrevGrossEarn);
		$rowGetPrevGrossEarn = $this->getSqlAssocI($resGetPrevGrossEarn);
		return $rowGetPrevGrossEarn['grossEarning'];
	}

	private function getEmpServiceCharge($empNo){
		$getPdPartner = array_search($this->get['pdNum'],$this->getArrPdNumPartner());
		$sqlEmpServiceCharge = "Select trnAmountE+(Select trnAmountE from tblEarningshist where empNo='$empNo' and pdNumber='$getPdPartner' and trnCode=8138) as amount from tblEarnings where empNo='$empNo' and pdNumber='{$this->get['pdNum']}' and trnCode=8138 \n";
		$rowEmpServiceCharge = $this->getSqlAssocI($this->execQryI($sqlEmpServiceCharge));
		return (float)$rowEmpServiceCharge['amount'];
	}
	private function getGovDedAmnt($monthlyGrossEarn){
		...
		$resGetGovDed = $this->execQryI($qryGetGovDed);
		return $this->getSqlAssocI($resGetGovDed);
	}
	...
	private function getSumGov($empNo){
		...
		$qryGetSumGov = "SELECT sssEmp,phicEmp,hdmfEmp FROM tblMtdGovt
						 ...
						 AND empNo = '{$empNo}'";
		$resGetSumGov = $this->execQryI($qryGetSumGov);
		return  $this->getSqlAssocI($resGetSumGov);
	...
	private function getNewEmployee()
	{
		$qryAddNewEmp = "Select empNo from tblEmpMast_new where dateReleased between '".$this->get['dtFrm']."' and '".$this->get['dtTo']."'";
		$resAddNewEmp = $this->execQryI($qryAddNewEmp);
		return $this->getArrResI($resAddNewEmp);
	}
	...
	private function getEmpAddTaxableIncome($empNo)
	{
		...
		return  $this->getSqlAssocI($resAddTaxable);
	}
	
	private function getEmpNoNonGovt()
	{
		$queryExecNonGovt = "Select empNo,cat from tblNonEmpGov where compCode='".$_SESSION["company_code"]."'";
		$resExecNonGovt = $this->execQryI($queryExecNonGovt);
		return $this->getArrResI($resExecNonGovt);
	}
	...
	private function getDataToYtdDataHist($empNo){
		$qryGetDataToYtdHist = "SELECT ytd.compCode,ytd.pdYear,ytd.empNo,ytd.YtdGross,ytd.YtdTaxable,ytd.YtdGovDed,ytd.YtdTax,ytd.YtdNonTaxAllow,ytd.Ytd13NBonus,ytd.YtdTx13NBonus,ytd.payGrp,ytd.pdNumber,
							emp.empPayGrp,emp.empPayCat,ytd.YtdBasic, ytd.sprtAllow,YtdGovDedMinWage,YtdGovDedAbvWage
	...
	{
		$qryAllowAdjOthrs = "Select  earnTranDtl.empNo, earnTranDtl.refNo, earnTranDtl.trnCode, SUM(earnTranDtl.trnAmount) AS sumEarnAmnt, earnTranDtl.trnTaxCd, allowType.sprtPS
							  ...
							  WHERE  (earnTranDtl.compCode = '{$this->session['company_code']}') AND 
								...
								GROUP BY earnTranDtl.empNo, earnTranDtl.trnCode, earnTranDtl.refNo, earnTranDtl.trnTaxCd, allowType.sprtPS";
		
		$resqryAllowAdjOthrs = $this->execQryI($qryAllowAdjOthrs);
		return $this->getArrResI($resqryAllowAdjOthrs);
	 }
	...
	{
		$emp_cntAbsent = 0;
		$qryTimeSheet = "Select * from ".$table." where compCode='".$_SESSION["company_code"]."' and 
						 empNo='".$empNo."' and tsDate between '".$dateFrom."' and '".$dateTo."'
						 and empPayGrp='".$_SESSION["pay_group"]."' and empPayCat='".$_SESSION["pay_category"]."'";
						 
		$resqryTimeSheet = $this->execQryI($qryTimeSheet);
		$rowqryTimeSheet = $this->getArrResI($resqryTimeSheet);
		...
		{
			$arrNewEmp = $this->getNewEmployee();
			foreach($arrNewEmp as $arrNewEmp_val)
			{
				if($arrNewEmp_val["empNo"]==$empNo)
				{
					if($this->getRecCountI($resqryTimeSheet)==0)
						$emp_cntAbsent = 12;
				}
			}
		}
	...
	private function getEmpEarningsInfo($compCode, $pdYear, $pdNum, $empNo, $tbl_Earn, $alltrnCode, $attnBase, $cntAbsentEmp,$allowCode)
	{
		...
		$resqryEarnings = $this->execQryI($qryEarnings);
		return  $this->getSqlAssocI($resqryEarnings);
	}
	...
	{	
		...
		if($where_curr!="")
		{
			...
			$resGetAllwTs = $this->execQryI($qryGetAllwTs);	
			
			foreach ((array)$this->getArrResI($resGetAllwTs) as $tsResVal){
		...
		if($where_prev!="")
		{
			...
			$resGetAllwTs_prev = $this->execQryI($qryGetAllwTs_prev);	
			
			foreach ((array)$this->getArrResI($resGetAllwTs_prev) as $tsResVal_prev){
	...
	private function unPostedTranOthEarn()
	{
		...
		$resGetOthEarn = $this->execQryI($qryGetOthEarn);
		return $this->getArrResI($resGetOthEarn);
	}
	...
	private function writeToTblUnpostedTran($empCompCd,$empId,$trnCd,$trnAt,$ActualAmt,$pdNum,$pdYr,$trnCntNo,$refNo)
	{
		...
		return $this->execQryI($writeToTblUnpostedTran);
	}
	...
	private function getCompAnnDate($compCode)
	{
		...
		$resgetCompAnnDate = $this->execQryI($qrygetCompAnnDate);
		
		return  $this->getSqlAssocI($resgetCompAnnDate);
	}
	...
	private function getEmpYtdData($empNo)
	{
		...
		$qrygetEmpYtdData = $this->execQryI($qrygetEmpYtdData);
		
		return $this->getSqlAssocI($qrygetEmpYtdData);
	}
	...
	private function TaxAdjsutment() {
		...
		return $this->getArrResI($this->execQryI($sqlEmptax));
	}	
	...
	private function computeWithTax($empNo,$gross_Taxable,$empTeu,$sumGov,$minBasicPay)
	{
        ...
        else
        {
			...
			if($this->get['taxType'] == 1) {
				...
				$ytd_basic = $arrSumYtdDataHis['YtdTaxable'];
				//echo $empNo." YTD Basic ==".$ytd_basic."\n" . "<br>";
				$total_basic = $ytd_basic + $stackedSalary;
	...
	function getTimeSheetAdjusments($empEn)
	{
		...
		if($empEn==1)
			return $this->getArrResI($this->execQryI($qryTSAdjust));
		else
			return $this->getSqlAssocI($this->execQryI($qryTSAdjust));
	}
...
*/	
	...
	function getotherdeductionlist($empNo,$cat){
		...
		return $this->getArrResI($this->execQryI($qryother));
	}
	...
	function updateotherdeductions($seqNo,$Amount,$tag,$dedtoAdv) {
		...
		return $this->execQryI($qryupdateotherdeductions);
	}
	...
	function updateloansdtl($empNo,$lonCd,$Amount,$lonRefNo,$tag,$dedAdv="") {
		...
		return $this->execQryI($qryupdateloansdtl);
	}
	//end of will program		
	...
	function reProcRegPayroll(){
		...
		if($TrnsA){
			$TrnsA = $this->execQryI($qryDeleAllowBrkDwn);
		}	
		...
		if($TrnsA){
			$TrnsA = $this->execQryI($qryDeleMtdGovt);
		}
...
$qryUpdateEmpLoans = "UPDATE tblEmpLoansDtl SET dedTag = ''
							  WHERE (tblEmpLoansDtl.compCode = '" . $this->session['company_code'] . "') 
									...
									";	
		if($TrnsA){
			$TrnsA = $this->execQryI($qryUpdateEmpLoans);	
		}	
		...
		$qryUpdtDedTran = "UPDATE tblDedTranDtl SET processTag = NULL
						...
						";
		if($TrnsA){
			$TrnsA = $this->execQryI($qryUpdtDedTran);	
		}
		...
		if($TrnsA){
			$TrnsA = $this->execQryI($qryDeleUnpostedTran);	
		}	
		...
		if($TrnsA){
			$TrnsA = $this->execMultiQryI($qryDelEarnTranDtl_Ts);	
		}
		
		if ($_GET["pdNum"] == 24) {
			$sqlDeleteTaxForAdj	="Delete from tblTaxAdj where payGrp ='{$_SESSION['pay_group']}' and payCat= '{$_SESSION['pay_category']}' and pdYear='{$_GET['pdYear']} '";
			if($TrnsA){
				$TrnsA = $this->execQryI($sqlDeleteTaxForAdj);	
			}			
		}
	...
	public function mainProcRegPayroll(){
		...
		$earnTrantrnAmount = 0;
		$Trns = $this->reProcRegPayroll();
		//$Trns = $this->beginTranI();//begin regular payroll transaction
		$Trns = true;
		...
		foreach ((array)$this->postAdjustmentOthers(">") as $arrPostAdjOthrsValPos){//foreach for post adjustment and others positive sign
			if($Trns){
				...
				$Trns = $this->execQryI($qryUpdtProctagOthAdj);
		...
		foreach ((array)$this->postAdjustmentOthers("<") as $arrPostAdjOthrsValNeg){//foreach for post adjustment and others negative sign
					...
					if($sumPostvErn >= 0){
						...
						$resCheckOtherEarn = $this->execQryI($qryCheckOtherEarn);
						$rowCheckOtherEarn = $this->getSqlAssocI($resCheckOtherEarn);
					
						
						if($this->getRecCountI($resCheckOtherEarn) > 0){
							...
							$qryUpdtTblEarn = "UPDATE tblEarnings SET trnAmountE = '".sprintf("%01.2f",$finalAmountE)."'
											   ...
											   AND trnCode = '{$arrPostAdjOthrsValNeg['trnCode']}' ";
							if($Trns){
								$Trns = $this->execQryI($qryUpdtTblEarn);
							}
							unset($finalAmountE);
						}
						else{
						...
						if($Trns){
							$Trns = $this->execQryI($qryUpdtProctag);
						}
					}
					else{
		...
		foreach ((array)$this->getArrAllow($emp_AccruedDays,$emp_AmntTardy,$emp_AmntUt,$emp_LegalDays) as $arrAllwIndex => $arrAllwVal){//foreach for allowance
			...
			if((float)$arrAllwVal != 0){
				if($Trns){
					...
					$Trns = $this->execQryI($qryToAllowBrkDwn)or die("error inser");	
				}//
			}//
			if((float)$arrAllwVal != 0){
				if($Trns){
		...
		foreach ((array)$this->getEmpForDeduction() as $empForDedVal){//foreach for deductions
			...
			*/
			...
			//echo "Taxable Earnings = ".$taxableGrossEarn."<br/>";
			...
			$SssEmp = $SssEmplr = $EcEmp = $PhicEmp = $PhicEmplr = $HdmfEmp = $HdmfEmplr = $empGovtDeduct = $mProveER = $mProveEE = 0 ;
			...
			if((int)$this->getCutOffPeriod() == 2)
			{
				...
				foreach($arr_EmpNoRemittance as $arr_EmpNoRemittance_val)
				{
					...
					{
						if ($arr_EmpNoRemittance_val["cat"] == 'all')
							$empGovtDeduct = 1;
						elseif ($arr_EmpNoRemittance_val["cat"] == 'sss')
							$empGovtDeduct = 2;
					}
				}
					...
					if($grossEarnings != 0 || $grossEarnings != "")
					{
						...
						//08-03-2023
						//echo $empForDedVal['empNo'] . " Monthly Gross ==" . $monthLyGrossEarn . "<br>";
						$arrEmpInfo = $this->getUserInfo($_SESSION["company_code"],$empForDedVal['empNo'],'');
						$monthly_salary = (float)$arrEmpInfo['empMrate'];
						//echo $empForDedVal['empNo'] . " Monthly Salary ==" . $monthly_salary . "<br>";
						//08-03-2023
			...
			if($withTax != 0){
				if($amntLimitDed >= $withTax){
					...
					$totalTaxDeducted += sprintf("%01.2f", $withTax); 
					if($Trns){
						$Trns = $this->writeToTblDeduction($empForDedVal['empNo'],'5100',$withTax);				
					}
				} elseif($amntLimitDed < $withTax && $amntLimitDed > 0){
					$totalTaxDeducted += sprintf("%01.2f", $amntLimitDed); 
					if ($this->get['pdNum'] == 24) {
						$taxAdj = $withTax - $amntLimitDed;
						$sqlTaxAdj = "Insert into tblTaxAdj (empNo,taxAmt,payGrp,payCat,pdYear) values ('{$empForDedVal['empNo']}','$taxAdj','{$_SESSION['pay_group']}','{$_SESSION['pay_category']}','{$_GET['pdYear']}')";
						if($Trns){
							$Trns = $this->execQryI($sqlTaxAdj);				
						}
					}
					if($Trns){
			...
			if((int)$this->getCutOffPeriod() == 2)
			{
					if((float)$SssEmp != 0){
						...
						if( $mProveEE != 0){
						if($amntLimitDed >= (float)$mProveEE){
							$amntLimitDed -= (float)$mProveEE;
							$totalGovDeducted += (float)$mProveEE;
							if($Trns){
								$mProveEE = $mProveEE;
								$mProveER = $mProveER;
								$sumGov_Deduct += $mProveEE;
								$Trns = $this->writeToTblDeduction($empForDedVal['empNo'],'5201',$mProveEE,0);
							}
						}
						else
						{
			...
			$minwage_txbleEarningsPd = 0;
			$totalGovDeducted_old = $totalGovDeducted;
			if($empForDedVal['empWageTag']=='Y')
			{
				$minwage_txbleEarningsPd = $txbleEarningsPd;
				$txbleEarningsPd = 0;
				$YtdGovDedMinWage = $totalGovDeducted;
				$totalGovDeducted = 0;
			}
			...
			$totalTaxAdj = 0;
			$YearEndTaxAdj = 0;
			$SSSContAdj = $PHICContAdj = $PAGIBIGContAdj = 0;
			foreach ($resultotherdeduction as $rsotherdeductionlist) {
				if ((float)$amntLimitDed >= (float)$rsotherdeductionlist['trnAmount']) {
					...
					//isama yung iba pang gov contri adj
					if ((int)$this->getCutOffPeriod() == 2)
						{
							if ($rsotherdeductionlist['trnCode'] == SSS_CONTRIB_ADJ) {
								$SSSContAdj = (float)$rsotherdeductionlist['trnAmount'];
								$totalGovDeducted_old += $SSSContAdj;
								$YtdGovDedMinWage += $SSSContAdj;
							}					
							
							if ($rsotherdeductionlist['trnCode'] == PHILHEALTH_CONTRIB_ADJ) {
								$PHICContAdj = (float)$rsotherdeductionlist['trnAmount'];
								$totalGovDeducted_old += $PHICContAdj;
								$YtdGovDedMinWage += $PHICContAdj;
							}					
							if ($rsotherdeductionlist['trnCode'] == PAGIBIG_CONTRIB_ADJ) {
								$PAGIBIGContAdj = (float)$rsotherdeductionlist['trnAmount'];
								$totalGovDeducted_old += $PAGIBIGContAdj;
								$YtdGovDedMinWage += $PAGIBIGContAdj;
							}					
						} 
			...
			if((int)$this->getCutOffPeriod() == 2) 
				{	
					...
					if(($totalGovDeducted_old!=0)||($totalGovDeducted_old!=""))
						{
							 $SssEmp += $SSSContAdj + $mProveEE;
							 $arrSSSEmplr = $this->getSSSEmplrAmt($SSSContAdj);
							 $multiplyer = ($SSSContAdj < 0) ? -1:1;
							 $SssEmplr += (float)($arrSSSEmplr['sssEmployer'] + $mProveER) * $multiplyer;
							 $EcEmp += (float)$arrSSSEmplr['EC'] * $multiplyer;
							 $PhicEmp += $PHICContAdj;
							 $PhicEmplr += $PHICContAdj;
							 $HdmfEmp += $PAGIBIGContAdj;
							 $HdmfEmplr += $PAGIBIGContAdj;
							 //insert rito
							...
							if($Trns){
								$Trns = $this->execQryI($qryToMtdGov);
							}
						}
				}
			//echo "Amount Limit Ded = ".$amntLimitDed."\n";
			...
			*/
			...
			$qryToPayrollSum = "INSERT INTO tblPayrollSummary(compCode,
														...
														VALUES('{$this->session['company_code']}',
															   ...
															   '".sprintf("%01.2f",$minwage_txbleEarningsPd)."',
			...
			$qryToYtdData = "INSERT INTO tblYtdData(compCode,pdYear,empNo,YtdGross,YtdTaxable,YtdGovDed,YtdTax,YtdNonTaxAllow,Ytd13NBonus,Ytdtx13NBonus,payGrp,pdNumber,YtdBasic,sprtAllow,sprtAdvance,YtdGovDedMinWage,YtdGovDedAbvWage)
							 VALUES('{$this->session['company_code']}',
							 		'{$this->get['pdYear']}',
							 		'{$empForDedVal['empNo']}',
							 		'".sprintf("%01.2f",$grossEarnings)."',
									...
									'".sprintf("%01.2f",$sprtAllowPSTotAmnt)."',
									'".sprintf("%01.2f",$allowAdvance)."',
									'".sprintf("%01.2f",$YtdGovDedMinWage)."',
									'".sprintf("%01.2f",$totalGovDeducted)."')";
	...
	function chkUnpostedTran()
	{
		...
		$resUnpostedTran = $this->execQryI($qryUnpostedTran);
		return $this->getRecCountI($resUnpostedTran);
	}
	...
	function getEmpeCola($empNo) {
		...
		return  $this->getSqlAssocI($this->execQryI($sqlEcola));
	}
	
	function getSSSEmplrAmt($amt) {
		$amt = abs($amt);
		$qrySSSEmplr = "SELECT sssEmployer,EC,mProveFund_EE FROM tblSssPhic WHERE (sssEmployee = '$amt') limit 1";
		return $this->getSqlAssocI($this->execQryI($qrySSSEmplr)); 
	}
	...
	function getEmpNonTaxAllow($empNo) {
		...
		$resNonTaxAllow = $this->execQryI($sqlNonTaxAllow);
		$resNonTaxAllow = $this->getSqlAssocI($resNonTaxAllow);
	...
	function getLatestSalary($compCode,$empNo) {
		$query = "SELECT new_empMrate FROM tblpaf_payrollrelatedhist 
						   WHERE compCode = '{$compCode}'
						   AND   empNo    = '".trim($empNo)."'
						   ORDER BY effectivitydate desc LIMIT 1";

		return $this->getSqlAssocI($this->execQryI($query));
	}

	function getOldSalary($compCode,$empNo) {
		$query = "SELECT old_empMrate FROM tblpaf_payrollrelatedhist 
		WHERE compCode = '{$compCode}'
		AND   empNo    = '".trim($empNo)."'
		AND   effectivitydate >= '{$_GET['dtTo']}'
		ORDER BY effectivitydate ASC LIMIT 1";
		//echo $query . '<br>';
		return $this->getSqlAssocI($this->execQryI($query));
	}
	
	function getThisPeriod($compCode,$period) 
	{
