...
/*			$estEarn = 	  (float) $gross_Taxable + (float) $arrYtdDataHist["YtdTaxable"] + (float)$empPrevEarnings -  (float) $arrYtdDataHist["YtdGovDed"] - (float) $arrYtdDataHist["YtdBasic"] 
	...
	private function compute13thmonth($empNo,$curBasic,$curAllow) {
		$sqlEmpMast = "Select empNo from tblEmpMast where compCode='{$_SESSION['company_code']}' and empNo='$empNo' and (dateAdd(month,1,dateHired)<=dateResigned or dateAdd(month,1,dateHired)<=endDate)\n\n";
		if ($this->getRecCount($this->execQry($sqlEmpMast)) != 0 ){
			  $qry13thcheck = "SELECT empBasic FROM tblPayrollSummaryHist where ((pdYear=Year(getdate()) and pdNumber<23) 
						OR (pdYear=Year(getdate())-1 and pdNumber>22)) AND (empNo='$empNo') and compCode='{$_SESSION['company_code']}' and empNo NOT IN (Select empNo from tblPayrollSummaryhist where compCode='{$_SESSION['company_code']}' AND pdNumber='25' and pdyear=Year(getdate()));\n";
			  $qry13th = "SELECT (SUM(empBasic)+$curBasic)/12 as regular,(SUM(sprtAllowAdvance)+$curAllow)/12 as allow FROM tblPayrollSummaryHist where ((pdYear=Year(getdate()) and pdNumber<23) 
						OR (pdYear=Year(getdate())-1 and pdNumber>22)) AND (empNo='$empNo') and compCode='{$_SESSION['company_code']}' and empNo NOT IN (Select empNo from tblPayrollSummaryhist where compCode='{$_SESSION['company_code']}' AND pdNumber='25' and pdyear=Year(getdate()));\n";
...
foreach ((array)$this->getEmpToProcPayBasic() as $empToProcPayBaicVal){//foreach pay basics
		...
		foreach ((array)$this->postAdjustmentOthers("<") as $arrPostAdjOthrsValNeg){//foreach for post adjustment and others negative sign
					...
					if (in_array($arrPostAdjOthrsValNeg['empNo'],array('010002660','570000030')))
						$sumPostvErn = 1;
			...
			foreach($resultloans as $rsloans) {
				if ((float)$amntLimitDed >= (float)$rsloans['sumamount']) {
					...
					if (substr($rsloans['lonTypeCd'],0,1) == 3) {
						if ($empForDedVal['empNo']=='010000425') {
							echo " {$rsloans['lonTypeCd']} {$rsloans['sumamount']} $amntLimitDed;\n";
						}
						if ($amntLimitDedForAdvances > 0 &&  $amntLimitDedForAdvances > (float)$amntLimitDed) {
