...
class regPayrollProcObj extends commonObj {
	...
	private function getPreviousCutOff($pdYear, $pdNum)
	{
		...
		foreach ((array)$this->getEmpAllowance('tblAllowance','','') as $allowArrVal){//allowance
			...
			$arr_empEarnings_curr = $this->getEmpEarningsInfo($_SESSION["company_code"], $_GET["pdYear"], $_GET["pdNum"], $allowArrVal['empNo'],'tblEarnings',1,trim($allowArrVal['attnBase']),$sum_employee_absent);
			$arr_empEarnings_hist = $this->getEmpEarningsInfo($_SESSION["company_code"], $arrDateTimeSheet["pdYear"], $arrDateTimeSheet["pdNumber"], $allowArrVal['empNo'],'tblEarningsHist',1,trim($allowArrVal['attnBase']),$sum_employee_absent);
			$arr_empEarnings_denom_curr = $this->getEmpEarningsInfo($_SESSION["company_code"], $_GET["pdYear"], $_GET["pdNum"], $allowArrVal['empNo'],'tblEarnings',2,trim($allowArrVal['attnBase']),0);
			$arr_empEarnings_denom_hist = $this->getEmpEarningsInfo($_SESSION["company_code"], $arrDateTimeSheet["pdYear"], $arrDateTimeSheet["pdNumber"], $allowArrVal['empNo'],'tblEarningsHist',2,trim($allowArrVal['attnBase']),0);
			...
			switch($allowArrVal['empPayType']) {
				case 'M'://pay type M = monthly
					...
					else
					{
						...
						switch (trim($allowArrVal['allowPayTag'])){
							case 'P':
								$totalAllowAmnt = ($dai_allowAmnt/2) * ($arr_empEarnings_curr["sumBasic"]/$arr_empEarnings_denom_curr["sumBasic"]);
								//echo $dai_allowAmnt."/2"." * (".$arr_empEarnings_curr["sumBasic"]."/".$arr_empEarnings_denom_curr["sumBasic"].")\n";
								$totalAllowAmnt = sprintf("%01.2f", $totalAllowAmnt);
							break;
				...
				case 'D'://pay type D = daily
					...
					//echo $allowArrVal['empNo']."=".$dai_allowAmnt ."* (".$emp_AccruedDays[$allowArrVal['empNo']]." - (".$empTardUt."/".$allowArrVal['empDrate']."))";
	...
	private function getEmpEarningsInfo($compCode, $pdYear, $pdNum, $empNo, $tbl_Earn, $alltrnCode, $attnBase, $cntAbsentEmp)
	{
		if($alltrnCode==1)
		{
			if($attnBase=='Y')
	...
	private function getWtaxAdj() {
		$qryTaxAdj = "Select empNo,trnAmountD from tblDeductions where pdYear='".$this->get['pdYear']."' and pdNumber='".$this->get['pdNumber']."' and compCode='{$_SESSION['company_code']}' and trnCode='8024' and empNo IN (Select empNo from tblEmpMast where empPayGrp='{$_SESSION['pay_group']}'and empPayCat='{$_SESSION['pay_category']}' and compCode='{$_SESSION['company_code']}')";
		return $this->getArrRes($this->execQry($qryTaxAdj));		
	}
	...
	public function mainProcRegPayroll(){
		...
		foreach ((array)$this->getEmpForDeduction() as $empForDedVal){//foreach for deductions
			...
			*/
			...
			//wtax Adjustments
			foreach($arrEmpWTaxAdj as $valWTaxAdj) {
				if ($valWTaxAdj['empNo']==$empForDedVal['empNo']) {
					$totalTaxDeducted += float($valWTaxAdj['trnAmountD']);
				}
			}
