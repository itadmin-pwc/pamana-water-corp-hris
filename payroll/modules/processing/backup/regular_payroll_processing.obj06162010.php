	...
	public function mainProcRegPayroll(){
		...
		foreach ((array)$this->getEmpForDeduction() as $empForDedVal){//foreach for deductions
			...
			*/	
			...
			$SssEmp = $SssEmplr = $EcEmp = $PhicEmp = $PhicEmplr = $HdmfEmp = $HdmfEmplr =0;
			...
			if((int)$this->getCutOffPeriod() == 2)
			{
					if((float)$SssEmp != 0){
						if($amntLimitDed >= (float)$SssEmp){
							...
							if($Trns){
								$Trns = $this->writeToTblDeduction($empForDedVal['empNo'],'5200',$SssEmp);
							}
						}
					}
					...
					if((float)$PhicEmp!= 0){
						if($amntLimitDed >= (float)$PhicEmp){
							...
							if($Trns){
								$Trns = $this->writeToTblDeduction($empForDedVal['empNo'],'5300',$PhicEmp);
							}
						}
					}
					...
					if((float)$HdmfEmp!= 0){
						if($amntLimitDed >= (float)$HdmfEmp){
							...
							if($Trns){ 
								$Trns = $this->writeToTblDeduction($empForDedVal['empNo'],'5400',$HdmfEmp);						
							}
						}				
					}
