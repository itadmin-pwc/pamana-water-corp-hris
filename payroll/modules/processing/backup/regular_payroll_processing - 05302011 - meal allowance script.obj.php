...
class regPayrollProcObj extends commonObj {
	...
	private function getPreviousCutOff($pdYear, $pdNum)
	{
		...
		foreach ((array)$this->getEmpAllowance('tblAllowance','','') as $allowArrVal){//allowance
			...
			$arr_empEarnings_curr = $this->getEmpEarningsInfo($_SESSION["company_code"], $_GET["pdYear"], $_GET["pdNum"], $allowArrVal['empNo'],'tblEarnings',1,trim($allowArrVal['attnBase']),$sum_employee_absent,$allowArrVal['allowTag'],$allowArrVal['allowCode']);
			$arr_empEarnings_hist = $this->getEmpEarningsInfo($_SESSION["company_code"], $arrDateTimeSheet["pdYear"], $arrDateTimeSheet["pdNumber"], $allowArrVal['empNo'],'tblEarningsHist',1,trim($allowArrVal['attnBase']),$sum_employee_absent,$allowArrVal['allowTag'],$allowArrVal['allowCode']);
	...
	{
		$qryAllowAdjOthrs = "Select  earnTranDtl.empNo, earnTranDtl.trnCode, SUM(earnTranDtl.trnAmount) AS sumEarnAmnt, earnTranDtl.trnTaxCd, allowType.sprtPS
							  ...
							  WHERE  (earnTranDtl.compCode = '{$this->session['company_code']}') AND 
								...
								GROUP BY earnTranDtl.empNo, earnTranDtl.trnCode, earnTranDtl.trnTaxCd, allowType.sprtPS";
	...
	private function getEmpEarningsInfo($compCode, $pdYear, $pdNum, $empNo, $tbl_Earn, $alltrnCode, $attnBase, $cntAbsentEmp,$allowCode, $emp_AllowCode)
	{
		...
		if($alltrnCode==1)
		{
			if($attnBase=='Y')
			{
				if($allowCode=='M')
				{
					if(($cntAbsentEmp>12) || ($emp_AllowCode==MEALALLOWCODE))
