	...
	private function ClossingPaySummary() {
		 $qryClossingSummary="Insert into tblPayrollSummaryHist 
							(compCode, pdYear, pdNumber, empNo, payGrp, payCat, empLocCode, empBrnCode, empBnkCd, grossEarnings,
							 taxableEarnings, totDeductions,nonTaxAllow, netSalary, taxWitheld, empDivCode, empDepCode,empSecCode,sprtAllow,empBasic,empMinWageTag,empEcola, empTeu,empYtdTaxable,empYtdTax,empYtdGovDed,sprtAllowAdvance)
							 ...
							 empDepCode, empSecCode,sprtAllow,empBasic,empMinWageTag,empEcola,empTeu,empYtdTaxable,empYtdTax,empYtdGovDed,sprtAllowAdvance from tblPayrollSummary 
					 		 WHERE compCode = '{$this->session['company_code']}'
							 AND payGrp = '{$this->session['pay_group']}'
							 AND payCat = '{$this->session['pay_category']}'
							 AND pdYear = '{$this->get['pdYear']}'
	...
	public function mainProcCloseRegPay(){
		...
		foreach ((array)$this->getEmployeeRestDay() as $GetEmpRD_Val){
			if($GetEmpRD_Val["empRestDay"]!=""){
				...
				$updateTblEmpRDBckUp = "Update tblEmpMast set empRestDay='06/27/2010,07/04/2010,07/11/2010' where empNo='".$GetEmpRD_Val["empNo"]."'";
				if($Trns){
