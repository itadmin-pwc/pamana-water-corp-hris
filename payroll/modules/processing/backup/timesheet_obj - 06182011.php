...
class inqTSObj extends commonObj 
{
	...
	{	
		...
		foreach($arrmtdGovtHist as $arrmtdGovtHist_val)
		{
			...
			if($arrmtdGovtHist_val["sssEmp"]!=0)
			{
	...
	function loanAdjustment($pdYear,$pdMonth)
	{
		$sqlLoanAdj = "SELECT     tblDeductionsHist.empNo, tblEmpMast.empLastName, tblEmpMast.empFirstName, tblEmpMast.empMidName, tblLoanType.lonTypeShortDesc, 
						...
						WHERE     tblDeductionsHist.compCode='".$_SESSION["company_code"]."' and (tblDeductionsHist.trnCode IN (N'5901', '5902')) 
						AND pdNumber IN (Select pdNumber from tblPayPeriod where compCode='".$_SESSION["company_code"]."' AND Month(pdPayable) = '$pdMonth' AND pdYear='$pdYear')
						AND pdYear='$pdYear'
						";
