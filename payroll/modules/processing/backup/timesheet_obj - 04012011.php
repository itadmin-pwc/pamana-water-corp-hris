...
class inqTSObj extends commonObj 
{
		...
		foreach($arrmtdGovtHist as $arrmtdGovtHist_val)
		{
			...
			{		//Falls on Jan, Apr, Jul, or Oct
				$txtOut .=$this->Space(1).$EmpCont.$this->Space(4)."0.00".$this->Space(4)."0.00".
						...
						$EmpEc.$this->Space(1)."0.00".$this->Space(2)."0.00".$this->Space(6).$empDRes.$Emp_DateHired."\r\n";
						
			}
			elseif($pdMonth==2 or $pdMonth==5 or $pdMonth==8 or $pdMonth==11) 
			{		//falls on FEB, MAY, AUG, & NOV
				$txtOut .=$this->Space(4)."0.00".$this->Space(1).$EmpCont.$this->Space(4)."0.00".$this->Space(2)."0.00".$this->Space(2)."0.00".
						  $this->Space(2)."0.00".$this->Space(2)."0.00".$this->Space(1).$EmpEc.$this->Space(2)."0.00".$this->Space(6).$empDRes.$Emp_DateHired."\r\n";
			}
			else 
			{   // falls on MAR, JUN, SEP, & DEC
				$txtOut .=$this->Space(4)."0.00".$this->Space(4)."0.00".$this->Space(1).$EmpCont.$this->Space(2)."0.00".$this->Space(2)."0.00".$this->Space(2)."0.00".
						  $this->Space(2)."0.00".$this->Space(2)."0.00".$this->Space(1).$EmpEc.$this->Space(6).$empDRes.$Emp_DateHired."\r\n";
			}
	...
	{	
		...
		{
			$Amort = number_format($SSSLoan_val["Amount"],2);
			$Amort = str_replace(",","",$Amort);
			$Amort = str_replace(".","",$Amort);
			$LoanAmt = number_format($SSSLoan_val["lonAmt"],2);
			$LoanAmt = str_replace(",","",$LoanAmt);
			$LoanAmt = str_replace(".","",$LoanAmt);
			$totAmt += round($SSSLoan_val["Amount"],2);
			$lonStart = date('ymd',strtotime($SSSLoan_val["lonStart"]));
			$txtOut.="10".$SSSLoan_val["empSssNo"].strtoupper(substr($SSSLoan_val["empLastName"],0,15)).$this->Space(15-strlen(substr($SSSLoan_val["empLastName"],0,15))).strtoupper(substr($SSSLoan_val["empFirstName"],0,15)).$this->Space(15-strlen(substr($SSSLoan_val["empFirstName"],0,15))).strtoupper($SSSLoan_val["empMidName"][0]).$this->Space(1)."S$lonStart0".$LoanAmt.$this->Space(10-strlen($LoanAmt),'0').$this->Space(9-strlen($Amort),'0').$Amort.$this->Space(1).$SSSLoan_val["empStat"]."\r\n";
			unset($Amort,$LoanAmt);
		}
		$totAmt = str_replace(".","",$totAmt);
		$txtOut .='99'.$this->Space(4-strlen(count($arrSSSLoan)),'0').count($arrSSSLoan).$this->Space(18-strlen($totAmt),'0').$totAmt."\r\n";
		return $txtOut;
	}		
	
	function Loans($compCode, $pdYear, $pdNumber,$lonCode) {
		$sqlHDMFLoan = "SELECT tblEmpMast.empNo,tblEmpMast.empLastName, tblEmpMast.empFirstName, tblEmpMast.empMidName, tblEmpLoansDtlHist.ActualAmt AS Amount, tblEmpLoans.lonAmt, 
                      tblEmpMast.empSssNo, empStat = CASE empStat WHEN 'TR' THEN 'T' ELSE ' ' END,lonStart,lonGranted,pdNumber
	...
	{	
		$arrHDMFLoan = $this->Loans($compCode,$pdYear,$pdMonth,3);
