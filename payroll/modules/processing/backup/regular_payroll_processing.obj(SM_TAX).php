...
class regPayrollProcObj extends commonObj {
	...
	private function getPreviousCutOff($pdYear, $pdNum)
	{
		...
		$qryPrevCutOff = "Select * from tblPayperiod where 
							...
							and pdYear='".$pdYear."' and pdNumber='".$pdNum."'
							and pdProcessTag='Y'";
	...
	private function getArrAllow(){
		...
		foreach ((array)$this->getEmpAllowance('tblAllowance','','') as $allowArrVal){//allowance
			...
			$qryGetAllwTs = "SELECT tsDate,hrsAbsent,hrsTardy,hrsUt FROM tblTimeSheet 
							WHERE compCode = '{$this->session['company_code']}' 
							AND empNo = '{$allowArrVal['empNo']}'
							AND empPayGrp = '{$allowArrVal['empPayGrp']}'
							AND empPayCat = '{$allowArrVal['empPayCat']}' ";			
					...
					if(($allowArrVal['allowSked'] == '1')||($allowArrVal['allowSked'] == '2'))
					{
						switch (trim($allowArrVal['allowPayTag'])){
							case 'P':
								...
								switch (trim($allowArrVal['attnBase'])){
									case 'Y':
										$qryGetAllwTs .= "AND tsDate BETWEEN '".$this->dateFormat($allowArrVal['allowStart'])."' AND '{$this->get['dtTo']}' 
														  AND hrsAbsent <> '8' ";
										$resGetAllwTs = $this->execQry($qryGetAllwTs);					
											foreach ((array)$this->getArrRes($resGetAllwTs) as $tsResVal){
												if((float)$tsResVal['hrsTardy'] != 0 || (float)$tsResVal['hrsUt'] != 0){
													$withTardy = $dailyAmnt-($hourlyAmnt*((float)$tsResVal['hrsTardy']+(float)$tsResVal['hrsUt']));
												}
												else{
													$withTardy = (float)$dailyAmnt;	
												}
												$totalAllowAmnt += (float)$withTardy;
											}
										
											if(($dsDate!="") && ($deDate!=""))
											{
												$qrygetPrevTS = "SELECT tsDate,hrsAbsent,hrsTardy,hrsUt FROM tblTimeSheetHist 
																WHERE compCode = '{$this->session['company_code']}' 
																AND empNo = '{$allowArrVal['empNo']}'
																AND empPayGrp = '{$allowArrVal['empPayGrp']}'
																AND empPayCat = '{$allowArrVal['empPayCat']}'
																AND tsDate BETWEEN '".$this->dateFormat($dsDate)."' AND '".$this->dateFormat($deDate)."'
																AND hrsAbsent <> '8'";
												$resgetPrevTS = $this->execQry($qrygetPrevTS);	
												foreach ((array)$this->getArrRes($resgetPrevTS) as $resgetPrevTS_Val)
												{
													if((float)$resgetPrevTS_Val['hrsTardy'] != 0 || (float)$resgetPrevTS_Val['hrsUt'] != 0){
														$withTardy = $dailyAmnt-($hourlyAmnt*((float)$resgetPrevTS_Val['hrsTardy']+(float)$resgetPrevTS_Val['hrsUt']));
													}
													else{
														$withTardy = (float)$dailyAmnt;	
													}
							...
							case 'T':
								...
								switch (trim($allowArrVal['attnBase'])){
									case 'Y':
										$qryGetAllwTs .= "AND tsDate BETWEEN '".$this->dateFormat($allowArrVal['allowStart'])."' AND '".$this->dateFormat($allowArrVal['allowEnd'])."' 
																  AND hrsAbsent <> '8' ";
							...
							switch (trim($allowArrVal['attnBase'])){
								...
								case 'N':
									...
									$qryGetAllwTs .= "AND tsDate BETWEEN '".$this->dateFormat($allowArrVal['allowStart'])."' AND '".$this->dateFormat($allowArrVal['allowEnd'])."'";
										$resGetAllwTs = $this->execQry($qryGetAllwTs);					
										foreach ((array)$this->getArrRes($resGetAllwTs) as $tsResVal){
											$totalAllowAmnt += (float)$dailyAmnt;	
	...
	private function getEmpForDeduction(){
		$qryGetEmpForDeduct = "SELECT  tblEmpMast.empNo, tblEmpMast.empTeu, tblEmpMast.empMrate, tblEmpMast.empBrnCode, tblBranch.minWage
	...
	private function computeEmpGrossEarnings($empNo,$and,$minWage){
		...
		if($minWage!=0)
		{
			$qryGetEmpGrossEarn.="AND (ern.trnCode!='0100') ";
		}
	...
	/* Gen's Function */
	private function computeSmWithTax($gross_Taxable,$empTeu)
	{
		
		//Computation for SM Tax
		$qrySmTax = "Select * from tblSmTax where teuCode='".$empTeu."' and '".$gross_Taxable."' >= txUpLimit order by teuCode,txSeqNo desc";
		$resSmTax = $this->execQry($qrySmTax);
		$rowSmTax = $this->getSqlAssoc($resSmTax);
		
		
		$empTax = $rowSmTax["txFixdAmt"] + ($rowSmTax["txAddPcent"] * ($gross_Taxable - $rowSmTax["txUpLimit"]));
		$empTax = ($empTax!=0?$empTax:0);
	...
	private function computePrevTax($empNo)
	{
		$with_Tax_Prev =0;
		$qrychkRecPaySum = "Select count(*) as cntRec
							from tblPayrollSummaryHist
							where compCode = '".$this->session['company_code']."' AND pdYear = '".$this->get['pdYear']."' and empNo = '".$empNo."'";
		$reschkRecPaySum = $this->execQry($qrychkRecPaySum);
		$rowchkRecPaySum = $this->getSqlAssoc($reschkRecPaySum);
		if($rowchkRecPaySum["cntRec"]==0)
		{
			$remMonths = 25 - $this->get['pdNum'];
			
			$qrygetPrevEmplr = "Select * from tblPrevEmployer where empNo='".$empNo."' and yearCd='".$this->get['pdYear']."' and compCode='".$this->session['company_code']."' and prevStat='A'";
			$resgetPrevEmplr = $this->execQry($qrygetPrevEmplr);
			while($rowgetPrevEmplr = $this->getSqlAssoc($resgetPrevEmplr))
			{
				$with_Tax_Prev+=$rowgetPrevEmplr["prevTaxes"];
			}
			 
			$prevEmpTax = $with_Tax_Prev;
			$prevEmpTax = $prevEmpTax/$remMonths;
			
		}
		else
		{
			$chkRemTaxtoPay = $this->getPrevEmplr($empNo,"taxDeducted");
			$getPrevTax = $this->getPrevEmplr($empNo,"prevTaxes");
			
			if($getPrevTax!=$chkRemTaxtoPay)
			{
				$prevEmpTax = $this->getPrevEmplr($empNo,"taxPerMonth");
			}
	...
	private function computeAnnTaxPrevEmp($gross_Taxable,$empTeu,$empNo)
	{
		//Compute its Semi Monthly Tax
		$EmpWithTax = $this->computeSmWithTax($gross_Taxable,$empTeu);
		
		//Add the Previous Tax
		$EmpWithTax +=$this->computePrevTax($empNo);
		
		
		return (float) $EmpWithTax;
	}
	...
	private function computeWithTax($empNo,$gross_Taxable,$empTeu)
	{
		//Check if the Employee has Previous Employer
		$qrychkPrevEmp = "Select count(*) as cntPrevEmp from tblPrevEmployer where compCode='".$this->session['company_code']."' and empNo='".$empNo."' and yearCd='".$this->get['pdYear']."'";
		$reschkPrevEmp = $this->execQry($qrychkPrevEmp);
		$rowchkPrevEmp = $this->getSqlAssoc($reschkPrevEmp);
		
		if($rowchkPrevEmp["cntPrevEmp"]>=1)
		{
			$EmpWithTax = $this->computeAnnTaxPrevEmp($gross_Taxable,$empTeu,$empNo);
		}
		else
		{
			$EmpWithTax = $this->computeSmWithTax($gross_Taxable,$empTeu);
	...
	//will program 
	function getdeductionlist($empNo) {
		$qryloans="SELECT tblEmpLoansDtl.lonTypeCd, SUM(tblEmpLoansDtl.trnAmountD) AS sumamount, tblLoanType.trnCode, 
                      tblEmpLoansDtl.empNo
					...
					AND (tblEmpLoansDtl.empNo = '$empNo') AND (dedTag<>'Y')
					AND (tblEmpLoansDtl.lonRefNo IN
                          (SELECT lonRefNo FROM tblEmpLoans
                            WHERE lonsked IN (3, {$this->getCutOffPeriod()})))
					GROUP BY tblEmpLoansDtl.lonTypeCd, tblEmpLoansDtl.empNo, tblLoanType.trnCode 
	...
	function getotherdeductionlist($empNo,$cat){
		if ($cat==1) {
			$qryother="select trnAmount,trnCode,seqNo,empNo from tblDedTranDtl 
						where empNo='$empNo' 
		...
		elseif ($cat==0) {
			$qryother="select processtag, SUM(trnAmount) AS sumamount, empNo, trnCode, compCode, trnPriority from tblDedTranDtl 
						where empNo='$empNo' 
						and compCode='" . $this->session['company_code'] . "' 
						and dedStat='A' 
						and payGrp='" . $this->session['pay_group'] . "' 
		...
		elseif ($cat==2) {
			$qryother="select trnAmount,trnCode,seqNo,empNo from tblDedTranDtl 
						...
						and (trnCode in 
												(SELECT trnCode FROM tblPayTransType where trnApply in (3,{$this->getCutOffPeriod()})))
						and (trnCode in (SELECT trnCode FROM tblLoanType where compCode='" .  $this->session['company_code'] . "'))						
						order by trnPriority";
	...
	function updateotherdeductions($seqNo) {
		$qryupdateotherdeductions="Update tblDedTranDtl set processtag='Y' where seqNo='$seqNo'"; 
		return $this->execQry($qryupdateotherdeductions);
	}
	
	function updateloansdtl($empNo,$lonCd) {
		$qryupdateloansdtl="Update tblEmpLoansDtl set dedTag='Y' 
							WHERE (tblEmpLoansDtl.compCode = '" . $this->session['company_code'] . "') 
									AND (tblEmpLoansDtl.pdYear = '" . $this->get['pdYear'] . "') 
							        AND (tblEmpLoansDtl.pdNumber = '" . $this->get['pdNum'] . "')
									AND (lonTypeCd='$lonCd')
...
$qryUpdateEmpLoans = "UPDATE tblEmpLoansDtl SET dedTag = ''
	...
	public function mainProcRegPayroll(){
		...
		foreach ((array)$this->getArrAllow() as $arrAllwIndex => $arrAllwVal){//foeach for allowance
		...
		foreach ((array)$this->getEmpForDeduction() as $empForDedVal){//foreach for deductions
			...
			*/	
			...
			$taxableGrossEarn = (float)$this->computeEmpGrossEarnings($empForDedVal['empNo'],"AND (trn.trnTaxCd = 'Y') AND (sprtPS IS NULL or sprtPS ='' or sprtPS='N') ",$empForDedVal['minWage']);
			//echo "Taxable Earnings = ".$taxableGrossEarn."\n";
			
			/*If the Branch.Minimum Wage!=0, Get 0100(Basic) of the Employee*/
			if($empForDedVal['minWage']!=0)
				$empMinWage_Basic = (float)$this->computeEmpGrossEarnings($empForDedVal['empNo'],"AND ern.trnCode='0100'",'');
			...
			*/
			...
			$taxGrossEarn_Gov = sprintf("%01.2f",($taxableGrossEarn - $sumGov));
			
			$withTax = $this->computeWithTax($empForDedVal['empNo'],$taxGrossEarn_Gov,$empForDedVal["empTeu"]);
			...
			$resultloans = $this->getdeductionlist($empForDedVal['empNo']);
			$rsadjustments = $this->getotherdeductionlist($empForDedVal['empNo'],2);
			foreach($resultloans as $rsloans) {
				$totaloansandadjustment = (float)$rsloans['sumamount'];
				foreach($rsadjustments as $rsadjusment) {
					if ($rsloans['trnCode'] == $rsadjusment['trnCode']) {
						$totaloansandadjustment += (float)$rsadjusment['trnAmount'];
						$ArSeq[]=$rsadjusment['seqNo'];
					}
				}	
				if ((float)$amntLimitDed >= (float)$totaloansandadjustment) {
					$amntLimitDed = (float)$amntLimitDed - $totaloansandadjustment;
					
					if($Trns){
							$trns = $this->updateloansdtl($rsloans['empNo'],$rsloans['lonTypeCd']);
					}
					if($Trns){
						$totDedForPeriod +=(float)$totaloansandadjustment;
						$trns = $this->writeToTblDeduction($rsloans['empNo'],$rsloans['trnCode'],$totaloansandadjustment);		
					}
					
					if (count($ArSeq) > 0) {
						for ($seqindex = 0;$seqindex < count($ArSeq); $seqindex++) {
							if($Trns){
								$trns = $this->updateotherdeductions($ArSeq[$seqindex]);
							}							
						}
					}
				}
			...
			foreach ($resultotherdeduction as $rsotherdeductionlist) {
				if ((float)$amntLimitDed >= (float)$rsotherdeductionlist['trnAmount']) {
					...
					if($Trns){
						$Trns = $this->updateotherdeductions($rsotherdeductionlist['seqNo']);
					}
				}
			}
			//echo "Amount Limit Ded = ".$amntLimitDed."\n";
			...
			$qryToYtdData = "INSERT INTO tblYtdData(compCode,pdYear,empNo,YtdGross,YtdTaxable,YtdGovDed,YtdTax,YtdNonTaxAllow,Ytd13NBonus,Ytdtx13NBonus,payGrp,pdNumber,YtdBasic)
							 VALUES('{$this->session['company_code']}',
							 		'{$this->get['pdYear']}',
							 		'{$empForDedVal['empNo']}',
							 		'".sprintf("%01.2f",$grossEarnings)."',
			...
			unset($grossEarnings,$netPayEarnings,$amntLimitDed,$nonTaxEarn,$taxableGrossEarn,$sumGov,$withTax,$totDedForPeriod,$netsalary,$totalGovDeducted,$arrAllwVal,$totEmpNonTaxAllow,$totalTaxDeducted,$ArSeq,$totaloansandadjustment,$sprtAllowPSTotAmnt,$taxGrossEarn_Gov,$empMinWage_Basic);
