...
*/	
	...
	function UpdateStrCodeShaw() {
		 $qryUpdate = "Update tblPayJournal set strCode2='1001202' where compCode='{$_SESSION['company_code']}' AND (pdYear = '{$this->get['pdYear']}') AND pdNumber = '{$this->get['pdNumber']}' AND payGrp='{$_SESSION['pay_group']}' and payCat = '{$_SESSION['pay_category']}' AND compGLCode not IN (102,103,104,105,301) AND majCode IN (350,35,310)";
		return  $this->execQry($qryUpdate);
	}
	function UpdateMinCode() {
		$qryUpdate = "Update tblPayJournal set minCode2='112' where compCode='{$_SESSION['company_code']}' AND (pdYear = '{$this->get['pdYear']}') AND pdNumber = '{$this->get['pdNumber']}' AND payGrp='{$_SESSION['pay_group']}' and payCat = '{$_SESSION['pay_category']}' AND minCode = 104 and strCode=901";
		return  $this->execQry($qryUpdate);
	}
...
/*		if (!$this->getPeriod($_GET['payPd'])) {
	...
	function EmGovContList() {
		...
		if ($_SESSION['pay_category'] == 1)	 {
			$qryGov .= " AND empNo not IN (select empNo from tblNonEmpGov where compCode='{$_SESSION['company_code']}')";
		}
