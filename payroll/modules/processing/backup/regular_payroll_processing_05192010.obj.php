...
class regPayrollProcObj extends commonObj {
	...
	private function getPreviousCutOff($pdYear, $pdNum)
	{
		...
		$arrChecker = array();
		$noCompDays = $empEarnings_numerator = 0;
		...
		foreach ((array)$this->getEmpAllowance('tblAllowance','','') as $allowArrVal){//allowance
			...
			$arr_empEarnings_curr = $this->getEmpEarningsInfo($_SESSION["company_code"], $_GET["pdYear"], $_GET["pdNum"], $allowArrVal['empNo'],'tblEarnings',1);
			$arr_empEarnings_hist = $this->getEmpEarningsInfo($_SESSION["company_code"], $arrDateTimeSheet["pdYear"], $arrDateTimeSheet["pdNumber"], $allowArrVal['empNo'],'tblEarningsHist',1);
			$arr_empEarnings_denom_curr = $this->getEmpEarningsInfo($_SESSION["company_code"], $_GET["pdYear"], $_GET["pdNum"], $allowArrVal['empNo'],'tblEarnings',2);
			$arr_empEarnings_denom_hist = $this->getEmpEarningsInfo($_SESSION["company_code"], $arrDateTimeSheet["pdYear"], $arrDateTimeSheet["pdNumber"], $allowArrVal['empNo'],'tblEarningsHist',2);
	...
	private function getEmpEarningsInfo($compCode, $pdYear, $pdNum, $empNo, $tbl_Earn, $alltrnCode)
	{
		if($alltrnCode==1)
