...
*/	
	...
	function UpdateStrCodeShaw() {
		 $qryUpdate = "Update tblPayJournal set strCode2='901' where compCode='{$_SESSION['company_code']}' AND (pdYear = '{$this->get['pdYear']}') AND pdNumber = '{$this->get['pdNumber']}' AND payGrp='{$_SESSION['pay_group']}' and payCat = '{$_SESSION['pay_category']}' AND compGLCode not IN (102,103,104,105,301) AND majCode IN (350,35,310)";
		return  $this->execQry($qryUpdate);
	}
