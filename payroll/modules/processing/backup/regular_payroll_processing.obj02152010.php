...
class regPayrollProcObj extends commonObj {
	...
	private function unPostedTranOthEarn()
	{
		...
		$qryGetOthEarn =  "SELECT dtl.compCode,empNo, dtl.trnCode, trnAmount AS sumEarnAmnt, pdNumber, pdYear, trnCntrlNo
							...
							WHERE (dtl.compCode = '".$_SESSION["company_code"]."') 
							  AND (dtl.processtag is null or dtl.processtag='N')
	...
	private function writeToTblUnpostedTran($empCompCd,$empId,$trnCd,$trnAt,$pdNum,$pdYr,$trnCntNo)
	{
		$writeToTblUnpostedTran = "INSERT INTO tblUnpostedTran
							  (compCode,empNo,trnCode,trnAmt,pdNumber,pdYear,trnCntrlNo,dateAdded)
							  VALUES('".$empCompCd."','".$empId."','".$trnCd."','".sprintf("%01.2f",$trnAt)."','".$pdNum."','".$pdYr."','".$trnCntNo."','".date("m/d/Y")."')";
	...
	private function computeWithTax($empNo,$gross_Taxable,$empTeu,$sumGov)
	{
		$empPrevTag = "";
		$empMinTag = "";
		$empPrevEarnings = 0;
		...
		$basicPay = (float)$this->computeEmpGrossEarnings($empNo,"AND ern.trnCode='0100'",'');
	...
	public function mainProcRegPayroll(){
		...
		foreach ((array)$this->getEmpForDeduction() as $empForDedVal){//foreach for deductions
			...
			*/
			...
			$withTax = $this->computeWithTax($empForDedVal['empNo'],$txbleEarningsPd,$empForDedVal["empTeu"],$sumGov);
		...
		{//foreach for Unposted Other Earnings
			if($Trns){
				$Trns = $this->writeToTblUnpostedTran($arrUnPosTranEarnVal["compCode"],$arrUnPosTranEarnVal["empNo"],$arrUnPosTranEarnVal["trnCode"],$arrUnPosTranEarnVal["sumEarnAmnt"],$arrUnPosTranEarnVal["pdNumber"],$arrUnPosTranEarnVal["pdYear"],$arrUnPosTranEarnVal["trnCntrlNo"]);
			}
		...
		{//foreach for Unposted Other Ded
			if($Trns){
				$Trns = $this->writeToTblUnpostedTran($arrUnPosTranDedVal["compCode"],$arrUnPosTranDedVal["empNo"],$arrUnPosTranDedVal["trnCode"],$arrUnPosTranDedVal["sumDedAmnt"],$arrUnPosTranDedVal["pdNumber"],$arrUnPosTranDedVal["pdYear"],$arrUnPosTranDedVal["trnCntrlNo"]);
			}
