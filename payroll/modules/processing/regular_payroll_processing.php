...
if($_SESSION['pay_category']==9) {
	header("Location: lastpay_processing.php");	
}
//include("../../../includes/userErrorHandler.php");
include("../../../includes/dbI.inc.php");
include("../../../includes/commonI.php");
include("regular_payroll_processing.obj.php");
include_once("regular_payroll_closing.obj.php");

$arrGET = $_GET;
$arrGET['dtFrm'] = date('Y-m-d',strtotime($_GET['dtFrm']));
$arrGET['dtTo'] = date('Y-m-d',strtotime($_GET['dtTo']));
$regPayProcObj = new regPayrollProcObj($arrGET,$_SESSION);
$closeRegPay = new closeRegPayroll($arrGET,$_SESSION);
...
if(isset($_GET['action'])){
	switch ($_GET['action']){
		case 'procRegPay':
				//$regPayProcObj->mainProcRegPayroll();
				$chckPeriodTags = $regPayProcObj->checkPeriodTags();//check timesheet tag processing

				if(trim($chckPeriodTags['pdEarningsTag']) == 'N' || trim($chckPeriodTags['pdEarningsTag']) == 'Y'){

					if($regPayProcObj->reProcRegPayroll() == true){
						
						if($regPayProcObj->mainProcRegPayroll() == true){
							if($regPayProcObj->chkUnpostedTran()>=1){
								echo 6;
							}
							else{
...
<HTML>
	...
	<BODY>
		<FORM name='frmProcRegPay' id="frmProcRegPay" action="<?=$_SERVER['PHP_SELF']?>" method="post" >
							...
							<tr>
								...
								<td class="gridDtlVal2">
									<font class="gridDtlLblTxt" id="payCat"><?
											...
											$paycatDesc = $regPayProcObj->getPayCat($_SESSION['company_code'],$andPayCat);
								...
								<td class="gridDtlVal2">
									<font class="gridDtlLblTxt" id="payPd"><?
											$andPayPeriod = "AND payGrp = '{$_SESSION['pay_group']}'
															 ...
															 AND pdStat IN ('O','') ";
											$arrPayPeriod = $regPayProcObj->getPayPeriod($_SESSION['company_code'],$andPayPeriod);
							...
							<tr>
								...
								<td class="gridDtlVal2">
										<font class="gridDtlLblTxt" id="DteCvrd"><?
											echo date('m/d/Y',strtotime($arrPayPeriod['pdFrmDate'])). " - " .date('m/d/Y',strtotime($arrPayPeriod['pdToDate']));
										?>
										</font>
								</td>
							</tr>
			...
			<?
				$pdPayable = explode("/",date('m/d/Y',strtotime($arrPayPeriod['pdFrmDate'])));
			?>
