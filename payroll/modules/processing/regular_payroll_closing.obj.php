...
class closeRegPayroll extends commonObj {
	...
	function getNewRestDays() {
		...
		$qryNewRestDay = "Exec sp_CreateRD $pdNum,$pdYear,{$this->session['pay_group']},{$this->session['pay_category']}";
		$res = $this->getSqlAssocI($this->execQryI($qryNewRestDay));		
		return date('m/d/Y',strtotime($res['day1'])).','.date('m/d/Y',strtotime($res['day2'])).','.date('m/d/Y',strtotime($res['day3']));
	}
		...
		$qrylist="SELECT tblEmpLoansDtl.compCode, tblEmpLoansDtl.empNo, tblEmpLoansDtl.lonTypeCd,tblEmpLoansDtl.lonRefNo,
				...
				AND (tblEmpLoansDtl.lonRefNo IN
                          (SELECT lonRefNo FROM tblEmpLoans
							...
							";
		return $this->getArrResI($this->execQryI($qrylist));
	}
	...
	function getlastinfoloans($empNo,$lonTypeCd,$lonRefNo){
		$qrylastinfo="Select lonWidInterst,lonPayments,lonPaymentNo,lonCurbal from tblEmpLoans 
					  ...
					  AND lonSked IN (3,".$this->getCutOffPeriod().")
					   ";
		return $this->getSqlAssocI($this->execQryI($qrylastinfo));	
	}
	...
	function updateemploan($empNo,$lonCd,$lonPayments,$lonPaymentNo,$lonCurbal,$lonLastPay,$lonStat,$lonRefNo) {
		...
		return $this->execQryI($qryupdateloans);	
	}
	...
	function deleteemploans() {
		$qrycloseloans="Delete from tblEmpLoansDtl 
						...
						AND pdNumber = '" . $this->get['pdNum'] . "'						
						";
		return $this->execQryI($qrycloseloans);
	}
	...
	function deleteotherdeductions() {
		$qryotherdeductions="Delete from tblDedTranDtl where 
						...
						and (trnCode in 
							...
							";
		return $this->execQryI($qryotherdeductions);
	}
	...
	private function getAllowBrkDwm(){
		...
		$resGetAllowBrkDwn = $this->execQryI($qryGetAllowBrkDwn);
		return $this->getArrResI($resGetAllowBrkDwn);
	}
	...
	private function getMtdGov(){
		...
		$resGetMtdGov = $this->execQryI($qryGetMtdGov);
		return $this->getArrResI($resGetMtdGov);
	}
	...
	private function getTimesheet(){
		$qryGetTs = "SELECT * FROM tblTimeSheet
					  ...
					  AND (empPayCat = '{$this->session['pay_category']}')
					  AND (tsStat = 'A')";
		$resGetTs = $this->execQryI($qryGetTs);
		return $this->getArrResI($resGetTs);
	}
	...
	private function chkEmpPaySum($empNo)
	{
		$qrychkEmpPaySum = "Select count(*) as cntEmpPaySum
							...
							and (payCat = '".$this->session['pay_category']."')";
		$rschkEmpPaySum =  $this->execQryI($qrychkEmpPaySum);
		$rowchkEmpPaySum =  $this->getSqlAssocI($rschkEmpPaySum);
	...
	private function getPrevTaxDeducted($empNo,$field)
	{
		$qrygetTaxDeducted = "Select     ".$field."
							  ...
							  and empNo='".$empNo."'";
		$rsgetTaxDeducted = $this->execQryI($qrygetTaxDeducted);
		$rowgetTaxDeducted =  $this->getSqlAssocI($rsgetTaxDeducted);
	...
	private function computeTax()
	{
		...
		$rsempwithPrevEmp  = $this->execQryI($getempwithPrevEmp);

		if(mysqli_num_rows($rsempwithPrevEmp)>=1)
		{
			while($rowempwithPrevEmp = mysql_fetch_array($rsempwithPrevEmp))
			{
				...
				return $this->execMultiQryI($qryUpdtblPrevEmplyr);	
	...
	private function getUnpostedTran()
	{
		...
		return $this->getArrResI($resUnpostedTran);
	}
	...
	private function getEmployeeRestDay()
	{
		$qryEmpRestD = "SELECT *
							...
							AND empPayGrp = '{$this->session['pay_group']}'
							AND empPayCat = '{$this->session['pay_category']}'
							AND empStat IN ('RG','PR','CN') AND empNo IN (Select empNo from tblPayrollSummary where payGrp='".$_SESSION["pay_group"]."' and payCat='".$_SESSION["pay_category"]."' and pdNumber ='".$this->get['pdNum']."' and pdYear='".$this->get['pdYear']."')";
		$resEmpRestD = $this->execQryI($qryEmpRestD);
		return $this->getArrResI($resEmpRestD);
	}
	...
	public function mainProcCloseRegPay(){
		...
		if($Trns){
				$Trns = $this->execQryI($qryUpdateEarnHeader);
		}
		...
		if($Trns)
		{
			$Trns = $this->execQryI($qryDeltblEarnHdr);
		}*/
		...
		foreach ((array)$this->getAllowBrkDwm() as $allowVal){
			$qryToAllowBrkDwnHst = "INSERT INTO tblAllowanceBrkDwnHst(compCode,empNo,allowCode,allowAmt,allowSked,allowTaxTag,allowPayTag,allowStart,allowEnd,allowStat,pdYear,pdNumber,sprtPS,allowTag)
									VALUES('{$allowVal['compCode']}',
										   ...
										   '{$allowVal['allowTag']}')";
			if($Trns){
				$Trns = $this->execQryI($qryToAllowBrkDwnHst);
			}	
		}
		...
		if($Trns)
		{
			$Trns = $this->execQryI($qryDeltblEmpLoans);
		}
		...
		if($Trns){
			$Trns = $this->execQryI($qryDeleAllowBrkDwn);
		}	
			...
			foreach ((array)$this->getMtdGov() as $mtdGovVal){
				...
				$qryToMtvGovtHist = "INSERT INTO tblMtdGovtHist
									 ...
									 VALUES('{$mtdGovVal['compCode']}',
									        ...
									        '{$mtdGovVal['hdmfEmplr']}')";
				if($Trns){
					$Trns = $this->execQryI($qryToMtvGovtHist);
				}				
			}

			/*	
			$qryToMtvGovtHist_superConfi = "DELETE FROM tblMtdGovt
						   WHERE compCode = '21'
						   AND pdYear = '{$this->get['pdYear']}'
						   AND pdMonth = '{$this->get['pdMonth']}'
						   AND empNo='001';"; 
			
			$qryToMtvGovtHist_superConfi.= "INSERT INTO tblMtdGovtHist
									 (compCode,pdYear,pdMonth,empNo,mtdEarnings,sssEmp,sssEmplr,ec,phicEmp,phicEmplr,hdmfEmp,hdmfEmplr)
									 VALUES('21',
									        '".$this->get['pdYear']."',
									        '".$this->get['pdMonth']."',
									        ...
									        '100');";
			if($Trns){
				$Trns = $this->execQryI($qryToMtvGovtHist_superConfi);
			}
			*/	
		...
		if($Trns){
			$Trns = $this->execQryI($qryDeleMtdGovt);
		}
		////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($Trns){
				$Trns = $this->execQryI($qryUpdateDedHeader);
		}
		...
		if($Trns)
		{
			$Trns = $this->execQryI($qryDeltblDedHdr);
		}*/
		...
		$qryDeleTsHist = "DELETE FROM tblTimeSheetHist
					  ...
					  ";
		if($Trns){
			$Trns = $this->execQryI($qryDeleTsHist);
		}
		...
		foreach ((array)$this->getTimesheet() as $tsVal){
			 $qryToTsHist = "INSERT INTO tblTimeSheetHist(compCode,empNo,tsDate,
														 ...
														 hrsRegNDLe8,amtRegNDLe8)
													VALUES('{$tsVal['compCode']}','{$tsVal['empNo']}','".$this->dateFormat($tsVal['tsDate'])."',
													       '".(float)$tsVal['hrsAbsent']."','".(float)$tsVal['hrsTardy']."','".(float)$tsVal['hrsUt']."',
													       '".(float)$tsVal['hrsOtLe8']."','".(float)$tsVal['hrsOtGt8']."','".(float)$tsVal['hrsNdLe8']."',
													       '".(float)$tsVal['hrsNdGt8']."','{$tsVal['tsRemarks']}','{$tsVal['empPayGrp']}',
													       '{$tsVal['empPayCat']}','{$tsVal['dayType']}','".(float)$tsVal['amtAbsent']."',
													       '".(float)$tsVal['amtTardy']."','".(float)$tsVal['amtUt']."','".(float)$tsVal['amtOtLe8']."',
													       '".(float)$tsVal['amtOtGt8']."','".(float)$tsVal['amtNdLe8']."','".(float)$tsVal['amtNdGt8']."',
													       '{$tsVal['tsRem']}','{$tsVal['tsStat']}','{$tsVal['trnOtLe8']}','{$tsVal['trnOtGt8']}','{$tsVal['trnNdLe8']}','{$tsVal['trnNdGt8']}','".(float)$tsVal['hrsRegNDLe8']."','".(float)$tsVal['amtRegNDLe8']."') \n";		
			if($Trns){
				$Trns = $this->execQryI($qryToTsHist);
			}	
		}
		...
		foreach ((array)$this->getUnpostedTran() as $Unposted_Val){
			...
			if($Trns){
				$Trns = $this->execQryI($writeToTblUnpostedTran);
			}	
		}
		...
		$qryDelUnpost =  "Delete from tblUnpostedTran
							...
							and pdNumber ='".$this->get['pdNum']."' and pdYear='".$this->get['pdYear']."')";
		if($Trns){
			$Trns = $this->execQryI($qryDelUnpost);
		}
	
		/*End of Unposted Tran*/
		
		/*Get all Employee RestDay*/
		/*$newRestDays = $this->getNewRestDays();
		foreach ($arrRD as $GetEmpRD_Val){
			if($GetEmpRD_Val["empRestDay"]!=""){
				...
				if($Trns){
					$Trns = $this->execQryI($writeTblEmpRDBckUp);
				}
				
				$updateTblEmpRDBckUp = "Update tblEmpMast set empRestDay='$newRestDays' where empNo='".$GetEmpRD_Val["empNo"]."'";
				if($Trns){
					$Trns = $this->execQryI($updateTblEmpRDBckUp);
				}	
			}
		}
		*/
		...
		/*if($Trns){
			$Trns = $this->TransferUploadedTS();
		}*/
		/*Create Textfile for Oracle*/
	...
	function getARDeductions() {
		$sqlDeductedAR = "Select 
							loans.lonrefNo+'-'+cast((loans.lonPaymentNo) as varchar(20)) as refNo,
							REPLACE(CONVERT(VARCHAR(13), GETDATE(), 106), ' ', '-') as transDate ,
							'CM' as transType,
							strname as branch,
							custNo,
							strname as branch2,
							...
							'005' as bline,
							'PAYROLL' as source,
							REPLACE(CONVERT(VARCHAR(13), GETDATE(), 106), ' ', '-') as transDate2,
							'' as blank,
							custNo as linedesc,
							1 as qty,
							dtl.ActualAmt*-1 as ded,
							'' as blank2,
							'PHP' as currency,
							loans.mmsNo as invoiceNo,
							loans.compGLCode
							from tblempLoans loans 
									inner join tblEmpLoansdtlhist dtl on loans.empNo=dtl.empNo and loans.lonTypeCd = dtl.lonTypeCd and loans.lonRefno=dtl.lonrefNo
									inner join tblEmpmast emp on loans.empNo=emp.empNo
									inner join tblCustomerNo cust on loans.empNo=cust.empno
							where loans.upLoadTag='2'
							AND loans.lonTypeCd Like '3%' 
							AND dtl.pdNumber = '".$this->get['pdNum']."'
							AND dtl.pdYear = '".$this->get['pdYear']."' 
							AND dtl.trnGrp='{$_SESSION['pay_group']}' 
							AND dtl.trnCat='{$_SESSION['pay_category']}'
							AND mmsNo is not NULL
							AND strname is not null
";	
				return $this->getArrResI($this->execQryI($sqlDeductedAR));		
	}
	function CreateTxtFile() {
		...
		$arrXtra 	= $this->getXtraBranches();
		$arrFiles 	= array();
		$dtTime 	= date('dmy')."_".date('His');
		foreach($this->getARDeductions() as $val) {
			switch($val['compGLCode']) {
				  case 700:
					  $filename = "PJ".$dtTime.".J01";
						if (!in_array($val['branch2'],$arrXtra))
							$BL = '005';
						else
							$BL = '006';
				  ...
				  break;	
				  case 101:
					  $filename = "PG".$dtTime.".J01";
					  $BL = '004';
				  break;	
				  case 1001:
					  $filename = "DC".$dtTime.".J01";
					   $BL = '007';
				  break;	
				  case 1002:
					  $filename = "DS".$dtTime.".J01";
					   $BL = '007';
				  break;	
			  }
			$ctr++;
	...
	function getXtraBranches() {
		$arr = array();
		$sqlXtra = "SELECT PG_PAYROLL.dbo.tblBranch.brnShortName FROM payroll_company..tblBLine INNER JOIN PG_PAYROLL.dbo.tblBranch ON PG_PAYROLL.dbo.tblBranch.glCodeStr = payroll_company..tblBLine.strCode WHERE (payroll_company..tblBLine.bline = 6)";
		$arrXtra = $this->getArrResI($this->execQryI($sqlXtra));
		foreach($arrXtra as $val) {
			$arr[] = $val['brnShortName'];
		}
		return $arr;
	}
	function TransferUploadedTS() {
		$sql = "  Insert into payroll_company..t_templ_hist (
					  empNo, [DATE], OT_RATE_CODE, INCOME_TYPE, ALLOW_TYPE, OTHER_LOC, DAYS_ABSENT, ABSENT_AMOUNT, REG_IN, REG_OUT, REG_HRSMIN, TARDY_HRS, 
                      TARDY_AMOUNT, UT_HRS, UT_AMOUNT, OT_IN, OT_OUT, OT_HOURS, OT_RATE, OT_AMOUNT, OT_EXCESS_HOURS, OT_EXCESS_AMOUNT, OT_ND, 
                      OT_ND_AMOUNT, OT_MEAL, OT_TRANS, INCOME_AMOUNT, ALLOWANCE_AMOUNT, REMARKS, IAS_VALIDATION, BATCH_#, DAYS_WORK, LUNCH_BREAK_OUT, 
                      LUNCH_BREAK_IN, BREAK_OUT, BREAK_IN, LEAVE_TYPE, SHIFT_CODE, AM_SHIFT_START, PM_SHIFT_END, PAYCODE, compCode
		)
					SELECT empNo, [DATE], OT_RATE_CODE, INCOME_TYPE, ALLOW_TYPE, OTHER_LOC, DAYS_ABSENT, ABSENT_AMOUNT, REG_IN, REG_OUT, REG_HRSMIN, TARDY_HRS, 
                      TARDY_AMOUNT, UT_HRS, UT_AMOUNT, OT_IN, OT_OUT, OT_HOURS, OT_RATE, OT_AMOUNT, OT_EXCESS_HOURS, OT_EXCESS_AMOUNT, OT_ND, 
                      OT_ND_AMOUNT, OT_MEAL, OT_TRANS, INCOME_AMOUNT, ALLOWANCE_AMOUNT, REMARKS, IAS_VALIDATION, BATCH_#, DAYS_WORK, LUNCH_BREAK_OUT, 
                      LUNCH_BREAK_IN, BREAK_OUT, BREAK_IN, LEAVE_TYPE, SHIFT_CODE, AM_SHIFT_START, PM_SHIFT_END, PAYCODE, compCode FROM payroll_company..t_templ
					  WHERE  empNo IN (SELECT empNo FROM tblEmpMast WHERE compCode = '".$this->session['company_code']."'
										AND empPayGrp = '".$this->session['pay_group']."'
										AND empPayCat = '".$this->session['pay_category']."'
										AND empStat IN ('RG','PR','CN'))
					  ";
		$sql .= "delete from payroll_company..t_templ WHERE empNo IN (SELECT empNo FROM tblEmpMast WHERE compCode = '".$this->session['company_code']."'
										AND empPayGrp = '".$this->session['pay_group']."'
										AND empPayCat = '".$this->session['pay_category']."'
									AND empStat IN ('RG','PR','CN'))";			  
