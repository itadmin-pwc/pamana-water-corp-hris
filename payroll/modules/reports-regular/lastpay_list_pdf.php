...
*/
			...
			if($this->arrPayPd['pdYear']!=$valData['pdYear'])
			{
				$taxableEarnings = $SSS = $MCR = $HDMF = 0;
			}
			else
			{
...
//					$this->Cell(23,5,number_format($this->getBasicValue($valData['empNo'],$valData['pdNumber'],$valData['pdYear']),2),0,0,'R');
			...
			$tincome=number_format(($totGrossCurYr + $empPrevEarnings)-$teuAmt,2);
			if($tincome<0)
				$tincome=0;
			$this->Cell(25,5,$tincome,0,1,'R');
			...
			$this->getEmpEearnings($arrempNo['empNo'],'Y');
	...
	function EmpBal($empNo) {
		...
		$ArrEmpBal = $this->getArrResI($this->execQryI($qryBal));
		$hdr=0;
	...
	function getEmpEearnings($empNo,$tax) {
		...
		$researnings = $this->execQryI($qryearnings);
		$ArrEarnings = $this->getArrResI($researnings);
		if ($tax=='Y') {
			...
			$this->Cell(140,5,number_format($GrosPay,2),0,1,'R');
			$this->SetFont('Arial','',8);
		} else {
	...
	function getEmpDeductions($empNo) {
		...
		$resdeductions = $this->execQryI($qrydeductions);
		$arrDed = $this->getArrResI($resdeductions);
	...
	function getEmpBasic() {
		...
		$qryearnings="Select empNo from tblPayrollSummary{$this->hist} where
								...
								AND compCode = '{$_SESSION['company_code']}'
									";
		$this->arrBasic = $this->getArrResI($this->execQryI($qryearnings));
	}
	function getGross($empNo,$pdNumber,$pdYear) {
		...
		$resGross = $this->getSqlAssocI($this->execQryI($qryGross));
	...
	function getData($empNo,$hist) {
		...
		$qryData="Select * from  tblPayrollSummary$hist where empNo='$empNo'   $payCat AND ((pdYear={$this->arrPayPd['pdYear']}) or (pdYear=({$this->arrPayPd['pdYear']}) AND pdNumber<22)) order by pdYear,pdNumber";
		return $this->getArrResI($this->execQryI($qryData));	
	}
	...
	function getGovData($hist) {
		...
		return $this->getArrResI($this->execQryI($qryGovData));	
	}
	function getPayPeriodDate() {
		...
		$this->arrPD = $this->getArrResI($this->execQryI($qryPayPeriod));
	}
	function getEmpGov() { 
		$sqlGov = "Select * from  tblGov_Tax_Added where compCode='{$_SESSION['company_code']}' and addStat='N'";
		$this->arrEmpGov = $this->getArrResI($this->execQryI($sqlGov));
	}
	function getPrevEmplr($empNo,$prevfield){
		 ...
		 $qrygetPrevEmplr = "Select ".$qryStat." from tblPrevEmployer where empNo='".$empNo."' and yearCd='".date('Y')."' and prevStat='A' \n";
...
$resEmpList = $psObj->execQryI($qryIntMaxRec);
$arrEmpList = $psObj->getArrResI($resEmpList);
$pdf->getEmpBasic();
