...
$reportPath = "allowance_list_sp_pdf.php?empNo=".$empNo."&empName=".$empName."&empDiv=".$empDiv."&empDept=".$empDept."&empSect=".$empSect."&orderBy=".$orderBy."&groupType=".$_SESSION["pay_group"]."&catType=".$_SESSION["pay_category"]."&payPd=".$payPd."&repType=".$_GET["repType"]."&tbl=".$tbl."&topType=".$topType."&reportType=".$reportType;
			...
			<TABLE border="0" width="100%" cellpadding="1" cellspacing="0" class="parentGrid">
				<tr>
					<td colspan="4" class="parentGridHdr"> &nbsp;<img src="../../../images/grid.png">
                    		&nbsp;PRINT ALLOWANCE REPORT :: <?php echo "GROUP - ".$_SESSION["pay_group"];?> :: <?=strtoupper($catName['payCatDesc']);?> :: PAYROLL PERIOD=<?=$inqTSObj->valDateArt($arrPayPd['pdPayable'])?> :: FROM=<?=$inqTSObj->valDateArt($arrPayPd['pdFrmDate'])?> :: TO=<?=$inqTSObj->valDateArt($arrPayPd['pdToDate'])?>
