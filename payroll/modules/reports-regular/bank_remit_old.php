...
*/
					...
					fclose($file);*/
					if ($_GET['totSalary'] == "") {
						header("Location: http://192.168.104.83/aub.php?compCode=$compCode&payGrp=$payGrp&pdYear=$pdYear&pdNumber=$pdNumber");
					} else	{
			...
			$compName=$bnkRmitObj->getCompany($_SESSION['company_code']);
				$bnkdata=$bnkRmitObj->getBankRemitDataMTC(1,MTC_BANK_CODE);
					...
					fclose($file);*/
						if ($_GET['totSalary'] == "") {
							echo 'wil';
							header("Location: http://192.168.104.83/mbtc.php?compCode=$compCode&payGrp=$payGrp&pdYear=$pdYear&pdNumber=$pdNumber");
						} else	{
...
/*		if(frm['cmbDiv'] != 0){
	...
	function genAUB(){
		...
		location.href='http://192.168.112.61/aub.php?<?='compCode='.$_SESSION['company_code'].'&payGrp='.$_SESSION['pay_group'].'&payCat='.$_SESSION['pay_category']?>&payPd='+$('payPd').value;
...
*/	}
	...
	function genBOC(){
			...
			location.href='http://192.168.104.83/boc.php?<?='compCode='.$_SESSION['company_code'].'&payGrp='.$_SESSION['pay_group'].'&payCat='.$_SESSION['pay_category']?>&payPd='+$('payPd').value;
	/*		new Ajax.Request(url,{
				method : 'get',
				parameters : $('frmBnkRmit').serialize(),
				onComplete : function (req){
	...
	*/	}
	...
	function genBDO()
	{
			...
			location.href='http://192.168.112.49/bdo.php?<?='compCode='.$_SESSION['company_code'].'&payGrp='.$_SESSION['pay_group'].'&payCat='.$_SESSION['pay_category']?>&payPd='+$('payPd').value;
			//location.href='http://192.168.110.38/PG-HRIS-SYSTEM/payroll/modules/reports-regular/bdo.php?<?='compCode='.$_SESSION['company_code'].'&payGrp='.$_SESSION['pay_group'].'&payCat='.$_SESSION['pay_category']?>&payPd='+$('payPd').value;
	...
	function txtreport(act) {
		...
		if(frm['payPd'] == 0){
			...
			<? if($_SESSION['company_code']<>'2') { ?>
				location.href='http://192.168.112.49/mbtc.php?<?='compCode='.$_SESSION['company_code'].'&payGrp='.$_SESSION['pay_group'].'&payCat='.$_SESSION['pay_category']?>&payPd='+$('payPd').value;
			<? }else{ ?>
				location.href='http://192.168.112.61/mbtc.php?<?='compCode='.$_SESSION['company_code'].'&payGrp='.$_SESSION['pay_group'].'&payCat='.$_SESSION['pay_category']?>&payPd='+$('payPd').value;
