...
class bankRemitObj extends commonObj {
	...
	function getBankRemitData($act="") {
			//echo "AND payGrp = '{$_SESSION['pay_group']}' AND payCat = '{$_SESSION['pay_category']}' AND pdPayable = '".date('Y-m-d',strtotime($this->get['payPd']))."'\n\n";
	$payPdSlctd = $this->getPayPeriod($_SESSION['company_code'],"AND payGrp = '{$_SESSION['pay_group']}' AND payCat = '{$_SESSION['pay_category']}' AND pdPayable = '".date('Y-m-d',strtotime($this->get['payPd']))."'");		
