...
class PDF extends FPDF
{
	...
	function getYTD($empNo) {
		$qryYTD="SELECT * from tblYtdData{$this->hist} where empNo='$empNo' and compCode='" . $_SESSION['company_code'] . "'";
		$resYTD = $this->execQry($qryYTD);
		return $this->getSqlAssoc($resYTD);	
	}
...
$qryIntMaxRec = "SELECT * FROM tblEmpMast 
				 ...
				 AND empNo IN (Select empNo FROM tblEarnings{$pdf->hist} where compCode='" . $_SESSION['company_code'] . "' and pdYear='" . $arrPayPd['pdYear']. "' and pdNumber='" . $arrPayPd['pdNumber'] . "' )
...
$pdf->Output('payslip_for_lastpay.pdf','D');
