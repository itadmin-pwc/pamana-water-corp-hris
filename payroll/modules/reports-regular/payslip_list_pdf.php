...
class PDF extends FPDF
{
	...
	function BasicTable($resEmpList)
	{
		...
		foreach ($resEmpList as $ArrEmpList) {
			...
			if ($PayPeriod['pdNumber']==25) {
				$date = date('12/1/'.(date('Y')-1). '- 11/30/Y');
			} else {
				/*if ($_SESSION['pay_group'] == 2) {
					if ($cutOff==2)
						$date = date('m/15/Y - m/',strtotime($PayPeriod['pdPayable'])).date("d/", strtotime('-1 second', strtotime('+1 month', strtotime(date("Y-m-01",strtotime($PayPeriod['pdPayable'])))))).date('Y',strtotime($PayPeriod['pdPayable']));     
					else
						$date = date('m/1/Y - m/15/Y',strtotime($PayPeriod['pdPayable']));
				} else {
					if ($cutOff==2) {
						$date = date('m/11/Y - m/25/Y',strtotime($PayPeriod['pdPayable']));
					} else { 				
						$date = date('m/26/Y',strtotime($PayPeriod['pdFrmDate']))."-".date('m/10/Y',strtotime($PayPeriod['pdToDate']));
					}*/
					$date = date('m/d/Y',strtotime($PayPeriod['pdPayable']));
				}
			//}			
			...
			$this->SetFont('Arial',''); 
			$this->Cell(23,6, 'Php ' .number_format($ArrEmpList['empMrate'],2),'TR',0);
			$this->Ln();
			
			$this->SetFont('Arial','B');
			$this->Cell(22,6,'PAY PERIOD: ','LTB','','L');
			$this->SetFont('Arial','');
			$this->Cell(28,6,$date,'TRB','','L');
			$this->SetFont('Arial','B','7'); 
			$this->Cell(37,6,'FOR THE CUT-OFF PERIOD:','LTB','','L');
			$this->SetFont('Arial',''); 
			$this->Cell(38,6,$cutOffDate,'TRB','','L');
			$this->SetFont('Arial','B'); 
			$this->Cell(17,6,'TAX STATUS:','TB');
			$this->SetFont('Arial',''); 
			$this->Cell(8,6,$ArrEmpList['empTeu'],'TRB');
			$this->SetFont('Arial','B'); 
			$this->Cell(13,6,'NET PAY:','LTB',0);
			$this->SetFont('Arial',''); 
			$this->Cell(27,6,'Php ' . number_format($NetPay,2),'TRB',0);
			$this->Ln();
			$this->SetFont('Arial','B'); 
	...
	function EarningsAndDeductions($empNo, $payPd, $act="", $oddEven) {
		...
		if ($_GET['tbl']==2 && $act=="") {
			$ArrempDeductions=$this->getEmpDeductions($empNo,$payPd,"'0',NULL");
		} else {
			$ArrempDeductions=$this->getEmpDeductions($empNo,$payPd,"'1'");
		}
		$totearnings = 0;
		$totdeductions = 0;
		$sss = 0;
		$sssDesc = 'SSS Contribution';
		for ($i=0;$i<16;$i++) {
			$empearn="";
	...
	function getEmpDeductions($empNo,$payPd,$tag="") {
	...
	function getDept($compCode,$empDiv,$empDept){
