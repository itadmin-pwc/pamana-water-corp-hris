...
/*		if(frm['cmbDiv'] != 0){
	...
	function genAUB(){
		...
		location.href='http://192.168.104.83/aub.php?<?='compCode='.$_SESSION['company_code'].'&payGrp='.$_SESSION['pay_group'].'&payCat='.$_SESSION['pay_category']?>&payPd='+$('payPd').value;
	...
	*/	}
	...
	function txtreport(act) {
		...
		if(frm['payPd'] == 0){
			...
			location.href='http://192.168.104.83/mbtc.php?<?='compCode='.$_SESSION['company_code'].'&payGrp='.$_SESSION['pay_group'].'&payCat='.$_SESSION['pay_category']?>&payPd='+$('payPd').value;
