...
foreach ($arrLoanList as $val){
	...
	if (!empty($val['lonGranted'])) {
		$lastpay =  date('m/d/Y',strtotime($val['lonGranted']));
		$lastpay =  ($lastpay == '01/01/1970') ? "": $lastpay;
	}
	if ($val['empNo'] != $empNo2) {
...
function HEADER_FOOTER($pdf, $maintEmpLoanObj, $compCode, $TOTAL_WIDTH, $dt) {
	...
	$refNo    		= ""; 
	$dtlLabelDown   = "Employee                 Loan Type     Ref. No.    Loan Amt   Loan Amt     Loan Start    Schedule    Ded. Per     Total    Cur. Bal.     Date     Setup Date";
	$dtlLabelDown2   = "                        	                                   (w/Interest)                              Sched.    Payments               Granted   ";
	#########################################################################
