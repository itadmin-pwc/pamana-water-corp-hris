...
################### INCLUDE FILE #################
	...
	class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->Cell(28.33,6,'HEAD COUNT','1','','R');
			$this->Cell(28.33,6,'BASIC','1','','R');
			$this->Cell(28.33,6,'ABSENT','1','','R');
			$this->Cell(28.33,6,'TARDY / UT','1','','R');
			$this->Cell(28.33,6,'OT / ND','1','','R');
			$this->Cell(28.33,6,'ADJ. BASIC','1','','R');
			$this->Cell(28.33,6,'AD. OT','1','','R');
			$this->Cell(28.33,6,'ALLOWANCE','1','','R');
			$this->Cell(28.33,6,'TOTAL','1','','R');
			$this->Ln();
		}
		...
		function displayContent($resQry,$divCode, $deptCode)
		{
			...
			$noOfBrnchDisp = 1;
			$grandTotalHeadCount = $grandTotalBasic = $grandTotalAbsent = $grandTotaltardUt = $grandTotalOtNd = $total=  $grandTotal = $grandTotalAllow = $grandTotalOthIncomeOt = 0;
			/*Display Per Branch*/
			$this->mysqliconnect();						
			foreach($arrBrnch as $arrBrnch_val)
			{
				...
				foreach($arrDiv as $arrDiv_val)
				{
					...
					foreach($arrDept as $arrDept_val)
					{
						...
						$brnCodes = $arrBrnch_val["brnCode"];
						$divCodes = $arrDiv_val["divCode"];
						$deptCodes = $arrDept_val["deptCode"];
						
					$arrgetHCount = $this->getHCountDetail($arrBrnch_val["brnCode"], $arrDiv_val["divCode"], $arrDept_val["deptCode"]);
					
					$qrytblEarnB = "call sp_PayrollSummary('{$_SESSION['company_code']}','{$this->fromDate}','{$this->toDate}','{$_SESSION['pay_group']}','{$brnCodes}','{$divCodes}','{$deptCodes}','','0100');";
					$qrytblEarnB = $this->execQryI($qrytblEarnB);
					$arrgetBasic=$this->getSqlAssocI($qrytblEarnB);
					$this->next_result($conn);
					$qrytblEarnA = "call sp_PayrollSummary('{$_SESSION['company_code']}','{$this->fromDate}','{$this->toDate}','{$_SESSION['pay_group']}','{$brnCodes}','{$divCodes}','{$deptCodes}','','0113');";
					$qrytblEarnA = $this->execQryI($qrytblEarnA);
					$arrgetAbsent=$this->getSqlAssocI($qrytblEarnA);	
					$this->next_result($conn);

					$qrytblEarnT = "call sp_PayrollSummary('{$_SESSION['company_code']}','{$this->fromDate}','{$this->toDate}','{$_SESSION['pay_group']}','{$brnCodes}','{$divCodes}','{$deptCodes}','','0111')";
					$qrytblEarnT = $this->execQryI($qrytblEarnT);
					$arrgetTard=$this->getSqlAssocI($qrytblEarnT);
					$this->next_result($conn);
					
					$qrytblEarnUT = "call sp_PayrollSummary('{$_SESSION['company_code']}','{$this->fromDate}','{$this->toDate}','{$_SESSION['pay_group']}','{$brnCodes}','{$divCodes}','{$deptCodes}','','0112')";
					$qrytblEarnUT = $this->execQryI($qrytblEarnUT);
					$arrgetUt=$this->getSqlAssocI($qrytblEarnUT);
					$this->next_result($conn);
					
							$sumTardandUt = $arrgetTard["sumtrnAmountE"] + $arrgetUt["sumtrnAmountE"];
							
					$qrytblEarnOT = "call sp_PayrollSummary('{$_SESSION['company_code']}','{$this->fromDate}','{$this->toDate}','{$_SESSION['pay_group']}','{$brnCodes}','{$divCodes}','{$deptCodes}','1','0200')";
					$arrgetOt=$this->getSqlAssocI($this->execQryI($qrytblEarnOT));
					$this->next_result($conn);
						
					$qrytblEarnND = "call sp_PayrollSummary('{$_SESSION['company_code']}','{$this->fromDate}','{$this->toDate}','{$_SESSION['pay_group']}','{$brnCodes}','{$divCodes}','{$deptCodes}','1','0300')";
					$NDi=$this->execQryI($qrytblEarnND);
					$arrgetNd=$this->getSqlAssocI($NDi);
					$this->next_result($conn);
					
					$sumOtandNd = $arrgetOt["sumtrnAmountE"] + $arrgetNd["sumtrnAmountE"];
							
					$qrytblEarnIN = "call sp_PayrollSummary('{$_SESSION['company_code']}','{$this->fromDate}','{$this->toDate}','{$_SESSION['pay_group']}','{$brnCodes}','{$divCodes}','{$deptCodes}','','0801')";
					$IN=$this->execQryI($qrytblEarnIN);
					$arrgetOthIncome=$this->getSqlAssocI($IN);
					$this->next_result($conn);
							
					$qrytblEarnOTIN = "call sp_PayrollSummary('{$_SESSION['company_code']}','{$this->fromDate}','{$this->toDate}','{$_SESSION['pay_group']}','{$brnCodes}','{$divCodes}','{$deptCodes}','','0802')";
					$OTIN=$this->execQryI($qrytblEarnOTIN);
					$arrgetOthIncome_OT=$this->getSqlAssocI($OTIN);
					$this->next_result($conn);
					
					$qrytblEarnALL = "call sp_PayrollSummary('{$_SESSION['company_code']}','{$this->fromDate}','{$this->toDate}','{$_SESSION['pay_group']}','{$brnCodes}','{$divCodes}','{$deptCodes}','3','')";
					$ALL=$this->execQryI($qrytblEarnALL);
					$arrgetAllow=$this->getSqlAssocI($ALL);
					$this->next_result($conn);
/*						$arrgetBasic = $this->getEarningsDetail($arrBrnch_val["brnCode"], $arrDiv_val["divCode"], $arrDept_val["deptCode"], EARNINGS_BASIC, 'sum(trnAmountE) as sumtrnAmountE', '');						
						...
						$sumOtandNd = $arrgetOt["sumtrnAmountE"] + $arrgetNd["sumtrnAmountE"];					
						$arrgetOthIncome = $this->getEarningsDetail($arrBrnch_val["brnCode"], $arrDiv_val["divCode"], $arrDept_val["deptCode"], ADJ_BASIC, 'sum(trnAmountE) as sumtrnAmountE', '');
						$arrgetOthIncome_OT = $this->getEarningsDetail($arrBrnch_val["brnCode"], $arrDiv_val["divCode"], $arrDept_val["deptCode"], ADJ_OT, 'sum(trnAmountE) as sumtrnAmountE', '');					
...
*/						
						...
						$this->Cell(28.33,6,$arrgetHCount["cntEmp"],'1','','R');
						$this->Cell(28.33,6,number_format($arrgetBasic["sumtrnAmountE"],2),'1','','R');
						$this->Cell(28.33,6,number_format($arrgetAbsent["sumtrnAmountE"],2),'1','','R');
						$this->Cell(28.33,6,number_format($sumTardandUt,2),'1','','R');
						$this->Cell(28.33,6,number_format($sumOtandNd,2),'1','','R');
						$this->Cell(28.33,6,number_format($arrgetOthIncome["sumtrnAmountE"],2),'1','','R');
						$this->Cell(28.33,6,number_format($arrgetOthIncome_OT["sumtrnAmountE"],2),'1','','R');
						$this->Cell(28.33,6,number_format($arrgetAllow["sumtrnAmountE"],2),'1','','R');
						$this->Cell(28.33,6,number_format($total,2),'1','0','R');
						$this->Ln();
						...
						$grandTotalOthIncomeOt_Branch+=$arrgetOthIncome_Branch["sumtrnAmountE"];
					...
					$this->Cell(28.33,6,$grandTotalHeadCount_Dept,'1','','R');
					$this->Cell(28.33,6,number_format($grandTotalBasic_Dept,2),'1','','R');
					$this->Cell(28.33,6,number_format($grandTotalAbsent_Dept,2),'1','','R');
					$this->Cell(28.33,6,number_format($grandTotaltardUt_Dept,2),'1','','R');
					$this->Cell(28.33,6,number_format($grandTotalOtNd_Dept,2),'1','','R');
					$this->Cell(28.33,6,number_format($grandTotalOthIncome_Dept,2),'1','','R');
					$this->Cell(28.33,6,number_format($grandTotalOthIncomeOt_Dept,2),'1','','R');
					$this->Cell(28.33,6,number_format($grandTotalAllow_Dept,2),'1','','R');
					$this->Cell(28.33,6,number_format($grandTotal_Dept,2),'1','','R');
					$this->Ln();
				...
				$this->Cell(28.33,6,$grandTotalHeadCount_Div,'1','','R');
				$this->Cell(28.33,6,number_format($grandTotalBasic_Div,2),'1','','R');
				$this->Cell(28.33,6,number_format($grandTotalAbsent_Div,2),'1','','R');
				$this->Cell(28.33,6,number_format($grandTotaltardUt_Div,2),'1','','R');
				$this->Cell(28.33,6,number_format($grandTotalOtNd_Div,2),'1','','R');
				$this->Cell(28.33,6,number_format($grandTotalOthIncome_Div,2),'1','','R');
				$this->Cell(28.33,6,number_format($grandTotalOthIncomeOt_Div,2),'1','','R');
				$this->Cell(28.33,6,number_format(grandTotalAllow_Div,2),'1','','R');
				$this->Cell(28.33,6,number_format($grandTotal_Div,2),'1','','R');
				$this->Ln();
				/*End Display Division*/
				
				$this->Cell(80,6,'BRANCH TOTAL','1','','L');
				$this->Cell(28.33,6,$grandTotalHeadCount_Branch,'1','','R');
				$this->Cell(28.33,6,number_format($grandTotalBasic_Branch,2),'1','','R');
				$this->Cell(28.33,6,number_format($grandTotalAbsent_Branch,2),'1','','R');
				$this->Cell(28.33,6,number_format($grandTotaltardUt_Branch,2),'1','','R');
				$this->Cell(28.33,6,number_format($grandTotalOtNd_Branch,2),'1','','R');
				$this->Cell(28.33,6,number_format($grandTotalOthIncome_Branch,2),'1','','R');
				$this->Cell(28.33,6,number_format($grandTotalOthIncomeOt_Branch,2),'1','','R');
				$this->Cell(28.33,6,number_format(grandTotalAllow_Branch,2),'1','','R');
				$this->Cell(28.33,6,number_format($grandTotal_Branch,2),'1','','R');
				$this->Ln();
			...
			$this->Cell(28.33,6,$grandTotalHeadCount,'1','','R');
			$this->Cell(28.33,6,number_format($grandTotalBasic,2),'1','','R');
			$this->Cell(28.33,6,number_format($grandTotalAbsent,2),'1','','R');
			$this->Cell(28.33,6,number_format($grandTotaltardUt,2),'1','','R');
			$this->Cell(28.33,6,number_format($grandTotalOtNd,2),'1','','R');
			$this->Cell(28.33,6,number_format($grandTotalOthIncome,2),'1','','R');
			$this->Cell(28.33,6,number_format($grandTotalOthIncomeOt,2),'1','','R');
			$this->Cell(28.33,6,number_format($grandTotalAllow,2),'1','','R');
			$this->Cell(28.33,6,number_format($grandTotal,2),'1','','R');
			$this->Ln();
	...
	$pdf->fromDate		= 	date('Y-m-d',strtotime($_GET["fromDate"]));
	$pdf->toDate		=	date('Y-m-d',strtotime($_GET["toDate"]));
	...
	$pdf->qrypdNum = "SELECT    pdNumber
					FROM       tblPayPeriod
					WHERE     (pdPayable BETWEEN '".date("Y-m-d", strtotime($fromDate))."' AND '".date("Y-m-d", strtotime($toDate))."') AND (payGrp = '".$_SESSION["pay_group"]."')";
	...
	$qryPayReg 	= "Select 
				...
				$pdf->where_empmast
				order by empLastName Limit 0,1;
				"; 
