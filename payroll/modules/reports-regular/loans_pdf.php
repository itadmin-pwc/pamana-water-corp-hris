...
############################ Q U E R Y ##################################
	...
	$lonTypeFilter = ($lonType !="") ? " AND tblEmpLoansDtl$hist.lonTypeCd='$lonType'":"";
		 $sqlLoans = "SELECT     tblLoanType.lonTypeShortDesc, tblEmpLoansDtl$hist.*, tblEmpMast.empLastName, tblEmpMast.empFirstName, tblEmpMast.empMidName, 
						...
						ORDER BY tblEmpLoansDtl$hist.lonTypeCd,tblEmpMast.empBrnCode, tblEmpMast.empLastName, tblEmpMast.empFirstName, tblEmpMast.empMidName,tblEmpLoansDtl$hist.lonRefNo	";
...
/*		if ($_GET['lonType'] != 302) {
		...
		if ($brnCode != $valLoans['empBrnCode']) {
			if ($brnCode !="") {
				$pdf->SetFont('Courier', 'B', '9');
				$pdf->Cell(130,$SPACES,"TOTAL",0,0,'R');
				$pdf->Cell(32,$SPACES,number_format($totLoanAmt,2),0,0,'R');
				$pdf->Cell(31,$SPACES,number_format($totCurBal,2),0,0,'R');
				$pdf->Cell(63,$SPACES,number_format($tot,2),0,1,'R');
				$pdf->SetFont('Courier', '', '9');
				$tot = 0;	
				$totLoanAmt = 0;
				$totCurBal = 0;
		...
		$nameInit = $valLoans['empLastName'] . ", " . $valLoans['empFirstName']." ".$valLoans['empMidName'][0].".";
			...
			$pdf->Cell(35,$SPACES,$valLoans['lonRefNo'],0,0);
...
function HEADER_FOOTER($pdf, $inqTSObj, $compCode, $TOTAL_WIDTH, $dt) {
	...
	$reppages 		= "";
	$repId    		= "Report ID: EMPLOANSDED";
	$repTitle 		= "LOANS DEDUCTED $branch";
	$refNo    		= ""; 
	$dtlLabelDown   = "  #  Emp. No.       Employee         Loan Type     Ref. No.               Loan Amount       Loan Bal.      Ded. Schedule     Actual Ded.";
	$dtlLabelDown2   = "";
	#########################################################################
	...
	$pdf->Text(155,15,"Payroll Date: ".date("m/d/Y",strtotime($dt)));
