	...
	class PDF extends FPDF
	{
		function Header()
		{
			...
			if($this->fPrint==0)
			{
				...
				if($this->pdNum=='25')
				{
					$this->Cell(80,5,"Report ID: 13THMNTHPAYREG01");
					$hTitle = $hTitle." 13Th Month Register for the Year ".$this->pdHeadTitle;
				}
				else
				{
			...
			else
			{
				...
				if($this->pdNum=='25')
				{
					$this->Cell(80,5,"Report ID: 13THMNTHPAYREG02", 0);
					$hTitle = $hTitle." 13Th Month Register for the Year ".$this->pdHeadTitle;
				}
				else
				{
				...
				$this->Cell(57,6,'ALLOW. / DED. TYPE',0,'','L');
		...
		function getDataLoanstblDeductions($trnRec,$pdYear,$pdNumber,$reportType)
		{
			...
			$qryLoansDeductions = "Select sum(trnAmountD) as totAmountD,empNo from ".$tbltable." 
							...
							and sprtPS='0' and pdYear='".$pdYear."' and pdNumber='".$pdNumber."'
							group by empNo
							";
		...
		function getDataOthAdjtblDeductions($trnRec,$pdYear,$pdNumber,$reportType)
		{
			...
			$qryOthAdjDeductions = "Select sum(trnAmountD) as totAmountD,empNo from ".$tbltable." 
							where trnCode NOT IN (8024,8124,5201) and trnCode in (Select trnCode from tblPayTransType 
							...
							and pdYear='".$pdYear."' and pdNumber='".$pdNumber."' and sprtPS='0'
							group by empNo
							";
		...
		function getWTaxAndTaxAdj($pdYear,$pdNumber,$reportType)
		{
			...
			$qryWTaxAndTaxAdj = "Select trnAmountD,trnCode,empNo from ".$tbltable." 
							where trnCode IN ('8024','5100',8124) 
							and pdYear='".$pdYear."' and pdNumber='".$pdNumber."' 
							";
		...
		function displayEarnContent()
		{
			...
			if ($_SESSION['pay_category'] !=9) {
				$where_trnCode = "trnCode in 
								...
								and pdNumber='".$this->pdNum."' and empNo in 
									(
									Select empNo from ".$this->tblPaySum." where
									pdYear='".$this->pdYear."' 
			...
			$qryDistinctEarnType = "Select trnCode, trnDesc 
									...
									order by trnDesc";
			//echo $qryDistinctEarnType;
			$resDisEarnType = $this->getArrRes($this->execQry($qryDistinctEarnType));
			return $resDisEarnType;
			
		}
		
		function displayContentDetails($resqryEarn, $arrqryDed)
		{
				$this->SetFont('Arial','','8'); 
				$arrEarnType=$this->displayEarnContent();
				$grandsumamt = $grand_earnSum = $grand_dedSum = 0;
				
				$arrqryEarn = $resqryEarn;
				foreach($resqryEarn as $empValue)
				{
					
					if($empValue['empNo']!=$empNoSp)
					{
						$earndedCnt = $earnSum = $dedSum = 0;
						$this->Cell(25,6,$empValue['empNo'],0);
						$this->Cell(57,6,$empValue['empLastName'] . ", ". $empValue['empFirstName'][0].".".$empValue['empMidName'][0].".",0);
						
						//ALLOWANCE
						foreach($arrqryEarn as $arrqryEarn_val)
						{
							if($empValue['empNo']==$arrqryEarn_val['empNo'])
							{
								if($earn_empNo!=$arrqryEarn_val['empNo'])
								{
									$this->Cell(57,6,$arrqryEarn_val["trnDesc"],0,'','L');
									$this->Cell(25,6,number_format($arrqryEarn_val["trnAmountE"],2),0,'1','R');
								}
								else
								{
									$this->Cell(25,6,'',0);
								...
								$earndedCnt++;
								$earnSum+=$arrqryEarn_val["trnAmountE"];
							}
							$earn_empNo = $arrqryEarn_val['empNo'];
							
						}
						
						//DEDUCTIONS
						foreach($arrqryDed as $arrqryDed_val)
						{
							if($empValue['empNo']==$arrqryDed_val['empNo'])
							{
									$this->Cell(25,6,'',0);
									$this->Cell(57,6,'',0);
									$this->Cell(57,6,$arrqryDed_val["trnDesc"],0,'','L');
									$this->Cell(25,6,number_format(($arrqryDed_val["trnAmountD"]*-1),2),0,'1','R');
									$earndedCnt++;
									$dedSum+=$arrqryDed_val["trnAmountD"];
							}
							
						}
						
						$diff_EarnDed = $earnSum - $dedSum;
						$grand_earnSum+=$earnSum;
						$grand_dedSum+=$dedSum;
						if($earndedCnt>=2)
						{
							$this->SetFont('Arial','B','8');
								$this->Cell(25,6,'',0);
								$this->Cell(57,6,'',0);
								$this->Cell(57,6,'SUB - TOTAL',0,'','L');
								$this->Cell(25,6,number_format($diff_EarnDed,2),0,'1','R');
								$grandsumamt+=$diff_EarnDed;
							$this->SetFont('Arial','','8'); 
						}
						else
						{
					...
					/*
					$this->Cell(57,6,$empValue["trnDesc"],0,'','L');
					...
					foreach($arrqryDed as $arrqryDed_val)
					{
						if($empValue['empNo'] == $arrqryDed_val["empNo"])
						{
							...
							$this->Cell(57,6,'',0);
							$this->Cell(57,6,$arrqryDed_val["trnDesc"],0,'','L');
							$this->Cell(25,6,number_format($arrqryDed_val["trnAmountD"],2),0,'','R');
						}
					}*/
					
					//$this->Ln();
					$subTotal+=$empValue["trnAmountE"];
					$empNoSp = $empValue['empNo'];
				}
				$this->Ln();
				$this->SetFont('Arial','B','8');
					$this->Cell(25,6,'',0);
					$this->Cell(57,6,'',0);
					$this->Cell(57,6,'GRAND - TOTAL (ALLOWANCE)',0,'','L');
					$this->Cell(25,6,number_format($grand_earnSum,2),0,'1','R');
					
					$this->Cell(25,6,'',0);
					$this->Cell(57,6,'',0);
					$this->Cell(57,6,'GRAND - TOTAL (DEDUCTIONS)',0,'','L');
					$this->Cell(25,6,number_format(($grand_dedSum*-1),2),0,'1','R');
					
					$this->Cell(25,6,'',0);
					$this->Cell(57,6,'',0);
					$this->Cell(57,6,'GRAND - TOTAL',0,'','L');
					$this->Cell(25,6,number_format($grandsumamt,2),0,'1','R');
		...
		function displayContent($resQry,$reportType,$pdYear,$pdNum,$divCode,$empDept,$empSec)
		{
			...
			$getempDeductionsSSSProveFund = $this->getDataOthtblDeductions(SSS_PROVEFUND,$pdYear,$pdNum,$reportType);
			...
			foreach($arrBrnch as $arrBrnch_val)
			{
				...
				foreach($arrLoc as $arrLoc_val)
				{
					...
					$this->SetFont('Arial','B','8'); 
					//$this->Cell(8,6,"",0,'','L');
					$this->Cell(8,6,"BRANCH = ".strtoupper($arrLoc_val["brnDesc"]),0,'','L');
					$this->Ln(5);
...
if($arrDiv_val["divCode"]==$resQryValue["empDivCode"])
										{
											...
											if($reportType=='0')
												$rowEmpYtdGross =  $rowEmpYtdGross + $resQryValue["YtdTaxable"];
											else
											...
											/*WITH TAX*/
											$rowWithTax = $rowWithTaxAdj = $rowYearEndTaxAdj = 0;
											foreach($getempWTaxAndTaxAdj as $valWTaxAndTaxAdj) {
												if ($valWTaxAndTaxAdj['empNo']==$resQryValue["empNo"]) {
													...
													if ($valWTaxAndTaxAdj['trnCode']==8124) {
														$rowYearEndTaxAdj= (float)$valWTaxAndTaxAdj['trnAmountD'];
													}														
												}
											}
											
											
											$grandempWithTax+=$rowWithTax + $rowWithTaxAdj + $rowYearEndTaxAdj;
											$subempWithTax+=$rowWithTax + $rowWithTaxAdj + $rowYearEndTaxAdj;
											$totDed =$rowWithTax + $rowWithTaxAdj + $rowYearEndTaxAdj;
											...
											/*SSS PROVE FUND*/
											foreach($getempDeductionsSSSProveFund as $getempDeductionsSSSProveFundValue)
											{
												if ($getempDeductionsSSSProveFundValue['empNo']==$resQryValue["empNo"]) {
														$rowDeductionsSSSProve=$getempDeductionsSSSProveFundValue["totAmountD"];
												}
											}
											...
											$totDed +=$rowDeductionsSSS + $rowDeductionsSSSProve;

											$rowDeductionsSSSProve = ($rowDeductionsSSSProve!=""?$rowDeductionsSSSProve:0);
											$grandempDeductionsSSS+=$rowDeductionsSSSProve;
											$subempDeductionsSSS+=$rowDeductionsSSSProve;
											// $totDed +=$rowDeductionsSSSProve;

											$this->Cell(24,6,number_format($rowDeductionsSSS+$rowDeductionsSSSProve,2),'','','R');
											...
											foreach ($getempYtdDataHist  as $getempYtdDataHistValue) {
											   if ($getempYtdDataHistValue['empNo']==$resQryValue["empNo"]) {
														...
														$rowEmpYtdGovDed=$getempYtdDataHistValue["YtdGovDed"]+ $resQryValue["YtdGovDedMinWage"];
												}
											}
											
											//echo $resQryValue["empNo"]."==".$resQryValue["empWageTag"]."==".$rowEmpYtdGovDed."<br>";
											
											if($resQryValue["empWageTag"]=='Y')
											{
												if($reportType=='0')
													$rowEmpYtdGovDed =  $rowEmpYtdGovDed + $resQryValue["YtdGovDedMinWage"];
												else
											...
											$this->Cell(24,6,'','','','R');
											$this->Cell(24,6,number_format($rowYearEndTaxAdj,2),'','','R');
											...
											$totBrnchWithTax+=$rowWithTax;
											$totBrnchAdjTax+=$rowWithTaxAdj;
											$totBrnchYETax+=$rowYearEndTaxAdj;
											
											$totBrnchSSS+=$rowDeductionsSSS+$rowDeductionsSSSProve;
											...
											$totLocWithTax+=$rowWithTax;
											$totLocAdjTax+=$rowWithTaxAdj;
											$totLocYETax+=$rowYearEndTaxAdj;
											$totLocSSS+=$rowDeductionsSSS +$rowDeductionsSSSProve;
											...
											$totDivAdjTax+=$rowWithTaxAdj;
											$totDivYETax+=$rowYearEndTaxAdj;
											$totDivSSS+=$rowDeductionsSSS +$rowDeductionsSSSProve;
											...
											$totGrandTotAdjTax+=$rowWithTaxAdj;
											$totGrandTotYETax+=$rowYearEndTaxAdj;
											
											$totGrandTotSSS+=$rowDeductionsSSS+$rowDeductionsSSSProve;
								...
								unset($testemp,
										...
										$rowEarningsAllow,$rowEarningsOth,$rowGrossTaxable,$rowGrossNonTaxable,$rowDeductionsSSS,$rowDeductionsSSSProve,$rowDeductionsHDMF,
						...
						$this->Cell(24,0,number_format($totDivAdjTax,2),'','','R');
						...
						$this->Cell(24,6,'','0','','R');
						$this->Cell(24,6,number_format($totDivYETax,2),'','','R');
				...
				/*	$this->SetFont('Arial','B','10'); 
					...
					$this->Cell(24,0,number_format($totLocAdjTax,2),'','','R');
					...
					$this->Cell(24,6,'','0','','R');
					$this->Cell(24,6,number_format($totLocYETax,2),'','','R');
				...
				$this->SetFont('Arial','B','8'); 
				$this->Cell(47,6,(strtoupper($arrLoc_val["brnDesc"]).' TOTAL '),'0','','L');
				...
				$this->Cell(24,0,number_format($totBrnchAdjTax,2),'','','R');
				...
				$this->Cell(24,6,'','0','','R');
				$this->Cell(24,6,number_format($totBrnchYETax,2),'','','R');
			...
			$this->Cell(24,0,number_format($totGrandTotAdjTax,2),'','','R');
			...
			$this->Cell(24,6,'','0','','R');
			$this->Cell(24,6,number_format($totGrandTotYETax,2),'','','R');
	...
	if($pdf->pdNum=='25')
		$pdf->pdHeadTitle	=	$pdf->pdYear." (Group ".$_SESSION[pay_group].", ".$catName['payCatDesc'].")";
	else
	...
	if ($empDiv>"" && $empDiv>0) {$empDiv2 = " AND (empDivCode= '{$empDiv}')"; } else {$empDiv2 = "";}
	...
	if ($empDiv>"" && $empDiv>0) {$empDivPaySum = " AND (empDiv = '{$empDiv}')"; } else {$empDivPaySum = "";}
	...
	if($RptRegular==1)
	{
		$pdf->fPrint = 0;
		 ...
		 $qryPayReg 	= "Select 
					...
					teuAmt, empWageTag,
					YtdGross,YtdTax, YtdTaxable, YtdGovDed, YtdGovDedMinWage
					from $tbl tblPaySum
	...
	if($RptAllowance==1)
	{
		...
		$qryEarn = "Select tblEarn.compCode, 
						...
						AND (sprtPS='Y')
						$where_trnCode
						$empNo2 $empName2 $empDivPaySum $empName2 $empDept2 $empSect2  $locType2
						order by empLastName, empFirstName, trnDesc";
		
		$qryDed_Sprt = "Select tblEarn.compCode, 
						tblEarn.trnCode,trnDesc,empmast.empNo, empLastName,
						empFirstName,empMidName,trnAmountD
						from $tblDed tblEarn, tblEmpMast empmast, tblPayTransType ptTrans 
						where
						tblEarn.empNo=empmast.empNo
						AND tblEarn.trnCode=ptTrans.trnCode
						AND tblEarn.compCode='".$_SESSION["company_code"]."'
						...
						AND (sprtPS='1')
						$where_trnCode
						$empNo2 $empName2 $empDivPaySum $empName2 $empDept2 $empSect2 $empBrnCode2 $locType2
						order by empLastName, empFirstName, trnDesc";
		
		$resEarn = $inqTSObj->execQry($qryEarn);
		$resDed = $inqTSObj->execQry($qryDed_Sprt);
		$arrSumEarn = $inqTSObj->getArrRes($resEarn);
		$arrSumDed = $inqTSObj->getArrRes($resDed);
		...
		if(count($arrSumEarn)>0)
		{
			...
			$pdf->printedby = $inqTSObj->getUserHeaderInfo($sessionVars['empNo'],$_SESSION['employee_id']); 
			$pdf->AddPage();
			$pdf->displayContentDetails($arrSumEarn,$arrSumDed);
		}
	}
