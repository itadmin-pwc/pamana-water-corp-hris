<?
	...
	$reportPath = "deductions_type_list_pdf.php?empNo=".$empNo."&empName=".$empName."&empDiv=".$empDiv."&empDept=".$empDept."&empSect=".$empSect."&groupType=".$_SESSION["pay_group"]."&catType=".$_SESSION["pay_category"]."&payPd=".$payPd."&repType=".$_GET["repType"]."&tbl=".$tbl."&topType=".$topType."&reportType=".$reportType."&empBrnCode=".$empBrnCode;
?>
...
<HTML>
	...
	<BODY>
			...
			<TABLE border="0" width="100%" cellpadding="1" cellspacing="0" class="parentGrid">
				<tr>
					<td colspan="4" class="parentGridHdr"> &nbsp;<img src="../../../images/grid.png">
                    	&nbsp;DEDUCTION SUMMARY BY DEDUCTION TYPE REPORT :: <?php echo "GROUP - ".$_SESSION["pay_group"];?> :: <?=strtoupper($catName['payCatDesc']);?> :: PAYROLL PERIOD=<?=$inqTSObj->valDateArt($arrPayPd['pdPayable'])?> :: FROM=<?=$inqTSObj->valDateArt($arrPayPd['pdFrmDate'])?> :: TO=<?=$inqTSObj->valDateArt($arrPayPd['pdToDate'])?>
						...
						<TABLE border="0" width="100%" cellpadding="1" cellspacing="1" class="childGrid" >
                            ...
                            <tr>
								<td colspan="20" align="center" class="childGridFooter">
									<? //$pager->_viewPagerButton("deductions_type_list_ajax.php","TSCont",$intOffset,$_GET['isSearch'],'txtSrch','cmbSrch','&empNo='.$empNo."&empName=".$empName."&empDiv=".$empDiv."&empDept=".$empDept."&empSect=".$empSect."&groupType=".$groupType."&orderBy=".$orderBy."&catType=".$catType."&payPd=".$payPd."&repType=$_GET[repType]&reportType=$reportType&topType=$topType");?>
								</td>
							</tr>
						</TABLE>
					</td>
		...
		<form name="frmEmpList" method="post">
          ...
          <input type="hidden" name="repType" id="repType" value="<?php echo $_GET["repType"]; ?>">
          <input type="hidden" name="reportType" id="reportType" value="<?php echo $_GET["reportType"]; ?>">
          <input type="hidden" name="topType" id="topType" value="<?php echo $_GET["topType"]; ?>">
		</form>
	</BODY>
</HTML>
