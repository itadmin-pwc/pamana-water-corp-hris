...
class maintEmpObj extends commonObj {
	...
	function validateStrCode($strCode) {
		$qryStrCode = "Select brnCode from tblBranch where glCodeStr = '$strCode'";	
		$resStrCode = $this->execQry($qryStrCode);
		return $this->getRecCount($resStrCode);
	}
	
	function createAccts($strCode) {
		$qryAccts = "Call sp_ChartofAccts($strCode)";	
		return $this->execQry($qryAccts);
	}
...
class employeeAllowanceObj extends commonObj {
	...
	function addEmpAllowance(){
		...
		$allowInfo = explode("-",$this->method['cmbAllowType']);
		...
		$qryToEmpAllowance = "INSERT INTO tblAllowance(
								...
								allowTag
							  	...
							  	'{$this->compCode}','{$this->method['empNo']}','{$allowInfo[0]}',
	...
	function editEmpAllowance(){
		...
		$qryEditEmpAllow = "UPDATE tblAllowance SET allowAmt    = '{$this->method['txtAllwAmount']}',
													...
													allowTag	 = '{$this->method['cmbAllwTag']}'
												WHERE compCode  = '{$this->compCode}' 
...
class changePassObj extends commonObj {
	...
	function checkOldPass(){
		$qryCheckOldPass = "SELECT userPass FROM tblUsers
							WHERE userId = '{$this->sessionVars['user_id']}'
	...
	function changePass(){
		$qryChangePass = " UPDATE pg_payroll..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";
		$qryChangePass .= " UPDATE dfclark_payroll..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";
		$qryChangePass .= " UPDATE dfsubic_payroll..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";
		$qryChangePass .= " UPDATE lusitano..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";
		$qryChangePass .= " UPDATE parco_gant_d3..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";
		$qryChangePass .= " UPDATE parco_gant_diamond..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";
		$qryChangePass .= " UPDATE parco_scv..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";
		$qryChangePass .= " UPDATE parco_super_agora..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";
		$qryChangePass .= " UPDATE parco_super_retail_vii..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";
		$qryChangePass .= " UPDATE parco_super_retail_xv..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";
		$qryChangePass .= " UPDATE pg_subic..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";
		$qryChangePass .= " UPDATE PGJR_PAYROLL..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";	
		$qryChangePass .= " UPDATE COMPANY_E_PAYROLL..tblUsers 
						  SET userPass = '".base64_encode($this->get['txtNewPass'])."'
						  WHERE userId = '{$this->sessionVars['user_id']}'
						  AND empNo = '{$this->sessionVars['employee_number']}'
						  AND userStat = 'A';\n";					  			  
