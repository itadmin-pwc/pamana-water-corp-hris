...
class branchObj extends commonObj {
	...
	function toTblBranch(){
		 $qry = "INSERT INTO tblBranch(compCode,brnCode,brnDesc,
									  ...
									  glCodeStr,brnLoc,coCtr,ecola,GrpCode)
								VALUES('{$_SESSION['company_code']}','{$this->get['brnCode']}','".str_replace("'","''",stripslashes(strtoupper($this->get['Desc'])))."',
									   '".str_replace("'","''",stripslashes(strtoupper($this->get['shrtDesc'])))."','".str_replace("'","''",stripslashes(strtoupper($this->get['add1'])))."','".str_replace("'","''",stripslashes(strtoupper($this->get['add2'])))."',
									   '".str_replace("'","''",stripslashes(strtoupper($this->get['add3'])))."','{$this->get['minWage']}','".str_replace("'","''",stripslashes(strtoupper($this->get['signatory'])))."',
									   '".str_replace("'","''",stripslashes(strtoupper($this->get['sgnTitle'])))."','{$this->get['brnGrp']}','{$this->get['brnStat']}',
									   '{$this->get['cmbGLStore']}','{$this->get['brnLoc']}','{$this->get['coCtr']}','{$this->get['ecolaAmnt']}','{$this->get['cmbGrp']}')";
	...
	function updateBranch(){
		$qry = "UPDATE tblBranch SET brnDesc = '".str_replace("'","''",stripslashes(strtoupper($this->get['Desc'])))."', 
									 ...
									 brnSignTitle = '".str_replace("'","''",stripslashes(strtoupper($this->get['sgnTitle'])))."',
									 brnDefGrp 	= '{$this->get['brnGrp']}',
									 brnStat 	= '{$this->get['brnStat']}',
									 glCode 	= '{$this->get['cmbGLStore']}',
