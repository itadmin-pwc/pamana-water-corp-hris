...
//		$newempcode=$this->createempno($this->compCode);
	...
	function viewprofile($empNo) {
		...
		, Day(empEndDate) as empEndDate_D , Month(empEndDate) as empEndDate_M, Year(empEndDate) as empEndDate_Y,empstat as status,empEndDate,dateResigned from tblEmpMast where empNo='$empNo' and compCode='{$this->oldcompCode}'";
		$res=$this->execQry($qryviewprofile);
		$res=$this->getArrRes($res);
		foreach ($res as $profile) {
			//General Tab	
			$this->empWageTag=$profile['empWageTag'];
...
//			$this->strprofile=$profile[''];
			...
			$this->old_empAcctType = $profile['empBankCd'];
			$this->old_chAcctNo = $profile['empAcctNo'];
			...
			$this->Status=$profile['employmentTag'];
...
//			$this->Release=$profile[''];
	...
	function updateemployee($empNox) {
		...
		$qryupdateemp="Update tblEmpMast set $idfields where empNo='$empNox'  and compCode='{$this->oldcompCode}';";
		$qryupdateemp.= "Insert into tblNonPaf_Hist(compCode,empNo,emp_oldempBankCd,emp_oldempAcctNo,emp_newempBankCd,emp_newempAcctNo,user_updated,date_updated) 
							values('".$_SESSION["company_code"]."', '".$this->strprofile."', '".$this->old_empAcctType."', '".$this->old_chAcctNo."',
									'{$this->bank}','".str_replace("'","''",stripslashes(strtoupper($this->bankAcctNo)))."', '".$_SESSION['employee_number']."', '".date("m/d/Y g:i:s A",strtotime("+8 hours"))."');";	
