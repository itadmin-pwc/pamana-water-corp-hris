...
/*		if ($this->get['prtag'] == "1") {
	...
	function others() {
		...
		if ($this->get['txtBioNum'] !="" && $this->get['txtBioNum'] != $this->get['old_txtBioNum']) {
					...
					case "others":
						...
						foreach ($arrproc as $val) {
							...
							if ($val['old_bioNumber'] !="") {
								$empfield_bio = " bioNumber='{$val['new_bioNumber']}', updatedBy='".$this->session['user_id']."', dateUpdated='".date("m/d/Y")."'";
							...
							{
								$arrChckBioEmp = $this->checkBioEmpExist(" and empNo='".$val['empNo']."'");
								if($arrChckBioEmp["empNo"]!="")
									$qryempBio.="Update tblBioEmp set $empfield_bio where empNo='{$val['empNo']}' and compCode='{$this->get['compCode']}';";
								else
									$qryempBio.="Insert into tblBioEmp(compCode, locCode, bioNumber, empNo, bioStat, addedBy, dateAdded)values('".$this->get['compCode']."','".$this->get['old_biobranchCode']."', '".$val['new_bioNumber']."', '".$val['empNo']."', 'A', '".$this->session['user_id']."', '".date("m/d/Y")."');";
							}
						}	
						 ...
						 $qryprocess = "	Insert into tblPAF_Othershist 
										...
										old_empAddr2, old_empCityCd, old_empTin,old_empSssNo, old_empPhicNo, old_empPagibig, old_bioNumber,
										new_empLastName, new_empFirstName, new_empMidName, new_empAddr1, new_empAddr2,
										new_empCityCd, new_empTin, new_empSssNo, new_empPhicNo, new_empPagibig, new_bioNumber, stat, 
