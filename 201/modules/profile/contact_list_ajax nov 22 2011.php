...
$qryDedList = "SELECT TOP $intLimit *
		FROM tblContactMast INNER JOIN tblContactTypeRef ON tblContactMast.contactCd = tblContactTypeRef.contactCd
		WHERE recNo NOT IN
        (SELECT TOP $intOffset recNo FROM tblContactMast INNER JOIN tblContactTypeRef ON tblContactMast.contactCd = tblContactTypeRef.contactCd WHERE empNo='{$_SESSION['strprofile']}' 
			     $recNo $statusType $compCode 
				 ORDER BY recNo ) 
				AND empNo = '{$_SESSION['strprofile']}'
				$compCode
			    ORDER BY recNo ";
					...
					<td>
						<TABLE border="0" width="100%" cellpadding="1" cellspacing="1" class="tblPrevEmp" >
							<tr>
								...
								<td width="30%" class="gridDtlLbl" align="center">Contact</td>
                               <?
                                if ($_SESSION['profile_act']!="View") {
								?>
							...
							<tr class="rowDtlEmplyrLst">
								...
								<td class="gridDtlVal" align="center"> 
									<span class="gridDtlVal"><a href="#" onClick="viewDetails('contact_act.php','Edit','<?=$dedListVal['recNo']?>','contact_list_ajax.php','TSCont',<?=$intOffset?>,<?=$_GET['isSearch']?>,'txtSrch','cmbSrch')"><img src="../../../images/application_get.png" border="0" class="actionImg" title="View Contact" /></a> <img onClick="deleContact('<?=$dedListVal['recNo']?>','contact_list_ajax.php','TSCont',<?=$intOffset?>,<?=$_GET['isSearch']?>,'txtSrch','cmbSrch','<?=htmlspecialchars(addslashes($dedListVal['contactName']));?>')"
												src="../../../images/prev_emp_dele.png" width="15" height="15" title="Delete Contact" /> </span></td>
                                    <? } ?>            
						  </tr>
							<?
								}
