...
//		$newempcode=$this->createempno($this->compCode);
		...
		echo $qryAddEmployee = "INSERT INTO tblEmpMast (
							$genfields
							$confields
							$perfields
							$idfields
...
//			$this->Release=$profile[''];
	...
	$value=str_replace("_"," ",stripslashes($value));
		if ($field != "empNo") {
			...
			if ($type=="1") {
				$sqlcheck="Select * from tblEmpMast where $field='$value' and $field<>''";
			}
			elseif ($type=="0") {
				$sqlcheck="Select * from tblEmpMast where $field='$value' and $field<>'' and not empNo='" . $_SESSION['strprofile'] . "' ";
			}
			$res=$this->execQry($sqlcheck);
			if ($this->getRecCount($res)==0) {
				if ($type=="1") {
					$sqlcheck="Select * from tblEmpMast_new where $field='$value' and $field<>''";
				}
				elseif ($type=="0") {
					$sqlcheck="Select * from tblEmpMast_new where $field='$value' and $field<>'' and not empNo='" . $_SESSION['strprofile'] . "' ";
				}
				$res=$this->execQry($sqlcheck);
			}
		}		
