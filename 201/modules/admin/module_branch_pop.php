<?
	...
	*/
	...
	if($_GET["btnUserDef"]!="")
	{
		$moduleAccssRghts->beginTranI();
		$Trns = "";
		$insQryBranch="";
		$usrAcc = $_GET["accesspop"];
		if($usrAcc!="")
		{
			$moduleAccssRghts->execQryI("Delete from tblUserBranch where compCode='".$_SESSION["company_code"]."' and empNo='".$_GET["empNo"]."'");
		
			foreach($usrAcc as $valBrnCode)
			{
				$insQryBranch = "Insert into tblUserBranch(compCode, empNo, brnCode) values('".$_SESSION["company_code"]."','".$_GET["empNo"]."','".$valBrnCode."'); ";
					$Trns = $moduleAccssRghts->execQryI($insQryBranch);
			}
			if($Trns==FALSE){
				$moduleAccssRghts->rollbackTranI();
				echo "<script type='text/javascript'>alert('Failed to update to User Branch Access.');</script>";
			}
			else{
				$moduleAccssRghts->commitTranI();
				echo "<script type='text/javascript'>alert('User Branch Access already updated.');</script>";
			}
			
		}
		else{
				echo "<script type='text/javascript'>alert('No Update to User Branch Access.');</script>";
		}
		exit();
	}
