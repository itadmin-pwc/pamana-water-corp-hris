...
if($_GET["createAcct"]=='createAccount')
{
	...
	$cntChkUsers = $moduleAccssRghts->chkUser($_GET["compCode"],$_GET["employeeNo"]);
	if($cntChkUsers=='1'){
...
<SCRIPT>
	...
	function editUserAccount(act, empNo, compCode)
	{
		...
		else if(act=='Edit Branch')
			winPrevEmp.setURL('module_branch_pop.php?act='+act+'&empNo='+empNo+'&compCode='+compCode);
		else
			winPrevEmp.setURL('module_access_level.php?act='+act+'&empNo='+empNo+'&compCode='+compCode);
	...
	function editPassword(url,title,empNo,compCode)
	{
		var winPrevEmp = new Window({
		id: "editUserPassword",
		className : 'mac_os_x',
		...
		hideEffect: Effect.SwitchOff, 
		draggable:true })
		winPrevEmp.setURL(url+'?empNo='+empNo+'&compCode='+compCode);
		winPrevEmp.showCenter(true);	
