...
switch($_GET['code']) {
	case "AddHoliday":
		$resqry=$maintEmpObj->recordChecker("Select compCode,holidayDate,brnCode,holidayDesc,dayType from tblHolidayCalendar where compCode='{$_SESSION['company_code']}' and holidayDate='{$_GET['txtdate']}' and brnCode='{$_GET['cmbbranch']}' and holidayDesc='{$_GET['txtdesc']}' and dayType='{$_GET['cmbday']}'");
		if($resqry){
			echo "alert('Holiday already exist.');";
			exit();
		}
		else{
	...
	break;
	case "EditHoliday":
		$resqry=$maintEmpObj->recordChecker("Select compCode,holidayDate,brnCode,holidayDesc,dayType from tblHolidayCalendar where compCode='{$_SESSION['company_code']}' and holidayDate='{$_GET['txtdate']}' and brnCode='{$_GET['cmbbranch']}' and holidayDesc='{$_GET['txtdesc']}' and dayType='{$_GET['cmbday']}' and seqno!='{$_GET['txtseqno']}'");
		if($resqry){
			echo "alert('Holiday already exist.');";
			exit();
		}
	...
	case "branch":
		if ($_GET['grp']!=0) {
			$maintEmpObj->DropDownMenu($maintEmpObj->makeArr(
					$maintEmpObj->getBranchperGrp($_GET['grp']),'brnCode','brnDesc','All'
				),
				'cmbbranch',$cmbbranch,'class="inputs"  title="Leave blank for all branches in a group."' 
			);
		} else {
			$maintEmpObj->DropDownMenu($maintEmpObj->makeArr($maintEmpObj->getBranch($_SESSION['company_code']),'brnCode','brnDesc','All'),'cmbbranch',$holinfo['brnCode'],'class="inputs" style="width:222px;"');		
		}	
		exit();
	break;	
}
	...
	<form action="" method="post" name="frmbank" id="frmbank">
      <table width="430" border="0" class="childGrid" cellpadding="2" cellspacing="1">
        ...
        <tr>
          ...
          <td class="gridDtlVal"><?
								$maintEmpObj->DropDownMenu($maintEmpObj->makeArr(
										$maintEmpObj->getBrnGrp(),'GrpCode','GrpDesc','All'
									),
									'cmbGrp',$cmbGrp,'class="inputs" onChange="getBranch(this.value);" title="Leave blank to apply Nationwide."' 
								);
							?>
        ...
        <tr>
          ...
          <td class="gridDtlVal">
            <div align="left" id="divBranch">
              <? $maintEmpObj->DropDownMenu($maintEmpObj->makeArr($maintEmpObj->getBranch($_SESSION['company_code']),'brnCode','brnDesc','All'),'cmbbranch',$holinfo['brnCode'],'class="inputs" style="width:222px;" title="Leave blank to apply Nationwide."'); ?>
          </div></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <?php if(($_SESSION['user_level']==1) || ($_SESSION['user_level']==2)){ ?>
           <input type="button" class="inputs" onClick="saveholiday();" name="save" id="save" value="Save">
