...
define('FPDF_FONTPATH','../../../includes/pdf/font');
	...
	$brnCode = $_GET['branch'];
	$branchName = $inqTSObj->getBranchName($compCode,$brnCode);
...
$worksheet->write(2,0,'STORE',$headerFormat); for($j=1; $j<=2; $j++){ $worksheet->write(2,$j,"",$headerFormat); } 
$worksheet->write(2,2,$branchName,'');
$worksheet->write(3,0,'GROUP',$headerFormat); for($j=1; $j<=2; $j++){ $worksheet->write(3,$j,"",$headerFormat); } 
$worksheet->write(3,2,$group,'');
$worksheet->write(4,0,'PAYROLL DATE',$headerFormat); for($j=1; $j<=2; $j++){ $worksheet->write(4,$j,"",$headerFormat); } 
$worksheet->write(4,2,$payrollDate,'');
$worksheet->write(5,0,'CUT OFF',$headerFormat); for($j=1; $j<=2; $j++){ $worksheet->write(5,$j,"",$headerFormat); } 
$worksheet->write(5,2,$cutoff,'');
...
$lastrow = 11;
	...
	$qryRank = "Select tblEmpMast_New.empRank, tblRankType.rankDesc
					...
					and tblEmpMast_New.empBrnCode='".mysql_result($resBr,$b,"brnCode")."' 
	...
	$qryRankDel = "Select tblEmpMast.empRank, tblRankType.rankDesc
					...
					where tblEmpMast.compCode='".$_SESSION['company_code']."' and tblEmpMast.empBrnCode='".mysql_result($resBr,$b,"brnCode")."' 
