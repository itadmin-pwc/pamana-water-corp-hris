...
switch ($inputId) {
	case "empSearch_blacklist":		
		...
		if ($empBrnCode!="0") {$empBrnCode1 = " $and (empBrnCode = '{$empBrnCode}')"; $where_clause = 1;} else {$empBrnCode1 = "";}
		...
		$sqlEmp = "SELECT * FROM tblBlacklistedEmp  
				   where
				   compCode<>-1 
				   $where $empBrnCode1 $empDept1 $empPos1 $dateEncoded1
				   ";	 
	...
	case "empSearch_ra1":	
		$sqlEmp = "Select * from tblEmpMast where compCode='".$empBrnCode."' and empStat NOT IN('RS','IN','TR') and empPayCat<>'0' and dateHired between '".date("Y-m-d", strtotime($monthfr))."' and '".date("Y-m-d", strtotime($monthto))."'";
		$resEmp = $inqTSObj->execQry($sqlEmp);	
		$numEmp = $inqTSObj->getRecCount($resEmp);		    
		...
		elseif ($numEmp >= 1) {
			if($listType=='r1A')
				echo "location.href = 'r1a_list_ajax.php?&empBrnCode=$empBrnCode&monthto=$monthto&monthfr=$monthfr'";
			else
				echo "location.href = 'er2_list_ajax.php?&empBrnCode=$empBrnCode&monthto=$monthto&monthfr=$monthfr'";
		}
	break;
