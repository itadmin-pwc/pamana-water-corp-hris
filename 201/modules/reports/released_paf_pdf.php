...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Ln();
		$this->Cell(100,5,"Report ID: RELEASEDPAF");
		if ($_GET['from'] != "" && $_GET['to'] != "") {
			$fromdt = $_GET['from'];
			$todt = $_GET['to'];
			$date = "$fromdt - $todt";
		} 
		$this->Cell(184,5,$this->reportlabel.'Released PAF');
...
$pdf->AddPage();
	...
	foreach($arrEmpList as $empListVal){
		$resArrOthers = $psObj->getPAF_others($empListVal['empNo'],$pafType,$datefilter." AND stat='R'",$type);
...
$pdf->Output('RELEASED_PAF_PROOFLIST.pdf','D');
