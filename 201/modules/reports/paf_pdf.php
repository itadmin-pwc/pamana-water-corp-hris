...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(184,5,$this->reportlabel.'Posted PAF (Group ' .$_GET['group'] .')');
...
if (empty($pafType) || $pafType =="others") {
	$psObj->arrOthers 		= $psObj->convertArr("tblPAF_Others$type", "  $datefilter1 AND empPayGrp='$group' $empNo1 $empName1 $empDiv1 $empDept1 $empSect1");
}
if (empty($pafType) || $pafType =="empstat") {
	$psObj->arrEmpStat 		= $psObj->convertArr("tblPAF_EmpStatus$type", "  $datefilter2 AND empPayGrp='$group' $empNo1 $empName1 $empDiv1 $empDept1 $empSect1");
}
if (empty($pafType) || $pafType =="branch") {	
	$psObj->arrBranch 		= $psObj->convertArr("tblPAF_Branch$type", "  $datefilter3 AND empPayGrp='$group' $empNo1 $empName1 $empDiv1 $empDept1 $empSect1");
}
if (empty($pafType) || $pafType =="position") {	
	$psObj->arrPosition 		= $psObj->convertArr("tblPAF_Position$type", "  $datefilter4 AND empPayGrp='$group' $empNo1 $empName1 $empDiv1 $empDept1 $empSect1");
}
if (empty($pafType) || $pafType =="payroll") {
	$psObj->arrPayroll 		= $psObj->convertArr("tblPAF_PayrollRelated$type", "  $datefilter5 AND empPayGrp='$group' $empNo1 $empName1 $empDiv1 $empDept1 $empSect1");
}
if (empty($pafType) || $pafType =="allow") {
	$psObj->arrAllow 		= $psObj->convertArr("tblPAF_Allowance$type", "   $datefilter6 AND empPayGrp='$group' $empNo1 $empName1 $empDiv1 $empDept1 $empSect1");
}
...
$pdf->Output('POSTED_PAF_PROOFLIST.pdf','D');
