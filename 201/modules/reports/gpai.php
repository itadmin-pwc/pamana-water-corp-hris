...
switch ($_GET['action']){
	case "startcutoff":
		$qryStartCutOff = $inqTSObj->makeArr($inqTSObj->getPayPrd($_SESSION['company_code']," and payGrp='".$_GET['paygroup']."' order by  year(pdFrmDate), month(pdFrmDate)"),'pdFrmDate','pdFrmDate','');
	...
	case "setBranchGrp";
		if($_GET['paygroup']==1){
			$where = " and compCode='".$_SESSION['company_code']."' and brnDefGrp='".$_GET['paygroup']."' or brnCode='0001'";	
		}
		else{
			$where = " and compCode='".$_SESSION['company_code']."' and brnDefGrp='".$_GET['paygroup']."'";	
		}
	
		$qryBranch = $inqTSObj->makeArr($inqTSObj->getBranchByCompGrp($where),'brnCode','brnDesc','All Branches');
        $inqTSObj->DropDownMenu($qryBranch,'cmbBranch','','class="inputs"');	exit();
	exit();
	break;
	
	case "endcutoff":
		$qryStartCutOff = $inqTSObj->makeArr($inqTSObj->getPayPrd($_SESSION['company_code']," and payGrp='".$_GET['paygroup']."' and pdFrmDate='".$_GET['startdate']."' order by year(pdToDate) ASC"),'pdToDate','pdToDate','');
	...
	case "processgpai":
		...
		$costart = $_GET['cutoffstart'];
		$coend = $_GET['cutoffend'];
		$branch = $_GET['branch'];
		
		$qry= $inqTSObj->getPayPrd($_SESSION['company_code']," and payGrp='".$pgroup."' and pdFrmDate='".$costart."' and pdToDate='".$coend."' and pdYear='".substr($coend,-4)."'");
		foreach($qry as $keyval=>$val){
			$pdpyable=date("F j, Y", strtotime($val['pdPayable']));	
		}
		if($branch!="0"){
			if($_GET['printFormat']=="gpai"){
				echo "window.location ='gpai_pdf.php?pgroup=$pgroup&costart=$costart&coend=$coend&branch=$branch&pddate=$pdpyable'";	
			}
			else{
				echo "window.location ='gpai_excel.php?pgroup=$pgroup&costart=$costart&coend=$coend&branch=$branch&pddate=$pdpyable'";	
			}
		}
		else{
			//echo "alert('Under construction!  n-h-o-m-e-r');";
			if($_GET['printFormat']=="gpai"){
				echo "window.location ='gpai_all_pdf.php?pgroup=$pgroup&costart=$costart&coend=$coend&pddate=$pdpyable'";	
			}
			else{
				echo "window.location ='gpai_excel_all.php?pgroup=$pgroup&costart=$costart&coend=$coend&branch=$branch&pddate=$pdpyable'";	
			}
		}
	exit();
	break;
...
<form name="frmgpai" id="frmgpai" method="" action="<?=$_SERVER['PHP_SELF'];?>">
	<table cellpadding="0" cellspacing="1" class="parentGrid" width="100%">
    	<tr>
        	<td class="parentGridHdr">&nbsp;<img src="../../../images/grid.png">&nbsp; GPAI Update </td>
        </tr>
        <tr>
        	<td class="parentGridDtl"><table cellspacing="1" cellpadding="1" border="0" width="100%" class="childGrid">
                ...
                <tr>
                    ...
                    <td class="gridDtlVal"><?
						$inqTSObj->DropDownMenu(array(""=>"","1"=>"Group 1"),'cmbGroup','','class="inputs" onChange="setCutOff(this.value); setBrnGrp(this.value);"');
					?></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td class="gridDtlVal"><div id="brnGrp"><?
						$qryBranch = $inqTSObj->makeArr($inqTSObj->getBranchByCompGrp(" and compCode='".$_SESSION['company_code']."'"),'brnCode','brnDesc','All Branches');
...
function setCutOff(id){
	var empInputs = $('frmgpai').serialize();
	...
	new Ajax.Request(params,{
		method	: 'get',
		parameters	: $('frmgpai').serialize(true),
...
function setBrnGrp(id){
	var empInputs = $('frmgpai').serialize();
	var grp = $('cmbGroup').value;
	var params;
		
	params = '<?=$_SERVER['PHP_SELF']?>?action=setBranchGrp&paygroup='+grp;
	new Ajax.Request(params,{
		method	: 'get',
		parameters	: $('frmgpai').serialize(true),
		onComplete	: function(req){
...
function refreshObj(id){
	var empInputs = $('frmgpai').serialize(true);
	if(id==1){
		$('gpai').disabled=false;	
		$('gpaiExcel').disabled=false;
...
function validateValues(id){
	...
	params = '<?=$_SERVER['PHP_SELF']?>?action=processgpai&paygroup='+pgrp+'&cutoffstart='+costart+'&cutoffend='+coend+'&branch='+brn+'&printFormat='+id;
	new Ajax.Request(params,{
		method	:	'get',
		parameters	:	$('frmgpai').serialize(true),
		onComplete	:	function(req){
