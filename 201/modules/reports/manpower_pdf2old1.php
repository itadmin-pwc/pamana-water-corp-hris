...
$page                   = 1;
############################ Q U E R Y ##################################
	    $sqlRD = "SELECT tblEmpMast.empNo, tblEmpMast.empLastName, tblEmpMast.empFirstName, tblEmpMast.empMidName, tblDepartment.deptDesc, 
				  tblPosition.posDesc, CONVERT(VARCHAR,tblEmpMast.empEndDate,101) AS empEndDate, salary_ko = CASE WHEN emppaytype = 'M' THEN 
				  empmrate WHEN emppaytype = 'D' THEN empdrate END, rateMode = CASE WHEN emppaytype = 'M' THEN 'Monthly' WHEN emppaytype = 'D' 
				  THEN 'Daily' END, tblEmpMast.empStat,emp_stat = case tblEmpMast.employmentTag when  'RG' then 'REGULAR' when  
				  'CN' then 'CONTRACTUAL' when 'PR' then 'PROBATIONARY' end, 
				  CONVERT(VARCHAR,tblEmpMast.dateHired,101) AS dateHired,empPayCat,CONVERT(VARCHAR,tblEmpMast.dateReg,101) AS dateReg 
				  FROM tblEmpMast 
...
for ($i=0;$i<$num;$i++){ 
	...
	$pdf->Cell(30,5,mysql_result($resGetDealsList,$i,"dateHired"),0,0);
	$pdf->Cell(25,5,mysql_result($resGetDealsList,$i,"empEndDate"),0,0);
	$pdf->Cell(25,5,mysql_result($resGetDealsList,$i,"dateReg"),0,0);	
	...
	$sqlAllow = "SELECT tblAllowType.allowDesc + '(' + tblAllowance.allowTag + ')=' + CONVERT(varchar, tblAllowance.allowAmt) AS allow FROM tblAllowance INNER JOIN tblAllowType ON tblAllowance.allowCode = tblAllowType.allowCode AND tblAllowance.compCode = tblAllowType.compCode WHERE (tblAllowance.empNo = ".mysql_result($resGetDealsList,$i,"empNo").")";
