...
if (empty($pafType) || $pafType =="others") {
	$psObj->arrOthers 		= $psObj->convertArr("tblPAF_Others$type", " AND stat='R' $datefilter $empNo1 $empName1 $empDiv1 $empDept1 $empSect1 $userview");
}
if (empty($pafType) || $pafType =="empstat") {
	$psObj->arrEmpStat 		= $psObj->convertArr("tblPAF_EmpStatus$type", " AND stat='R' $datefilter $empNo1 $empName1 $empDiv1 $empDept1 $empSect1 $userview");
}
if (empty($pafType) || $pafType =="branch") {	
	$psObj->arrBranch 		= $psObj->convertArr("tblPAF_Branch$type", " AND stat='R' $datefilter $empNo1 $empName1 $empDiv1 $empDept1 $empSect1 $userview");
}
if (empty($pafType) || $pafType =="position") {	
	$psObj->arrPosition 		= $psObj->convertArr("tblPAF_Position$type", " AND stat='R' $datefilter $empNo1 $empName1 $empDiv1 $empDept1 $empSect1 $userview");
}
if (empty($pafType) || $pafType =="payroll") {
	$psObj->arrPayroll 		= $psObj->convertArr("tblPAF_PayrollRelated$type", " AND stat='R' $datefilter $empNo1 $empName1 $empDiv1 $empDept1 $empSect1 $userview");
}
if (empty($pafType) || $pafType =="allow") {
	$psObj->arrAllow 		= $psObj->convertArr("tblPAF_Allowance$type", "  AND allowStat='R' $datefilter $empNo1 $empName1 $empDiv1 $empDept1 $empSect1 $userview");
}
...
$filename = 'Released PAF Proof List'.$todaynewdate. '.xls';
$workbook->send($filename);
$worksheet=&$workbook->addWorksheet('Released PAF Proof List');
...
$worksheet->write(1,0,"Released PAF Proof List",$headerFormat); for($j=1;$j<9;$j++){$worksheet->write(1,$j,"",$headerFormat);}
$worksheet->write(3,0,"Run Date: ".$today);
$worksheet->write(4,0,"Report ID: RELEASEDPAF");
