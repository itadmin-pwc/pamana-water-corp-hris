		...
		foreach($resDiv as $valDiv){
			...
			foreach($resDept as $valDept){
				...
				$this->Cell(43.7,5,$valDept['deptShortDesc'],'1','0');
					...
					where empBrnCode='".$as."' and empStat='RG' and empDiv='{$valDept['divCode']}' and empDepCode='{$valDept['deptCode']}' 
					...
					from tblEmpMast emp 
					inner join tblRankType rank on emp.empRank = rank.rankCode 
					where empBrnCode<>'0001' and empBrnCode='{$_GET['type']}' and empStat='RG' and empDiv='{$valDept['divCode']}' 
					and empDepCode='{$valDept['deptCode']}' 
					group by empStat,empRank,employmentTag, empDiv, empDepCode order by empStat,empRank";
...
####Set up to show data	
$pdf->Output('HEAD_COUNT_DIVISION_BRANCH_PROOFLIST.pdf','D');
?>