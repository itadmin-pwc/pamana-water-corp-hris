...
####################### HEADER 10.0012
	...
	$pdf->SPACES	       	= 5;
############################ Q U E R Y ##################################
	$arrEventLogs = $inqTSObj->CS($_GET['branch'],$_GET['hist'],$_GET['from'],$_GET['to'],$_GET['group']);
...
foreach($arrBranch as $valbranch) {
	...
	foreach ($arrEventLogs as $val){
		$reasons=$inqTSObj->getTblData("tblTK_Reasons "," and stat='A' and reason_id='{$val['csReason']}'"," order by reason","sqlAssoc");
		if ($valbranch['brnCode']==$val['brnCode']) {
			...
			$pdf->Cell(24,$SPACES,date('m/d/Y',strtotime($val['csDateFrom'])),0,0);
			$pdf->Cell(26,$SPACES,$val['csShiftFromIn'],0,0);
			$pdf->Cell(28,$SPACES,$val['csShiftFromOut'],0,0);

			$pdf->Cell(20,$SPACES,date('m/d/Y',strtotime($val['csDateTo'])),0,0);
			$pdf->Cell(28,$SPACES,$val['csShiftToIn'],0,0,'C');
			$pdf->Cell(36,$SPACES,$val['csHiftToOut'],0,0,'C');

			$pdf->Cell(24,$SPACES,$val['refNo'],0,0);
			$pdf->MultiCell(70,$SPACES,$reasons['reason'],0,'L');
...
function HEADER_FOOTER($pdf, $inqTSObj, $compCode, $TOTAL_WIDTH, $dt) {
	...
	$reppages 		= "";
	$repId    		= "Report ID: CSAPPLICATIONS";
	$repTitle 		= "CHange Shift Applications           $dt";
	$refNo    		= ""; 

	$dtlLabelDown   = "                  FROM                                       TO                        Ref. No.     Reason";
	$dtlLabelDown2   = "   DATE        Shift In     Shift Out       Date         Shift In      Shift Out";
	#########################################################################
