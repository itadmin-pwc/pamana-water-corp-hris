...
####################### HEADER 10.0012
	...
	$pdf->SPACES	       	= 5;
############################ Q U E R Y ##################################
	$arrEventLogs = $inqTSObj->OT_Prooflist($_GET['branch'],$_GET['hist'],$_GET['from'],$_GET['to'],$_GET['group']);
...
foreach($arrBranch as $valbranch) {
	...
	foreach ($arrEventLogs as $val){
		if ($valbranch['brnCode']==$val['brnCode']) {
			...
			$pdf->Cell(57,$SPACES,$val['dayTypeDesc'],0,0);
			$pdf->Cell(40,$SPACES,number_format($val['hrsOTLe8'],2),0,0,'C');
			$pdf->Cell(30,$SPACES,number_format($val['hrsOTGt8'],2),0,0,'C');
			$pdf->Cell(40,$SPACES,number_format($$val['hrsNDLe8'],2),0,0,'C');
...
function HEADER_FOOTER($pdf, $inqTSObj, $compCode, $TOTAL_WIDTH, $dt) {
	...
	$reppages 		= "";
	$repId    		= "Report ID: OTPROOFLIST";
	$repTitle 		= "OVERTIME PROOF LIST           $dt";
	$refNo    		= ""; 

	$dtlLabelDown   = "    DATE       DAY TYPE                            OT LESS 8        OT GREATER 8        ND LESS 8 ";
	$dtlLabelDown2   = "";
	#########################################################################
