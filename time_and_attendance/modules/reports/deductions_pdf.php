...
####################### HEADER 10.0012
	...
	$pdf->SPACES	       	= 5;
############################ Q U E R Y ##################################
	$arrEventLogs = $inqTSObj->Deductions($_GET['branch'],$_GET['hist'],$_GET['from'],$_GET['to'],$_GET['group']);
...
foreach($arrBranch as $valbranch) {
	...
	foreach ($arrEventLogs as $val){
		if ($valbranch['brnCode']==$val['brnCode']) {
			...
			$pdf->Cell(44,$SPACES,date('m/d/Y',strtotime($val['tsDate'])),0,0);
			$pdf->Cell(40,$SPACES,number_format($val['hrsTardy'],2),0,0,'C');
			$pdf->Cell(30,$SPACES,number_format($val['hrsUT'],2),0,0,'C');
...
function HEADER_FOOTER($pdf, $inqTSObj, $compCode, $TOTAL_WIDTH, $dt) {
	...
	$reppages 		= "";
	$repId    		= "Report ID: DEDPROOFLIST";
	$repTitle 		= "DEDUCTIONS PROOF LIST           $dt";
	$refNo    		= ""; 

	$dtlLabelDown   = "    DATE                                         TARDY            UNDERTIME ";
	$dtlLabelDown2   = "";
	#########################################################################
