...
<form name="frmTS" method="post" action="<? echo $_SERVER['PHP_SELF']; ?>">
  <table cellpadding="0" cellspacing="1" class="parentGrid" width="100%">
    <tr>
      ...
      <td class="parentGridHdr"> &nbsp;<img src="../../../images/grid.png">&nbsp;Violations Report</td>
	</tr>
	<tr>
		<td class="parentGridDtl"  height="35" ><TABLE border="0" width="100%" cellpadding="1" cellspacing="1" class="childGrid">
          <tr> 
            <td class="gridToolbar" colspan="7" > <input  name="hide_option" type="hidden" id="hide_option" value="<? echo $option_menu; ?>"> 
              ...
              <input name='fileName' type='hidden' id='fileName' value="violations.php"></td>
          </tr>
        </TABLE></td>  
    </tr>      
    <tr><td class="parentGridDtl" ><TABLE border="0" width="100%" cellpadding="1" cellspacing="1" class="childGrid" >
			...
			$arrBranch = $inqTSObj->getArrRes($inqTSObj->execQry($sqlBranch));
								...
								$inqTSObj->DropDownMenu($arrBranch,'branch',$empDiv,$cmbtable_dis);
							?>            </td>
            </tr>
          <tr>
          ...
          <tr> 
            <td class="gridDtlLbl">Violation Type </td>
            <td class="gridDtlLbl">:</td>
            <td colspan="2" class="gridDtlVal"><? 	
			$sqlVal = "Select * from tblTK_ViolationType where compCode='{$_SESSION['company_code']}' 
						and violationStat='A' order by violationDesc";				
			$arrVal = $inqTSObj->getArrRes($inqTSObj->execQry($sqlVal));
								$arrVals = $inqTSObj->makeArr($arrVal,'violationCd','violationDesc','');
								$inqTSObj->DropDownMenu($arrVals,'violations','',$cmbtable_dis);
							?></td>
	...
	<tr><td class="parentGridDtl"><TABLE border="0" width="100%" cellpadding="1" cellspacing="1" class="childGrid">
				  <tr>
					<td>
						<CENTER>
                <input type="button" name="salary" id="salary" class="inputs" <? echo $searchTS4_dis; ?> value="Employees Violations Report" onClick="ViolationsReport();">
					  </CENTER>
					</td>
				  </tr>
			  </table>    
