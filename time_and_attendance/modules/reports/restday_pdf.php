...
####################### HEADER 10.0012
	...
	$pdf->SPACES	       	= 5;
############################ Q U E R Y ##################################
	$arrEventLogs = $inqTSObj->RestDay($_GET['branch'],$_GET['hist'],$_GET['from'],$_GET['to'],$_GET['group']);
...
foreach($arrBranch as $valbranch) {
	...
	foreach ($arrEventLogs as $val){
		$reasons=$inqTSObj->getTblData("tblTK_Reasons "," and stat='A' and reason_id='{$val['cRDReason']}'"," order by reason","sqlAssoc");
		if ($valbranch['brnCode']==$val['brnCode']) {
			...
			$pdf->Cell(32,$SPACES,date('m/d/Y',strtotime($val['cRDDateFrom'])),0,0);
			$pdf->Cell(30,$SPACES,date('m/d/Y',strtotime($val['cRDDateTo'])),0,0);
			$pdf->Cell(30,$SPACES,date('m/d/Y',strtotime($val['dateFiled'])),0,0);
			$pdf->Cell(16,$SPACES,$val['refNo'],0,0);
...
function HEADER_FOOTER($pdf, $inqTSObj, $compCode, $TOTAL_WIDTH, $dt) {
	...
	$reppages 		= "";
	$repId    		= "Report ID: RDPROOFLIST";
	$repTitle 		= "REST DAY PROOF LIST           $dt";
	$refNo    		= ""; 

	$dtlLabelDown   = "    FROM DATE        TO DATE        DATE FILED    REF No    REASON ";
	$dtlLabelDown2   = "";
	#########################################################################
