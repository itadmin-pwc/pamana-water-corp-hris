...
####################### HEADER 10.0012
	...
	$pdf->SPACES	       	= 5;
############################ Q U E R Y ##################################
	$arrEventLogs = $inqTSObj->OverBreaks1($_GET['branch'],$_GET['hist'],$_GET['from'],$_GET['to'],$_GET['group']);
...
foreach($arrBranch as $valbranch) {
	...
	foreach ($arrEventLogs as $val){
		if ($valbranch['brnCode']==$val['brnchCd']) {
			...
			$pdf->Cell(25,$SPACES,$val['shftBreakOut'],0,0);
			$pdf->Cell(25,$SPACES,$val['shftBreakIn'],0,0);
			$pdf->Cell(25,$SPACES,$val['breakOut'],0,0);
			$pdf->Cell(20,$SPACES,$val['breakIn'],0,0);
			$pdf->Cell(20,$SPACES,(float)$val['n1']-15 ." min(s).",0,'1','C');
...
function HEADER_FOOTER($pdf, $inqTSObj, $compCode, $TOTAL_WIDTH, $dt) {
	...
	$repTitle 		= "OVER BREAK REPORT (Coffee Break)         $dt";
	$refNo    		= ""; 
