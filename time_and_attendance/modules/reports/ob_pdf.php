...
####################### HEADER 10.0012
	...
	$pdf->SPACES	       	= 5;
############################ Q U E R Y ##################################
	$arrEventLogs = $inqTSObj->OB($_GET['branch'],$_GET['hist'],$_GET['from'],$_GET['to'],$_GET['group']);
...
foreach($arrBranch as $valbranch) {
	...
	foreach ($arrEventLogs as $val){
		$reasons=$inqTSObj->getTblData("tblTK_Reasons "," and stat='A' and reason_id='{$val['obReason']}'"," order by reason","sqlAssoc");
		if ($valbranch['brnCode']==$val['brnCode']) {
			...
			$pdf->Cell(24,$SPACES,date('m/d/Y',strtotime($val['obDate'])),0,0);
			$pdf->Cell(30,$SPACES,date('m/d/Y',strtotime($val['dateFiled'])),0,0);
			
			$pdf->Cell(24,$SPACES,$val['refNo'],0,0);
			$pdf->Cell(20,$SPACES,$val['obSchedIn'],0,0);
			$pdf->Cell(24,$SPACES,$val['obSchedOut'],0,0,'C');
			$pdf->Cell(22,$SPACES,$val['obActualTimeIn'],0,0,'C');
			$pdf->Cell(20,$SPACES,$val['obActualTimeOut'],0,0,'C');
			$pdf->Cell(25,$SPACES,$val['obStat'],0,0,'C');
			$pdf->Cell(30,$SPACES,($val['hrs8Deduct']=="Y")?"YES":"NO",0,0,'C');
			$pdf->Cell(40,$SPACES,$val['userName'],0,0,'c');
			$pdf->Cell(25,$SPACES,$val['obDestination'],0,0,'C');
			$pdf->MultiCell(45,$SPACES,$reasons['reason'],0,'1','L');			
...
function HEADER_FOOTER($pdf, $inqTSObj, $compCode, $TOTAL_WIDTH, $dt) {
	...
	$reppages 		= "";
	$repId    		= "Report ID: OBAPPLICATIONS";
	$repTitle 		= "OB Applications           $dt";
	$refNo    		= ""; 

	$dtlLabelDown   = "    DATE       DATE FILED     REF. NO.    SCHED. IN    SCHED. OUT    ACT. IN    ACT. OUT    STATUS   8HRS. CREDITED   USER CREDITED        DESTINATION      REASON ";
	$dtlLabelDown2   = "";
	#########################################################################
