...
####################### HEADER 10.0012
	...
	$pdf->SPACES	       	= 5;
############################ Q U E R Y ##################################
	$arrEventLogs = $inqTSObj->offSetHour($_GET['branch'],$_GET['hist'],$_GET['from'],$_GET['to'],$_GET['group']);
...
foreach($arrBranch as $valbranch) {
	...
	foreach ($arrEventLogs as $val){
		if ($valbranch['brnCode']==$val['brnCode']) {
			...
			$pdf->Cell(25,$SPACES,date('m/d/Y',strtotime($val['tsDate'])),0,0,'L');
			$pdf->Cell(26,$SPACES,$val['hrsRequired'],0,1,'C');
			$ctr++;
...
function HEADER_FOOTER($pdf, $inqTSObj, $compCode, $TOTAL_WIDTH, $dt) {
	...
	$reppages 		= "";
	$repId    		= "Report ID: OFFSETHOURSWORK";
	$repTitle 		= "MORE THAN 9 HOURS OF SHIFT SCHEDULE REPORT           $dt";
	$refNo    		= ""; 

	$dtlLabelDown   = "                   EMPLOYEE                                       DATE       HOURS SCHEDULE";
	$dtlLabelDown2   = "";
	#########################################################################
