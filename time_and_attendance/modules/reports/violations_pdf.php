...
class PDF extends FPDF
{
	...
	function Header() {
		...
		$this->Cell(90,5,'VIOLATIONS REPORT',0,1);
		$this->Cell(195,7,'EMPLOYEE No. BIO No.         NAME                   DATE COMMITED             VIOLATIONS','TB',1);
	...
	function Main($arrViolations){
		...
		foreach($arrViolations as $valViolations){		
			...
			$this->SetFont('Courier','B',9);	
			$this->Cell(25,5,$valViolations['empNo'],0,0,'L');	
			$this->SetFont('Courier','',9);	
			$this->Cell(18,5,$valViolations['bioNumber'],0,0,'L');			
			...
			$this->Cell(28,5,$valViolations['dateCommited'],0,0,'L');
			$this->MultiCell(67,4,$valViolations['violationDesc'],0,1,'L');						
