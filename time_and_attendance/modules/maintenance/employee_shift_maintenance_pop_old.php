...
$arr_empInfo = $empShiftMaint->getEmployee($_SESSION["company_code"],$_GET["empNo"],'');
...
switch($_GET["action"])
{
	case "dispShiftDtl":
		...
		{
			$arr_ShiftCode_Dtl = $empShiftMaint->getShiftInfo("tblTK_ShiftDtl", " and shftCode='".$shiftcode."' and dayCode='".$dayI."'", " ");
	...
	case "Add":
		...
		$btnName = "Save";
		$arr_empBioInfo = $empShiftMaint->getShiftInfo("tblBioEmp", " and empNo='".($_GET["empNo"]!=""?$_GET["empNo"]:$_GET["txtEmpNo"])."'  and cast(locCode as int)='".$arr_empInfo["empBrnCode"]."'", " ");
	...
	case "Save":
		...
		$chkBio_EmpShiftDtl = $empShiftMaint->getShiftInfo('tblBioEmp', " and bioNumber='".$_GET["txtEmpBio"]."' and locCode='".$_GET["empBrnCode"]."'", '');
		
		if($_GET["txtEmpBio"]!="")
		{
			if(($chkBio_EmpShiftDtl["empNo"]!="") && ($chkBio_EmpShiftDtl["empNo"]!=$_GET["txtEmpNo"]))
			{
				echo "alert('Encoded Bio Number is being used by employee :".$chkBio_EmpShiftDtl["empNo"]."');";
				exit();
			}
			else
			{
	...
	case "Update":
		...
		if($_GET["txtEmpBio"]!="")
		{
			if($chkBio_EmpShiftDtl["empNo"]!="")
			{
				
				if($chkBio_EmpShiftDtl["empNo"]!=$_GET["txtEmpNo"])
				{
					echo "alert('Encoded Bio Number is being used by employee :".$chkBio_EmpShiftDtl["empNo"]."');";
					exit();
				}
	...
	case "Update Bio Id":
		...
		if($_GET["txtEmpBio"]!="")
		{
			if($chkBio_EmpShiftDtl["empNo"]!="")
			{
				...
				{
					if($empShiftMaint->maint_EmpShift("UpdateBio",$_GET))
						echo "alert('Employee Shift Successfully Updated.');";
					else
						echo "alert('Error in Updating Employee Shift.');";
				}
...
//$arr_OpenPeriod = $empShiftMaint->makeArr($empShiftMaint->getPeriodWil($_SESSION["company_code"],$arr_empInfo["empPayGrp"],$arr_empInfo["empPayCat"]," "),'pdSeries','pdPayable','');


$pdStat = " and pdTsStat='O'";
$period_Info = $empShiftMaint->getPeriodWil($_SESSION["company_code"],$arr_empInfo["empPayGrp"],$arr_empInfo["empPayCat"],$pdStat);
$pdSeries = $period_Info["pdSeries"];



$arr_period_Info = $empShiftMaint->getPayPeriod($_SESSION["company_code"], " and payGrp='".$arr_empInfo["empPayGrp"]."' and payCat='".$arr_empInfo["empPayCat"]."' and pdSeries='".$pdSeries."'");
...
<HTML>
	...
	<BODY>
        ...
        <input type="hidden" name="empBrnCode" id="empBrnCode" value="<?=$arr_empInfo["empBrnCode"]?>">
            ...
            <TABLE border="0" width="100%" cellpadding="1" cellspacing="1" class="childGrid" >
                ...
                <tr>
                    ...
                    <td  width='25%' class='gridDtlVal'>
                        <input type='text' class='inputs' name='txtEmpBio' id='txtEmpBio' style='width:60%;' value='<?=$empBioNum?>' onKeyDown="javascript:return dFilter (event.keyCode, this, '####');" >
                    </td>
                    ...
                    <td  width='25%' class='gridDtlVal'>
                        <?php 
                            $empShiftMaint->DropDownMenu(array('A'=>'Active','H'=>'Held'),'cmbShitCodeStat',$cmbShitCodeStat,'class="inputs" style="width:130px;"');
                        ?>
                    </td>
               </tr>
               ...
               <tr>
                    ...
                    <td  width='25%' class='gridDtlVal'>
						<? 					
                            $arrShifts = $empShiftMaint->makeArr($empShiftMaint->getListShift(),'shiftCode','shiftDesc','');
                            $empShiftMaint->DropDownMenu($arrShifts,'shiftcode',$shiftcode,"onChange=\"getShiftCodeDetail();\" ".$disabled."");
                        ?>
                       
                    </td>
                    
                    <td width='25%' class='gridDtlLbl' align='left'>Cut Off </td>
                    <td width='1%' class='gridDtlLbl' align='center'>:</td>
                        
                    <td  width='25%' class='gridDtlVal'>
                        <?php 
                           //$empShiftMaint->DropDownMenu($arr_period_Info,'pdSeries',$pdSeries,"readonly");
                        ?>
                        <input type="hidden" name="txtPdNum" id="txtPdNum" value="<?=$arr_period_Info["pdNumber"]?>">
                        <input type="hidden" name="txtPdYear" id="txtPdYear" value="<?=$arr_period_Info["pdYear"]?>">
                        <input type="text" class="inputs" style="width:100%;" name="opnPayPeriod" id="opbPayPeriod" readonly value="<?=date("m/d/Y", strtotime($arr_period_Info["pdFrmDate"]))." - ".date("m/d/Y", strtotime($arr_period_Info["pdToDate"]));?>">
                    </td>
               </tr>
                ...
                <tr>
                	<td colspan="6" >
                    	<table border="1" width="100%" style="border-collapse:collapse;">
                        	<tr class='gridDtlVal'>
                            	<td><input type="checkbox" name="chkAbsentExemptTag"  onClick="return readOnlyCheckBox();" value="1" <?php echo ($chkAbsentExemptTag=='Y'?"checked":"");?>></td>
                                <td><font class='gridDtlLblTxt'><b>Absent Exempt</font></td>
                                ...
                                <td><font class='gridDtlLblTxt'><b>UT Time  Exempt</font></td>
                                 <td><input type="checkbox" name="chkLunchExemptTag" onClick="return readOnlyCheckBox();"   value="1" <?php echo ($chkLunchExemptTag=='Y'?"checked":"");?>></td>
                                <td><font class='gridDtlLblTxt'><b>Lunch Hrs. Exempt</font></td>
                            </tr>
                            <tr class='gridDtlVal'>
                                ...
                                <td colspan="3"><font class='gridDtlLblTxt'><b>OT Exempt</font></td>
