...
class transactionObj extends commonObj {
	...
	*/
	...
	{	
		...
		{
			case "Add":
				$Qry_TsCorr = "Insert into tblTK_TimeSheetCorr(compCode, empNo, tsDate, timeIn, lunchOut, lunchIn, breakOut,breakIn, timeOut, crossTag, editReason, encodeDate, encodedBy, stat) 
								values('".$_SESSION["company_code"]."', '".$array["empNo"]."', '".$array["txttsDate"]."', ".$time[1].", ".$time[2].", 
									   ".$time[3].", ".$time[4].", ".$time[5].",  ".$time[6].", ".($array["txtCheckTag"]!=""?"'Y'":"NULL").", ".($array["violationCd"]==0?"NULL":"'".$array["violationCd"]."'").", '".date("m/d/Y")."'
			...
			case "Update":
				...
				$Qry_TsCorr = "Insert into tblTK_TimeSheetCorr_original(compCode, empNo, tsDate, timeIn, lunchOut, lunchIn, breakOut,breakIn, timeOut, crossTag, editReason, encodeDate, encodedBy, stat) 
								values('".$_SESSION["company_code"]."', '".$array["empNo"]."', ".date("m/d/Y", strtotime($arr_EmpTsInfo["tsDate"])).", ".($arr_EmpTsInfo["timeIn"]!=""?"'".$arr_EmpTsInfo["timeIn"]."'":"NULL").", ".($arr_EmpTsInfo["lunchOut"]!=""?"'".$arr_EmpTsInfo["lunchOut"]."'":"NULL").", 
									   ...
									   ".($arr_EmpTsInfo["crossTag"]!=""?"'Y'":"NULL").", ".($arr_EmpTsInfo["editReason"]==0?"NULL":"'".$arr_EmpTsInfo["editReason"]."'").", '".date("m/d/Y", strtotime($arr_EmpTsInfo["encodeDate"]))."'
		...
		switch($action)
		{
			...
			while ($i < count($a)) {
				...
				if(substr(htmlspecialchars(urldecode($b[0])),0,13)=='chkDayEnabled')
				{
					...
					$qryIns.= "Insert into tblTK_ScheduleHist(compCode,empNo,tsDate,dayType,shftTimeIn,shftLunchOut,shftLunchIn,shftBreakOut,shftBreakIn,shftTimeOut, tsAppTypeCd, dateEdited,editedBy)
							   ...
							   ".($arr_EmpTsInfo["tsAppTypeCd"]!=""?"'".$arr_EmpTsInfo["tsAppTypeCd"]."'":"NULL").", '".date("m/d/Y")."', '".$_SESSION["employee_number"]."');\n";
					//Get DayType
