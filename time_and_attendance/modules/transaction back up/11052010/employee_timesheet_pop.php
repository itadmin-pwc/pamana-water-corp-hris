...
switch($_GET["action"])
{
	...
	case "getShiftCodeDtl":
		...
		{
			echo "$('txtElunchIn').value='".$arr_EmpTsInfo["lunchIn"]."';";
			echo "$('txtElunchOut').value='".$arr_EmpTsInfo["lunchOut"]."';";
		}
		exit();
	break;
...
<SCRIPT>
	...
	function getShiftCodeDetail(test)
	{
		...
		params = 'employee_timesheet_pop.php?action=getShiftCodeDtl&empNo='+empNo+'&tsDate='+tsDate+'&violationCd='+test;
