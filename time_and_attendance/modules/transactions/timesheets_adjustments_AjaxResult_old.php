...
//display records
$qrygetTimesheetAdjustmentsDtl = "SELECT TOP $intLimit 
				  					 taj.compCode,taj.empNo,taj.tsDate,taj.dayType,emp.empFirstName,
									 emp.empMidName,emp.empLastName,emp.empNo,taj.seqNo,taj.adjBasic,taj.adjOt,
									 taj.adjNd,taj.adjHp,taj.adjEcola,taj.adjCtpa,taj.adjAdv,taj.tsStat
				   				FROM tblTK_TimesheetAdjustment taj 
				   				INNER JOIN tblEmpMast emp ON taj.compCode = emp.compCode 
									AND taj.empNo = emp.empNo 
				   				WHERE taj.seqNo NOT IN (SELECT TOP $intOffset taj.seqNo
										 				FROM tblTK_TimesheetAdjustment taj 
														INNER JOIN tblEmpMast emp ON taj.compCode = emp.compCode 
															AND taj.empNo = emp.empNo 
				                         				WHERE taj.compCode = '{$_SESSION['company_code']}' $brnCodelist
...
$qrygetTimesheetAdjustmentsDtl .= "ORDER BY emp.empLastName)";
		...
		<?
				...
				?>
                			<tr style="height:20px;"  bgcolor="<?php echo $bgcolor; ?>" <?php echo $on_mouse; ?>>
                                ...
                                <?php
									if(($timesheetAdjustmentsDtlVal["tsStat"]=='O') || (($timesheetAdjustmentsDtlVal["userApproved"]==$_SESSION['employee_number']) && ($timesheetAdjustmentsDtlVal["tsStat"]=='P')))
									{
								...
								<td class="gridDtlVal" align="CENTER"><font color="<?=$f_color?>"><?=date("m/d/Y", strtotime($timesheetAdjustmentsDtlVal["tsDate"]));?></td>
