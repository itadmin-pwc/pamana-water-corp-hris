...
$qryIntMaxRec = "SELECT dtl.empNo
				 ...
				 and dtl.payGrp in (Select payGrp from tblProcGrp where status='A')				 AND dtl.refNo = '{$_GET['refNo']}' ";
        if($_GET['isSearch'] == 1){
        	if($_GET['srchType'] == 0){
...
$qryGetEarnDtl = "SELECT dtl.compCode,dtl.refNo,dtl.empNo,dtl.trnCntrlNo,dtl.trnAmount,dtl.payCat,
				   	emp.empFirstName,emp.empMidName,emp.empLastName,emp.empPayGrp,emp.empPayCat
				  FROM tblEarnTranDtl as dtl 
...
$qryGetEarnDtl .=	"	and dtl.payGrp in (Select payGrp from tblProcGrp where status='A')
		...
		<td class="parentGridDtl" valign="top">
			...
			<TABLE width="100%" cellpadding="1" cellspacing="1" border="0" class="hdrTable">
				...
				<tr>
					...
					<td class="gridDtlVal" colspan="4">
						<?
							$opnPrd = $earningsObj->getPayPeriod_OtherEarn($_SESSION['company_code'],"AND  payCat = '3' AND pdStat = 'O' and payGrp in (Select payGrp from tblProcGrp) ");
							if($cmbPeriod == "-"){
								$cmbPeriod = $opnPrd['pdYear']."-".$opnPrd['pdNumber'];
							}
							$earningsObj->DropDownMenu(
								$earningsObj->makeArrDate(
									$earningsObj->getPayPeriod_OtherEarn($_SESSION['company_code'],"AND payGrp in (Select payGrp from tblProcGrp) AND payCat = '3' AND pdNumber >= '{$opnPrd['pdNumber']}' and pdYear>='{$opnPrd['pdYear']}'"),'pdYear','pdNumber','pdPayable',''
				...
				<tr>
					...
					<td class="gridDtlVal">
						<?
							$earningsObj->DropDownMenu(
								$earningsObj->makeArr(
									$earningsObj->getTransType($_SESSION['company_code'],'earnings',''),'trnCode','trnDesc',''
			...
			<TABLE width="100%" cellpadding="0" cellspacing="1" border="0" class="hdrTable">
				...
				<tr>
					...
					<td class="gridDtlVal" align="center">
						<input type="hidden" name="hdnPayCat" id="hdnPayCat" />
