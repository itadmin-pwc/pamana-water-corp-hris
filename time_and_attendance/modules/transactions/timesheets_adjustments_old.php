...
//			}			
		...
		case ('updateTimesheetAdjustment'):
			...
			$timesheetchecker = $timesheetsadjustmentsObj->recordChecker("Select * from tblTK_TimesheetAdjustment where tsDate='{$tsdate}' and  seqNo<>'{$seqno}'");
			if(!$timesheetchecker){
		...
		case 'Approved':
			...
			foreach($chkSeqNo as $indchkSeqNo => $chkSeqNo_val)
			{
				$qryApprove = "UPDATE tblTK_TimesheetAdjustment 
							   SET dateApproved='".date("m/d/Y")."',userApproved='".$_SESSION["employee_number"]."', 
		...
		case "getSeqNo":
			...
			$qry = $timesheetsadjustmentsObj->execQry("SELECT ts.compcode, ts.empNo, ts.tsDate, ts.dayType, ts.payGrp, ts.payCat, ts.pdYear, 
															ts.pdNumber, ts.entryTag, ts.includeAllowTag, ts.includeAdvTag, ts.hrsReg, 
															ts.hrsOtLe8, ts.hrsOtGt8, ts.hrsNd, ts.hrsNdGt8, ts.adjBasic, ts.adjOt, ts.adjNd, 
															ts.adjHp, ts.adjEcola, ts.adjCtpa, ts.adjAdv, ts.tsStat, ts.seqNo, emp.empLastName, 
			...
			echo "$('txtTSAmntDate').value='".date("m/d/Y", strtotime($resQry['tsDate']))."';";
			...
			if($resQry['entryTag']=="H"){
				echo "$('txtTSDate').value='".date("m/d/Y", strtotime($resQry['tsDate']))."';";
