		...
		<td class="parentGridDtl" valign="top">
			...
			<TABLE width="100%" cellpadding="1" cellspacing="1" border="0" class="hdrTable">
				...
				<tr>
					...
					<td class="gridDtlVal" colspan="4">
						<?
							$opnPrd = $earningsObj->getPayPeriod_OtherEarn($_SESSION['company_code'],"AND  payCat = '3' AND pdStat = 'O' ");//and payGrp in (Select payGrp from tblProcGrp)//AND payGrp in (Select payGrp from tblProcGrp) 
							if($cmbPeriod == "-"){
								$cmbPeriod = $opnPrd['pdYear']."-".$opnPrd['pdNumber'];
							}
							$earningsObj->DropDownMenu(
								$earningsObj->makeArrDate(
									$earningsObj->getPayPeriod_OtherEarn($_SESSION['company_code'],"AND payCat = '3' AND pdNumber >= '{$opnPrd['pdNumber']}' and pdYear>='{$opnPrd['pdYear']}'"),'pdYear','pdNumber','pdPayable',''
