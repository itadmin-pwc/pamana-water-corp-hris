...
switch($_GET["action"])
{
	...
	case "saveRdSched":
			$i=$_GET['rdnTran'];
			//Check no. of Records			
			$arr_RdRec = $crdObj->getTblData("tblTK_ChangeRDApp", " and empNo='".$_GET["txtAddEmpNo"]."' and cRDDateTo='".date("Y-m-d", strtotime($_GET["rdDateTo$i"]))."' and cRDStat='P'", "", "sqlAssoc");
			
		
			$getPdNum_NxtCutOff = $crdObj->getTblData("tblPayPeriod", " and payGrp='".$_GET["empPayGrp"]."' and payCat='".$_GET["empPayCat"]."'  and  pdTSStat='O' ", "", "sqlAssoc");
			...
			$arr_EmpTsDtl = $crdObj->getTblData("tblTK_Timesheet", " and empNo='".$_GET["txtAddEmpNo"]."'  and dayType in ('02','05','06')", "", "");
			foreach($arr_EmpTsDtl as $arr_EmpTsDtl_Val)
			{
				$currDateRd.=date("Y-m-d", strtotime($arr_EmpTsDtl_Val["tsDate"])).",";
			}
			...
			//Check From Date
			$getPdNum_Open = $crdObj->getTblData("tblPayPeriod", " and payGrp='".$_GET["empPayGrp"]."' and payCat='".$_GET["empPayCat"]."'  and '".$_GET["rdDateFrom$i"]."' between pdFrmDate and pdToDate and pdTSStat='O' ", "", "sqlAssoc");
			...
			else
			{
				...
				if($getPdNum_Open["pdSeries"]!="")
				{
					 ...
					 if(in_array(date("Y-m-d", strtotime($_GET["rdDateFrom$i"])), $currDateRd))
					 {
					 	 /*Check if the To Date is between the Open Cut Off*/
						$getPdNum_OpenToDate = $crdObj->getTblData("tblPayPeriod", " and payGrp='".$_GET["empPayGrp"]."' and payCat='".$_GET["empPayCat"]."'  and '".$_GET["rdDateTo$i"]."' between pdFrmDate and pdToDate and pdTSStat='O' ", "", "sqlAssoc");
						if($getPdNum_OpenToDate["pdSeries"]!="")
						{
							if(in_array(date("Y-m-d", strtotime($_GET["rdDateTo$i"])), $currDateRd))
					 		{
								if($_GET["Edited"]!="")
								{
						...
						{
							$getPdNum_OpenToDate2 = $crdObj->getTblData("tblPayPeriod", " and payGrp='".$_GET["empPayGrp"]."' and payCat='".$_GET["empPayCat"]."'   and '".$_GET["rdDateTo$i"]."' between pdFrmDate and pdToDate and pdNumber='".$pdNum."' and pdYear='".$pdYear."' ", "", "sqlAssoc");
							if($getPdNum_OpenToDate2["pdSeries"]!="")
							{
								if(date("w", strtotime($_GET["rdDateTo$i"]))!=$empCurrRd)
								{
									$insRecRdTran = $crdObj->tran_Crd($_GET,($_GET["Edited"]!=""?"Update":"Add"));
									echo "alert('".$insRecRdTran."')";
								}
				...
				{
					$getPdNum_Open2 = $crdObj->getTblData("tblPayPeriod", " and payGrp='".$_GET["empPayGrp"]."' and payCat='".$_GET["empPayCat"]."'   and '".$_GET["rdDateFrom$i"]."' between pdFrmDate and pdToDate and pdNumber='".$pdNum."' and pdYear='".$pdYear."' ", "", "sqlAssoc");
					
					if($getPdNum_Open2["pdSeries"]!="")
					{
						if(date("w", strtotime($_GET["rdDateFrom$i"]))==$empCurrRd)
						{
							/*Check if the To Date is between the Open Cut Off*/
							$getPdNum_OpenToDate = $crdObj->getTblData("tblPayPeriod", " and payGrp='".$_GET["empPayGrp"]."' and payCat='".$_GET["empPayCat"]."'  and '".$_GET["rdDateTo$i"]."' between pdFrmDate and pdToDate and pdTSStat='O' ", "", "sqlAssoc");
							if($getPdNum_OpenToDate["pdSeries"]!="")
	...
	case "Delete":
		...
		{
			echo "alert('Select RD Application to be deleted.')";
		}
		exit();
	break;
	...
	case "disabledFields":
		for($x=1; $x<=10; $x++)
		{
			if($x==$_GET["onField"])
			{
				...
				echo "document.frmRD.cmbReasons".$_GET["onField"].".disabled=false; ";
			}
			else
			{
				echo "document.frmRD.btnSave".$x.".disabled=true; ";
				echo "document.frmRD.cmbReasons".$x.".disabled=true; ";
			}
		}
...
<SCRIPT>
	...
	function saveRdDetail(id)
	{
		
		var rdFields = $('frmRD').serialize(true);
		var n = document.getElementById('rdnTran'+id).value;
		...
		if((rdFields["rdDateFrom"+id]=="")||(rdFields["rdDateFrom"+id]==":"))
		{
		...
		if((rdFields["rdDateTo"+id]=="")||(rdFields["rdDateTo"+id]==":"))
		{
		...
		if(rdFields["rdDateFrom"+id]==rdFields["rdDateTo"+id])
		{
			alert("No changes to be made.");
			return false;
		}
		
		if(rdFields["cmbReasons"+id]==0)
		{
