...
<SCRIPT>
	...
	pager('employee_timesheet_listAjaxResult.php','empTimeSheet','load',0,0,'','','&empNo=<?=$_GET['empNo']?>&pdSeries=<?=$_GET["pdSeries"]?>&url=<?=$_GET["url"]?>','../../../images/');  
	
	function changePayPeriod()
	{
		var pdSeries = document.frmEmpTSheet.pdSeries.value;
		pager('employee_timesheet_listAjaxResult.php','empTimeSheet','load',0,0,'','','&empNo=<?=$_GET['empNo']?>&url=<?=$_GET["url"]?>&pdSeries='+pdSeries,'../../../images/');  
	}
	...
	function maintShiftCode(act,tsDate,empNo,pdSeries,URL,ele,offset,maxRec,isSearch,txtSrch,cmbSrch)
	{
		...
		draggable:true })
		editAllw.setURL('employee_timesheet_pop.php?&action='+act+'&tsDate='+tsDate+'&empNo='+empNo+'&url=<?=$_GET["url"]?>');
		editAllw.show(true);
		editAllw.showCenter();	
		  ...
		  myObserver = {
		    onDestroy: function(eventName, win) {
		      ...
		      if (win == editAllw) {
		        editAllw = null;
		        pager('employee_timesheet_listAjaxResult.php','empTimeSheet','load',0,0,'','','&empNo='+empNo+'&pdSeries='+pdSeries+'&url=<?=$_GET["url"]?>','../../../images/');  
