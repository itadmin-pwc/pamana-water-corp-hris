...
//					$hrsOt = (($hrsWrk-$SchedHrsWork)<$OtHrs) ? $hrsWrk-$SchedHrsWork:$OtHrs;
				...
				if ($hrsWrk>4) {
					...
					$hrsregND=	($hrsregND > 0) ? $hrsregND:0;
				...
				$hrsOt 		= $time['hrsOT'];
				if ($hrsWrk>0 || $hrsOt>0 ){
				$hrsND 		= ($time['hrsND']=="")? $time['hrsregND']:$time['hrsND'];
				
				}
				//$hrsND 		= $time['hrsND']; //originalcode
			...
			$hrsND 		= ($hrsND<$hrsregND)? $hrsND:$hrsND-$hrsregND;
				...
				if ($hrsWrk == 0 && $otTag =="" && $valTSList['dayType']=='01' )  {
				$hrsND = $hrsregND = 0;
			}
			$otTag = "";
			if ($hrsregND=="" && $hrsND !=""){//alejocode
					$hrsregND=$hrsND ;
					$hrsregND=($hrsregND>8) ? 8:$hrsregND;
					$hrsND 		= 0;
				}
...
*/			
		...
		foreach($this->getOTforComputation() as $valOT) {
			...
			$amtRegNDLe8 = round((float)$valOT['hrsRegNDLe8'] * (float)$valOT['ndPrem8'] * ((float)$valOT['empDrate']/8),2);
...
/*					$OTOut = $arr['otOut'];
				...
				if (in_array($arr['empDiv'],array('2','3','4','5','6','7','10')) && !in_array($arr['dayType'],array('02','05','06'))) {	
					...
					//compute ND
					if ((float)str_replace(":",".",$arr['timeOut'])<22 &&(float)str_replace(":",".",$arr['otOut'])<22 && $arr['otCrossTag']=='Y')  {
						$dtTo = ((float)str_replace(":",".",$arr['timeOut'])<(float)str_replace(":",".",$arr['otOut'])) ? $this->DateAdd($arr['tsDate'])." {$arr['timeOut']}":$this->DateAdd($arr['tsDate'])."  {$arr['otOut']}";
						$time['hrsND'] += round($this->calDiff("{$arr['tsDate']} 22:00",$dtTo,'m')/60,2);
					}
