...
//					$timeIn = ((float)str_replace(":",".",$valTSList['timeIn'])>(float)str_replace(":",".",$valTSList['shftTimeIn'])) ? "{$valTSList['tsDate']} {$valTSList['timeIn']}":"{$valTSList['tsDate']} {$valTSList['shftTimeIn']}";						
				...
				$hrsTardy = ((float)str_replace(":",".",$valTSList['timeIn'])>(float)str_replace(":",".",$valTSList['shftTimeIn'])) ? round($this->calDiff("{$valTSList['tsDate']} {$valTSList['timeIn']}","{$valTSList['tsDate']} {$valTSList['shftTimeIn']}",'m')/60,2):0;
...
//					$hrsOt = (($hrsWrk-$SchedHrsWork)<$OtHrs) ? $hrsWrk-$SchedHrsWork:$OtHrs;
			...
			$hrsregND 	= ($hrsregND >= 5 && $shfthrsLunch!=0.5) ? $hrsregND-1:$hrsregND;
			...
			if ($hrsOt >= 0.50) {
				if (in_array($valTSList['dayType'],array('03','04')) && $SchedHrsWork==3.5) {
					$hrsOt = $hrsOt + 5.5;
				}
				if (!$this->checkExemp($valTSList['empNo'],'OT')) {
				...
				if ($valTSList['empBrnCode'] == 0001 && $valTSList['CWWTag']=='' && $sat==6 && $hrsOt>=3 ){
...
*/			
		...
		foreach($this->getempLegalHolidays() as $valHol) {
			if ((float)$valHol['hrsOT']>0) {
				$sqlLegHold .=" Update tblTK_timesheet set legalPayTag='Y' where empNo='{$valHol['empNo']}' and tsDate='{$valHol['tsDate']}'; \n";
			} else {
				if (in_array($valHol['empNo'],$arrEmpWithPay)) {
					...
					if ($this->checkExemp($valHol['empNo'],'Absent')) {
						...
						$legalTag = $this->getEmpTimesheet($valHol['empNo'],date('Y-m-d',strtotime($valHol['tsDate'])),$valHol['dayType']);
						$datehol =date('Y-m-d',strtotime($valHol['tsDate']));
						$sqlLegHold .=" Update tblTK_timesheet set legalPayTag='$legalTag' where empNo='{$valHol['empNo']}' and tsDate='$datehol';\n";
					}
				}
			}
		}
...
/*					if ($arr['empNo'] == '280000309')
					...
					if ($arr['tsAppTypeCd']==13) {
						$time['hrsWork'] = ($time['hrsWork'] + $SchedHrsWork) - $time['hrsTardy'] - $time['hrsUT'];					
...
/*					$OTOut = $arr['otOut'];
				...
				{
					$time['hrsWork'] = ($time['hrsWork']>=5) ? $time['hrsWork']-$shfthrsLunch:$time['hrsWork'];
				}	
...
*/
	...
	function getSchedHrsWork($arr) {
		if ((float)str_replace(":",".",$arr['shftTimeOut'])==0) {
			...
			if($arr['crossDay']=="Y"){
				...
				$tOut = $this->DateAdd($arr['tsDate']) . " {$arr['shftTimeOut']}";//EDITED BY ALEJO
				$hrsWork = round($this->calDiff($tIn,$tOut,'m')/60,2)-1;
			}
			else{
	...
	/*	if ($empNo == '040001617' && date('Y-m-d',strtotime($tsDate)) == '3/23/2013') {
			echo "wil $sqlTScur\n";
		}*/
	...
	function getempLegalHolidays() {
		$sqlEmpLegalHolidays = "Select tk.empNo,tk.tsDate,tk.dayType,hrsOtle8 as hrsOT from tblTK_Timesheet tk 
										inner join tblEmpMast emp on tk.empNo=emp.empNo 
										left join tblTK_overtime ot on tk.empNo=ot.empNo and tk.tsDate=ot.tsDate
										where tk.daytype IN ('03','05') and tk.compCode='{$_SESSION['company_code']}' AND tk.tsDate between '{$this->pdFrom}' AND '{$this->pdTo}' AND  empPayGrp='{$this->Group}'
