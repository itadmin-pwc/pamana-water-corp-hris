...
//ALEJO UPDATE 2020
	...
	$appTag = "";
		...
		if ($NewdayType != $arrTS['dayType']) {
			$qrynewday = "update tbltk_timesheet set dayType = '$NewdayType' where empNo='$empNo' and compCode='{$_SESSION['company_code']}' and tsDate='$tsDate'";
			$this->execQryI($qrynewday);
...
*/
		...
		if ($ctr>0) 
		{
				...
				if ($arrTS['obTag']=='') 
				{
					...
					if ($timeOutUT == "")	{
						$arrQry[] = "Update tblTK_Timesheet Set empNo='$empNo' $checkTag " . $this->ProcessEventLog($ctr,$arrLog,$arrTS['tsAppTypeCd'],$arrTS)." $appTag $logsExceededTag where empNo='$empNo' and compCode='{$_SESSION['company_code']}' and tsDate='$tsDate';";
						//$arrQry[] = "Update tblTK_EventLogs set ProcessTag='Y' where $dayTypeField cStoreNum='{$arrTS['empBrnCode']}' AND ETAG = '{$arrTS['bioNo']}' AND EDATE='".date('Ymd',strtotime($tsDate))."'";
				...
				else 
				{
					...
					$arrQry[] = "Update tblTK_Timesheet Set empNo='$empNo' $checkTag $timeInOB $timeOutOB  $appTag $logsExceededTag where empNo='$empNo' and compCode='{$_SESSION['company_code']}' and tsDate='$tsDate';";
					// $dayTypeField $arrQry[] = "Update tblTK_EventLogs set ProcessTag='Y' where cStoreNum='{$arrTS['empBrnCode']}' AND ETAG = '{$arrTS['bioNo']}' AND EDATE='".date('Ymd',strtotime($tsDate))."'";
					//$arrQry[] = "Update tblTK_EventLogs set ProcessTag='Y' where ETAG = '{$arrTS['bioNo']}' AND EDATE='".date('Ymd',strtotime($tsDate))."'";
					return $arrQry;
				}
	...
	function checkHolidayDate($tsDate,$RDTag,$brnCode,$empdiv) {
		...
		foreach($this->arrHolidays as $valHol) {
			...
			$qcday='2021-08-19';
			$hdate=date('Y-m-d',strtotime($valHol['holidayDate']));

			if( $hdate== date('Y-m-d',strtotime($qcday)) && $empdiv == '6' && date('Y-m-d',strtotime($qcday)) == date('Y-m-d',strtotime($tsDate))  ){
				if ($RDTag=="Y") {
					$dayType='02';
				}else{
					$dayType='01';
...
FROM
tk_logs
RIGHT JOIN tbltk_timesheet ON tk_logs.empNo = tbltk_timesheet.empNo AND tk_logs.compCode = tbltk_timesheet.compcode AND  tbltk_timesheet.tsDate = tk_logs.tsdate
LEFT JOIN tblempmast ON tk_logs.empNo = tblempmast.empNo AND tk_logs.compCode = tblempmast.compCode
  	";
