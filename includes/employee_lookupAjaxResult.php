...
if ($_SESSION['module_id'] != 1){
	$qryIntMaxRec = "SELECT * FROM tblEmpMast
				 WHERE compCode = '{$compCode}' and empPayGrp='{$_SESSION['pay_group']}' and empPayCat='{$_SESSION['pay_category']}' ";
}else{
	$qryIntMaxRec = "SELECT * FROM tblEmpMast
...
if ($_SESSION['pay_category'] == 9) {		
	...
	if ($_SESSION['pay_group'] != '') {
		$qryIntMaxRec .=	"AND empStat NOT IN('RS','IN','TR')  and empPayGrp='{$_SESSION['pay_group']}' and empPayCat='{$_SESSION['pay_category']}'
					 	";
	}else{
		//added cinderu - for emp look up in Time and attendance
		$qryIntMaxRec .=	"AND empStat NOT IN('RS','IN','TR') AND empBrnCode IN (SELECT brnCode FROM tblTK_UserBranch WHERE compCode ='{$_SESSION['company_code']}'and empNo = '{$_SESSION['employee_number']}')
						    ";
		//end
	}
}
...
if ($_SESSION['pay_category'] == 9) {		
	$qryEmpList .=	"AND empStat IN ('RS','EOC','AWOL')  and empPayGrp='{$_SESSION['pay_group']}' and empNo IN (Select empNo from tblLastPayEmp where compCode='{$_SESSION['company_code']}')
					 ORDER BY empLastName";
} else {
	//added cinderu - for emp look up in Time and attendance
	if ($_SESSION['pay_group'] == ''){					 
		$qryEmpList .=	"AND empStat NOT IN('RS','IN','TR','EOC','AWOL') AND empBrnCode IN (SELECT brnCode FROM tblTK_UserBranch WHERE compCode ='{$_SESSION['company_code']}' and empNo = '{$_SESSION['employee_number']}') ORDER BY empLastName";
	//end 
	}else{
